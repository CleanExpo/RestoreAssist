  ...
  try {
    ...
    if (job.status === "SCRIPT_READY") {
      if (!job.voiceoverText) {
        throw new Error(
          `Job ${job.id} is SCRIPT_READY but has no voiceoverText`,
        );
      }
...
export async function runContentPipeline(
  ...
  if (!voiceId) {
    await prisma.contentJob.update({
      ...
      data: {
        status: "FAILED",
        errorMessage: "ELEVENLABS_VOICE_ID is not configured",
      },
    });
    return { jobId: job.id, finalStatus: "FAILED" };
  }
  ...
  try {
    ...
    console.log(
      `[ContentPipeline] Video submitted for job ${job.id} (HeyGen: ${heygenRenderJobId})`,
    );
