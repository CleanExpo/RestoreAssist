...
function getDriveClient() {
  ...
  if (!credentials.client_email || !credentials.private_key) {
    throw new Error(
      "Google Drive service account credentials not configured. Please set GOOGLE_CLIENT_EMAIL and GOOGLE_PRIVATE_KEY in .env",
    );
  }
...
export async function searchDriveFiles(
  ...
  try {
    ...
    console.error(
      `[Google Drive Search] Error searching for "${keyword}":`,
      error.message,
    );
...
export function getStandardsFolderId(): string {
  return (
    process.env.GOOGLE_DRIVE_STANDARDS_FOLDER_ID ||
    "1lFqpslQZ0kGovGh6WiHhgC3_gs9Rzbl1"
  );
}
