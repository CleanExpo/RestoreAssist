...
export function buildVisualCentricReportPrompt(data: {
  ...
  const {
    ...
    dryingIndex,
    dryingStatus,
    totalAmpsNumber: _totalAmpsNumber,
    hasHazards,
    isOccupied,
    ...
    affectedContents: _affectedContents,
    structuralConcerns: _structuralConcerns,
    buildingServices: _buildingServices,
    insuranceConsiderations: _insuranceConsiderations,
    timelineRequirements: _timelineRequirements,
    dryingPreferences: _dryingPreferences,
    chemicalTreatment: _chemicalTreatment,
    totalAffectedArea: _totalAffectedArea,
    class4Drying: _class4Drying,
  } = data;
  ...
  equipmentSelection && equipmentSelection.length > 0
    ? (() => {
        ...
        if (dehumidifiers.length > 0) {
          ...
          const totalCostRow = totalDailyRate * estimatedDays;
          rows.push(
            `| LGR (${totalQty}) | $${totalDailyRate.toFixed(2)} | $${totalCostRow.toFixed(2)} |`,
          );
        }
        ...
        if (airMovers.length > 0) {
          ...
          const totalCostRow = totalDailyRate * estimatedDays;
          rows.push(
            `| Air (${totalQty}) | $${totalDailyRate.toFixed(2)} | $${totalCostRow.toFixed(2)} |`,
          );
        }
