...
export class AnswerMappingEngine {
    ...
    question.fieldMappings.forEach((mapping) => {
      ...
      if (populatedField) {
        ...
        if (mapping.transformer) {
          derivationNotes[mapping.formFieldId] =
            `Derived from question "${question.text}" using field transformer`;
        } else if (mapping.value !== undefined && mapping.value !== answer) {
          derivationNotes[mapping.formFieldId] =
            `Static value from field mapping`;
        } else {
          derivationNotes[mapping.formFieldId] =
            `Direct answer to question "${question.text}"`;
        }
      }
    });
  ...
  private static mapFieldMapping(
    ...
    try {
      ...
      const finalConfidence = Math.round(
        (baseConfidence * answerConfidence) / 100,
      );
  ...
  static mapMultipleAnswers(
    ...
    const confidenceValues = Object.values(allFieldPopulations).map(
      (p) => p.confidence,
    );
  ...
  static generateQualityReport(fieldPopulations: {
    ...
    const completeness = Math.min(
      100,
      (popArray.length / estimatedTotalFields) * 100,
    );
    ...
    const averageConfidence =
      popArray.length > 0
        ? Math.round(
            popArray.reduce((sum, p) => sum + p.confidence, 0) /
              popArray.length,
          )
        : 0;
    ...
    if (completeness < 50) {
      recommendations.push(
        "Only 50% of form fields auto-populated. Review interview for completeness.",
      );
    }
    ...
    if (completeness >= 90 && averageConfidence >= 85) {
      recommendations.push(
        "✓ High quality auto-population. Ready for form submission.",
      );
    }
  ...
  static exportAsFormPayload(fieldPopulations: {
    ...
    const avgConfidence =
      popArray.length > 0 ? Math.round(totalConfidence / popArray.length) : 0;
