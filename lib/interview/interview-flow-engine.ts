...
export class InterviewFlowEngine {
  ...
  private static getNextQuestion(
    ...
    if (state.currentQuestion?.skipLogic) {
      ...
      if (nextQuestionId) {
        const skipToQuestion = state.allQuestions.find(
  ...
  static goToPreviousQuestion(state: InterviewSessionState): {
    ...
    for (let i = state.currentQuestionIndex - 1; i >= 0; i--) {
      ...
      if (
        QuestionGenerationEngine.evaluateConditionalShow(
          question,
          state.answers,
        )
      ) {
    ...
    question.fieldMappings.forEach((mapping) => {
      ...
      if (mapping.transformer) {
        try {
          ...
          console.error(
            `Transformer error for field ${mapping.formFieldId}:`,
            e,
          );
          return;
        }
  ...
  private static updateProgressPercentage(state: InterviewSessionState): void {
    ...
    state.progressPercentage =
      state.totalQuestionsCount > 0
        ? Math.round(
            (state.questionsAnsweredCount / state.totalQuestionsCount) * 100,
          )
        : 0;
  }
   ...
   */
  static calculateIICRCClassification(state: InterviewSessionState): {
    category: number;
    class: number;
    reasoning: string;
    ...
    let category = 1;
    if (
      waterSource === "grey_water" ||
      waterSource === "slightly_contaminated"
    ) {
      category = 2;
    } else if (
      waterSource === "black_water" ||
      waterSource === "heavily_contaminated" ||
      hasMold
    ) {
      category = 3;
    }
    ...
    reasons.push(
      `Class ${waterClass}: ${this.getClassName(waterClass)} area affected (${affectedArea}%)`,
    );
    if (timeSinceLoss > 72) {
  ...
  private static generateRecommendedActions(
    ...
    if (materialsAffected.includes("carpet")) {
      actions.push(
        "Assess carpet salvageability (typically removed if wet >12h)",
      );
    }
  ...
  static restoreSession(
    ...
    const currentQuestion = questionResponse.questions.find(
      (q) => !savedAnswers[q.id],
    );
    if (currentQuestion) {
  ...
  static validateCompletion(state: InterviewSessionState): {
    ...
    const missingQuestions = state.allQuestions.filter(
  ...
  static getDiagnostics(state: InterviewSessionState): {
    ...
    return {
      ...
      unansweredQuestions:
        state.totalQuestionsCount - state.questionsAnsweredCount,
