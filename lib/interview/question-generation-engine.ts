...
export class QuestionGenerationEngine {
  ...
  static generateQuestions(
    ...
    const tieredQuestions = this.organizeTiers(
      questions,
      context.userTierLevel,
    );
   ...
   */
  private static buildQuestionLibrary(
    context: QuestionGenerationContext,
  ): Question[] {
    ...
    return questionLibrary.filter((q) => {
      ...
      if (
        q.minTierLevel &&
        this.getTierLevel(q.minTierLevel) >
          this.getTierLevel(context.userTierLevel)
      ) {
        return false;
      }
  ...
  private static organizeTiers(
    ...
    const tier2 = questions.filter(
      (q) => q.sequenceNumber && q.sequenceNumber > 5 && q.sequenceNumber <= 8,
    );
    const tier3 = questions.filter(
      (q) => q.sequenceNumber && q.sequenceNumber > 8 && q.sequenceNumber <= 13,
    );
    const tier4 = questions.filter(
   ...
   */
  static evaluateConditionalShow(
    question: Question,
    previousAnswers: Map<string, any>,
  ): boolean {
  ...
  private static answerMatches(answer: any, expectedValue: any): boolean {
    if (Array.isArray(expectedValue)) {
      return (
        Array.isArray(answer) && answer.some((a) => expectedValue.includes(a))
      );
    }
    return (
      answer === expectedValue ||
      String(answer).toLowerCase() === String(expectedValue).toLowerCase()
    );
  }
  ...
  static validateQuestion(question: Question): {
    ...
    if (
      !question.standardsReference ||
      question.standardsReference.length === 0
    ) {
    ...
    question.fieldMappings?.forEach((mapping, index) => {
      if (!mapping.formFieldId)
        errors.push(`Field mapping ${index} missing formFieldId`);
