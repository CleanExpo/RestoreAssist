...
describe("Interview Question Templates", () => {
  describe("Library Structure", () => {
    ...
    it("should have questions from all 4 tiers", () => {
      ...
      const tier2 = INTERVIEW_QUESTION_LIBRARY.filter(
        (q) =>
          q.sequenceNumber && q.sequenceNumber > 5 && q.sequenceNumber <= 8,
      );
      const tier3 = INTERVIEW_QUESTION_LIBRARY.filter(
        (q) =>
          q.sequenceNumber && q.sequenceNumber > 8 && q.sequenceNumber <= 13,
      );
      const tier4 = INTERVIEW_QUESTION_LIBRARY.filter(
        (q) => !q.sequenceNumber || q.sequenceNumber > 13,
      );
  ...
  describe("Individual Question Quality", () => {
    ...
    it("each question should have text longer than 5 characters", () => {
      const shortTextQuestions = INTERVIEW_QUESTION_LIBRARY.filter(
        (q) => q.text.length < 5,
      );
    ...
    it("each question should have valid type", () => {
      ...
      const invalidTypes = INTERVIEW_QUESTION_LIBRARY.filter(
        (q) => !validTypes.includes(q.type as string),
      );
  ...
  describe("Standards References", () => {
    it("each question should reference at least 1 standard", () => {
      const noStandards = INTERVIEW_QUESTION_LIBRARY.filter(
        (q) => !q.standardsReference || q.standardsReference.length === 0,
      );
    ...
    it("each question should have standards justification", () => {
      const noJustification = INTERVIEW_QUESTION_LIBRARY.filter(
        (q) =>
          !q.standardsJustification || q.standardsJustification.length === 0,
      );
    ...
    it("should cover key standards across library", () => {
      const allReferences = INTERVIEW_QUESTION_LIBRARY.flatMap(
        (q) => q.standardsReference,
      ).join(" ");
  ...
  describe("Field Mappings", () => {
    it("each question should have at least 1 field mapping", () => {
      const noMappings = INTERVIEW_QUESTION_LIBRARY.filter(
        (q) => !q.fieldMappings || q.fieldMappings.length === 0,
      );
    ...
    it("field mappings should have valid formFieldId", () => {
      const invalidMappings = INTERVIEW_QUESTION_LIBRARY.filter(
        (q) =>
          q.fieldMappings &&
          q.fieldMappings.some(
            (m) => !m.formFieldId || m.formFieldId.length === 0,
          ),
      );
    ...
    it("direct field mappings should have high confidence (>85)", () => {
      const directMappings = INTERVIEW_QUESTION_LIBRARY.flatMap((q) =>
        q.fieldMappings
          .filter((m) => !m.transformer)
          .map((m) => ({ questionId: q.id, mapping: m })),
      );
  ...
  describe("Skip Logic", () => {
    it("skip logic rules should have valid nextQuestionId", () => {
      const invalidSkipLogic = INTERVIEW_QUESTION_LIBRARY.filter((q) => {
        ...
        return q.skipLogic.some(
          (rule) => !rule.nextQuestionId || rule.nextQuestionId.length === 0,
        );
      });
    ...
    it("skip logic nextQuestionId should reference existing questions", () => {
      ...
      const invalidReferences = INTERVIEW_QUESTION_LIBRARY.filter((q) => {
        if (!q.skipLogic) return false;
        return q.skipLogic.some(
          (rule) => !questionIds.has(rule.nextQuestionId),
        );
      });
  ...
  describe("Conditional Shows", () => {
    ...
    it("conditional show rules should have valid operators", () => {
      ...
      const invalidOperators = INTERVIEW_QUESTION_LIBRARY.filter((q) => {
        if (!q.conditionalShows) return false;
        return q.conditionalShows.some(
          (c) => !validOperators.includes(c.operator),
        );
      });
  ...
  describe("Tier Organization", () => {
    ...
    it("tier 4 questions should primarily have premium minTierLevel", () => {
      ...
      const tier4Premium = tier4.filter(
        (q) => q.minTierLevel === "premium" || q.minTierLevel === "enterprise",
      );
  ...
  describe("Helper Functions", () => {
    ...
    describe("getQuestionsForSubscriptionTier", () => {
      ...
      it("should return standard + premium questions for premium subscription", () => {
        ...
        expect(premiumQuestions.length).toBeGreaterThanOrEqual(
          standardQuestions.length,
        );
        premiumQuestions.forEach((q) => {
          expect(
            !q.minTierLevel ||
              q.minTierLevel === "standard" ||
              q.minTierLevel === "premium",
          ).toBe(true);
        });
      });
      ...
      it("should return all questions for enterprise subscription", () => {
        const enterpriseQuestions =
          getQuestionsForSubscriptionTier("enterprise");

        expect(enterpriseQuestions.length).toBe(
          INTERVIEW_QUESTION_LIBRARY.length,
        );
      });
      ...
      it("premium subscription should have more questions than standard", () => {
        ...
        expect(premiumQuestions.length).toBeGreaterThan(
          standardQuestions.length,
        );
      });
      ...
      it("enterprise should have most questions", () => {
        ...
        expect(premiumQuestions.length).toBeLessThanOrEqual(
          enterpriseQuestions.length,
        );
      });
    });
  });
      ...
      INTERVIEW_QUESTION_LIBRARY.forEach((q) => {
        if (q.skipLogic) {
          ...
          q.skipLogic.forEach((rule) => {
            const nextQ = INTERVIEW_QUESTION_LIBRARY.find(
              (sq) => sq.id === rule.nextQuestionId,
            );
            if (nextQ?.sequenceNumber) {
              ...
              expect(
                Math.abs((nextQ.sequenceNumber || 999) - qSeq),
  ...
  describe("Documentation Quality", () => {
    ...
    it("standards justification should be meaningful (>20 characters)", () => {
      const shortJustifications = INTERVIEW_QUESTION_LIBRARY.filter(
        (q) => q.standardsJustification.length < 20,
      );
