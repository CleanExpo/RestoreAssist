...
export function analyzeAutoTriggers(
  tier1: any,
  tier2: any,
  tier3: any,
): AutoTrigger[] {
  ...
  if (tier1.T1_Q2_constructionYear?.includes("Pre-1970")) {
    triggers.push({
      ...
      action:
        "Automatically flag asbestos risk and lead paint risk. Adjust PPE requirements. Add specialist referral notes.",
      severity: "critical",
    });
  }
  ...
  if (
    waterSource.includes("Category 2") ||
    ...
    triggers.push({
      ...
      action:
        "Automatically trigger biohazard protocols. Flag EPA notification requirement. Add specialist contractor requirements. Adjust PPE and air filtration requirements.",
      severity: "critical",
    });
  }
  ...
  if (materials.some((m: string) => m.includes("Yellow tongue"))) {
    triggers.push({
      ...
      action:
        "Automatically trigger Class 4 drying protocols. Add sandwich drying methodology. Add injection system requirements. Flag for manual specialist quote.",
      severity: "critical",
    });
  }
    ...
    hazards.forEach((hazard: string) => {
      if (hazard.includes("asbestos")) {
        triggers.push({
          ...
          action:
            "Create STOP WORK flag. Add specialist referral line items. Flag WorkSafe notification. Create cost line for abatement specialist. May prevent certain work from being quoted.",
          severity: "critical",
        });
      }
      if (hazard.includes("mould")) {
        triggers.push({
          ...
          action:
            "Create STOP WORK flag. Refer to IICRC S520-certified specialist. Flag for mould remediation cost.",
          severity: "critical",
        });
      }
      if (hazard.includes("Biohazard")) {
        triggers.push({
          ...
          action:
            "Create STOP WORK flag. Flag EPA notification. Add biohazard specialist requirements.",
          severity: "critical",
        });
      }
  ...
  if (
    tier1.T1_Q1_propertyType?.includes("Multi-storey") ||
    ...
    triggers.push({
      ...
      action:
        "Automatically trigger water migration questions in Tier 2. Adjust drying protocols for multi-level. Extend timeline estimates.",
      severity: "info",
    });
  }
  ...
  if (
    waterDuration.includes("3-7 days") ||
    ...
    triggers.push({
      ...
      action:
        "Automatically flag secondary damage risk. Add mould advisory. Extend drying timeline. Add occupant health warning.",
      severity: "warning",
    });
  }
  ...
  if (tier1.T1_Q1_propertyType?.includes("Heritage")) {
    triggers.push({
      ...
      action:
        "Trigger heritage-specific building codes. Conservation protocols required. Material preservation requirements.",
      severity: "warning",
    });
  }
  ...
  if (
    tier2?.T2_Q5_structuralConcerns &&
    ...
    triggers.push({
      ...
      action:
        "Flag builder/carpenter assessment required. May require structural engineer clearance.",
      severity: "warning",
    });
  }
  ...
  if (tier3?.T3_Q5_class4DryingAssessment?.includes("Class 4 confirmed")) {
    triggers.push({
      ...
      action:
        "Requires qualified specialist assessment and quote. Not included in preliminary estimate.",
      severity: "critical",
    });
  }
...
export function applyAutoTriggersToReport(
