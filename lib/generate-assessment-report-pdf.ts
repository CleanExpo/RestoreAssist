...
export async function generateAssessmentReportPDF(
  ...
  const insurerName =
    report.insurerName || report.clientName || "Not specified";
  ...
  const phase1Start = report.phase1StartDate
    ? new Date(report.phase1StartDate)
    : null;
  const phase1End = report.phase1EndDate
    ? new Date(report.phase1EndDate)
    : null;
  const phase2Start = report.phase2StartDate
    ? new Date(report.phase2StartDate)
    : null;
  const phase2End = report.phase2EndDate
    ? new Date(report.phase2EndDate)
    : null;
  const phase3Start = report.phase3StartDate
    ? new Date(report.phase3StartDate)
    : null;
  const phase3End = report.phase3EndDate
    ? new Date(report.phase3EndDate)
    : null;
