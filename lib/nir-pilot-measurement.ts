...
import { EVIDENCE_REGISTER, type EvidenceClaim } from '@/lib/nir-evidence-architecture'
...
export interface ClaimCriteria {
  ...
  evaluate: (nirObs: PilotObservation[], controlObs: PilotObservation[]) => ClaimEvaluation
}
...
const PILOT_CRITERIA: ClaimCriteria[] = [
  ...
  {
    claimId: 'CLAIM-002',
    description: 'Per-claim cost differential (AUD): NIR vs. existing-format claims',
    ...
    evaluate(nir, control) {
      if (nir.length < 50 || control.length < 50) {
        return {
          ...
          observationsNeeded: Math.max(0, 50 - nir.length) + Math.max(0, 50 - control.length),
        }
      }
      ...
      const met         = difference > 0 && welchTTest(nir.map(o => o.value), control.map(o => o.value)) < 0.05
      return {
        met,
        summary: met
  ...
  {
    claimId: 'CLAIM-003',
    description: 'Re-inspection rate: proportion of claims requiring a second inspection',
    ...
    evaluate(nir, control) {
      ...
      const ctrlRate    = control.length >= 50 ? mean(control.map(o => o.value)) * 100 : null
      const met         = nirRate < 10
      const ctrlSummary = ctrlRate !== null ? `; control rate ${ctrlRate.toFixed(1)}%` : ' (control baseline pending)'
      return {
        met,
        summary: met
  ...
  {
    claimId: 'CLAIM-004',
    description: 'Adjuster report review time (minutes): NIR vs. non-standardised formats',
    ...
    evaluate(nir, control) {
      ...
      const reduction = ctrlMean > 0 ? ((ctrlMean - nirMean) / ctrlMean) * 100 : 0
  ...
  {
    claimId: 'CLAIM-005',
    description: 'Technician ease-of-use rating (1–5 scale): proportion rating ≥4',
  ...
  {
    claimId: 'CLAIM-007',
    description: 'Claim cycle time (business days from open to scope approval): NIR vs baseline',
    ...
    evaluate(nir) {
      ...
      const companies  = new Set(nir.map(o => o.context?.['companyId']).filter(Boolean)).size
      return {
        met: met && companies >= 3,
        summary: met && companies >= 3
...
function welchTTest(a: number[], b: number[]): number {
  ...
  const meanA = mean(a), varA = variance(a), nA = a.length
  const meanB = mean(b), varB = variance(b), nB = b.length
...
export function deriveCycleTimeObservations(
  ...
  return inspections
    .filter(i => i.status === 'COMPLETED' && i.inspectionDate && i.completedAt)
    .map(i => {
...
export function evaluateClaimReadiness(
  ...
  if (!claim || (claim.status !== 'HYPOTHESIS' && claim.status !== 'DERIVED')) return null
 ...
 */
export function generatePilotReport(observations: PilotObservation[]): PilotReport {
  ...
  for (const r of inProgress) {
    ...
    if (!evaluation.sufficientData) {
      ...
      const parts: string[] = []
      if (needNir > 0)  parts.push(`${needNir} more NIR ${criteria.observationType.replace(/_/g, ' ')} observations`)
      if (needCtrl > 0) parts.push(`${needCtrl} more control ${criteria.observationType.replace(/_/g, ' ')} observations`)
      actionItems.push(`${r.claim.id}: Collect ${parts.join(' + ')} — ${criteria.description}`)
    } else {
      actionItems.push(`${r.claim.id}: Data collected but threshold not yet met — ${evaluation.summary}`)
    }
  }
  ...
  return {
    ...
    actionItems,
    pilotComplete: readyToPromote.length === results.length && results.length > 0,
  }
}
...
export function validateObservation(obs: NewPilotObservation): string[] {
  ...
  if (!validClaimIds.includes(obs.claimId)) {
    errors.push(`claimId '${obs.claimId}' is not a HYPOTHESIS claim. Valid: ${validClaimIds.join(', ')}`)
  }
  ...
  if (obs.claimId === 'CLAIM-003' && obs.value !== 0 && obs.value !== 1) {
    errors.push('CLAIM-003 reinspection_event values must be 0 (no re-inspection) or 1 (re-inspection required)')
  }
