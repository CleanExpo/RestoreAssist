  ...
  providers: [
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        if (!user) {
          console.log(
            "[Contractor Credentials] User not found:",
            credentials.email,
          );
          logSecurityEvent({
        ...
        if (credentials.password?.startsWith("gauth:")) {
          ...
          if (parts.length === 3) {
            ...
            if (age > -5000 && age < 60_000) {
              ...
              if (
                ...
                console.log(
                  "[Contractor Credentials] Google auth token verified:",
                  credentials.email,
                );
                logSecurityEvent({
          ...
          console.log(
            "[Contractor Credentials] Invalid Google auth token for:",
            credentials.email,
          );
          return null;
        }
        ...
        if (isPasswordEmpty) {
          ...
          if (!user.password) {
            console.log(
              "[Contractor Credentials] Google user authenticated:",
              credentials.email,
            );
            logSecurityEvent({
              eventType: "LOGIN_SUCCESS",
          ...
          console.log(
            "[Contractor Credentials] Password required for user:",
            credentials.email,
          );
          return null;
        }
        ...
        if (!user.password) {
          console.log(
            "[Contractor Credentials] User has no password but password was provided:",
            credentials.email,
          );
          return null;
        }
        ...
        if (!isPasswordValid) {
          console.log(
            "[Contractor Credentials] Invalid password for user:",
            credentials.email,
          );
          logSecurityEvent({
            eventType: "LOGIN_FAILED",
        ...
        console.log(
          "[Contractor Credentials] User authenticated successfully:",
          credentials.email,
        );
        logSecurityEvent({
          eventType: "LOGIN_SUCCESS",
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        if (!clientUser) {
          console.log(
            "[Client Credentials] Client user not found:",
            credentials.email,
          );
          logSecurityEvent({
            eventType: "LOGIN_FAILED",
        ...
        if (!isPasswordValid) {
          console.log(
            "[Client Credentials] Invalid password for client:",
            credentials.email,
          );
          logSecurityEvent({
            eventType: "LOGIN_FAILED",
        ...
        console.log(
          "[Client Credentials] Client authenticated successfully:",
          credentials.email,
        );
