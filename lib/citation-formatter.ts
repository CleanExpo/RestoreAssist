 ...
 */
const DOCUMENT_STANDARDS: Record<
  string,
  { name: string; code: string; jurisdiction: string }
> = {
...
export function normalizeDocumentName(
  ...
  if (lowerInput.includes("as/nzs 3000"))
    return DOCUMENT_STANDARDS["AS/NZS 3000"];
  if (lowerInput.includes("as/nzs 3500"))
    return DOCUMENT_STANDARDS["AS/NZS 3500"];
  if (lowerInput.includes("as 1668")) return DOCUMENT_STANDARDS["AS 1668"];
  if (lowerInput.includes("as/nzs 3666"))
    return DOCUMENT_STANDARDS["AS/NZS 3666"];
  if (lowerInput.includes("consumer law"))
    return DOCUMENT_STANDARDS["Australian Consumer Law"];
  if (lowerInput.includes("work health"))
    return DOCUMENT_STANDARDS["Work Health and Safety Act 2011"];
  if (lowerInput.includes("insurance contract"))
    return DOCUMENT_STANDARDS["Insurance Contracts Act 1984"];
 ...
 */
export function formatSectionReference(
  sectionType: string,
  number: string,
): string {
  switch (sectionType) {
...
export function formatMultiPartCitation(
  ...
  const partStrings = parts.map((p) =>
    formatSectionReference(p.type, p.number),
  );
