...
export async function generateForensicReportPDF(
  ...
  const inspectionDate = report.technicianAttendanceDate
    ? new Date(report.technicianAttendanceDate).toLocaleDateString("en-AU", {
        day: "2-digit",
        month: "long",
        year: "numeric",
      })
  ...
  const bioMouldDetected =
    ...
    (report.microbialGrowth && report.microbialGrowth !== "");
  ...
  const companyName =
    businessInfo?.businessName && businessInfo.businessName.trim()
...
async function renderPage1(
  ...
  // Summary text
  const summaryText = buildForensicSummary(
    options.report,
    options.analysis,
    waterCategory,
    waterClass,
  ...
  if (bioMouldDetected) {
    ...
    const bioBadgeText = bioMouldCategory
      ? `BIO/MOULD: POSITIVE - ${bioMouldCategory}`
      : "BIO/MOULD: POSITIVE";
  ...
  const expertisePoints = buildExpertisePoints(
    options.report,
    options.analysis,
    options.standardsContext || "",
  );
  ...
  const ripRepairPoints = buildRipRepairPoints(
    options.report,
  ...
  const mitigationPoints = buildMitigationPoints(
    options.report,
    options.timelineData,
  );
...
async function renderPage2(
  ...
  const remediationText =
    "The remediation is to scope the whole structural implementation, remediation, and preservation of remediation structures, testing remediation barometers, and resource of the whole collocation rate the Investigation Chomtice:";
  ...
  scopeItems.forEach((item, index) => {
    ...
    // Justification
    const justLines = wrapText(
      item.justification,
      colWidths.justification - 10,
      helvetica,
      9,
    );
...
async function renderPage3(
  ...
  if (psychrometricData?.readings && psychrometricData.readings.length > 0) {
    ...
    psychrometricData.readings
      ...
      .forEach((reading: any, index: number) => {
        ...
        page.drawText(
          sanitizeTextForPDF(String(reading.temperature || "N/A")),
          {
            x: colX,
            y: yPosition - 15,
            size: 9,
  ...
  for (let week = 1; week <= Math.min(weeks, 4); week++) {
    ...
    const weekX =
      margin +
      (week - 1) * (ganttWidth / Math.min(weeks, 4)) +
      ganttWidth / Math.min(weeks, 4) / 2 -
      25;
  ...
  phases.forEach((phase: TimelinePhase, index: number) => {
    ...
    // Phase name
    const phaseName =
      phase.name.length > 20 ? phase.name.substring(0, 17) + "..." : phase.name;
...
async function renderPage4(
  ...
  const authText =
    "The Authorisation & Acceptance is processed for both details, allow safer and common sections, and foremostly accepted tusuler site-vaimcrintioort umiermatarove amcohstiamentes. Please are both your comehers for authertation a signature:";
...
function addHeaderFooter(
  ...
  if (isFirstPage) {
    ...
    if (sectionTitle) {
      ...
      if (jobRef) {
        const refWidth = helvetica.widthOfTextAtSize(
          sanitizeTextForPDF(jobRef),
          9,
        );
...
function buildScopeItems(
  ...
  items.push({
    item: "Hazard Control",
    description:
      "Preparative remediation is consistent from the tasking of hazard control protocols and safety measures including containment setup and safety plan implementation.",
    justification:
      "OH&S compliance requires comprehensive hazard identification and control measures prior to remediation activities. Data capture and standards adherence for encounter-process safety protocols ensure worker and occupant safety.",
  ...
  if (bioMouldDetected) {
    items.push({
      ...
      description:
        "Specialized mould remediation protocols including containment, removal of affected materials, and antimicrobial treatment.",
      justification:
        "IICRC S520 requires specialized protocols for mould remediation. Visible mould growth requires containment, removal of affected materials, and antimicrobial treatment to prevent regrowth.",
  ...
  if (methScreen === "POSITIVE") {
    items.push({
      ...
      description:
        "Specialized decontamination protocols for methamphetamine contamination including testing, removal of affected materials, and verification.",
      justification:
        "Methamphetamine contamination requires specialized decontamination protocols. Affected porous materials must be removed and disposed of according to hazardous materials protocols.",
...
function buildMoistureData(data: ReportData): MoistureDataResult {
  ...
  if (report.moistureReadings) {
    try {
      const parsed =
        typeof report.moistureReadings === "string"
    ...
    scopeAreas.forEach((area: any) => {
      if (area.moisture || area.moistureReading) {
        moistureReadings.push({
          ...
          material: Array.isArray(area.materials)
            ? area.materials.join(", ")
            : area.materials || "Various",
          reading: `${area.moisture || area.moistureReading}%`,
          dryStandard: "12%",
          status:
            (area.moisture || area.moistureReading || 0) > 20 ? "WET" : "DRY",
        });
      }
    });
  }
...
function buildPsychrometricData(data: ReportData): PsychrometricDataResult {
  ...
  if (report.psychrometricReadings) {
    try {
      const parsed =
        typeof report.psychrometricReadings === "string"
...
function buildTimelineData(
  ...
  if (phase2Start && phase2End) {
    ...
    const duration = Math.ceil(
      (p2End.getTime() - p2Start.getTime()) / (1000 * 60 * 60 * 24),
    );
    phases.push({
  ...
  if (phase3Start && phase3End) {
    ...
    const duration = Math.ceil(
      (p3End.getTime() - p3Start.getTime()) / (1000 * 60 * 60 * 24),
    );
    phases.push({
  ...
  if (phases.length === 0) {
    ...
    let estimatedDays =
      report.estimatedDryingTime || report.estimatedDryingDuration || null;
    ...
    if (estimatedDays) {
      ...
      const p3Start = new Date(
        p2Start.getTime() + p2Duration * 24 * 60 * 60 * 1000,
      );
      phases.push({
        name: "Phase 3: Verification/Handover",
        startDate: p3Start,
      ...
      latestDate = new Date(
        p3Start.getTime() + p3Duration * 24 * 60 * 60 * 1000,
      );
    } else {
  ...
  const normalizedPhases = phases.map((phase) => {
    const daysFromStart = Math.ceil(
      (phase.startDate.getTime() - earliestDate.getTime()) /
        (1000 * 60 * 60 * 24),
    );
    const normalizedStart = Math.round((daysFromStart / totalDays) * 14);
    const normalizedDuration = Math.max(
      1,
      Math.round((phase.duration / totalDays) * 14),
    );
...
function buildExpertisePoints(
  ...
  if (
    standardsContext.toLowerCase().includes("iicrc") ||
    standardsContext.toLowerCase().includes("certified")
  ) {
  ...
  if (
    standardsContext.toLowerCase().includes("oh&s") ||
    standardsContext.toLowerCase().includes("safety")
  ) {
...
function buildRiskPoints(
  ...
  if (
    ...
    (report.structureType.toLowerCase().includes("cavity") ||
      report.structureType.toLowerCase().includes("void"))
  ) {
...
function buildCommunicationPlan(
  ...
  if (
    ...
    (report.propertyAddress.toLowerCase().includes("unit") ||
      report.propertyAddress.toLowerCase().includes("commercial"))
  ) {
    rows.push({
      ...
      method: "Email",
      content:
        "Tenant notifications, access arrangements, and completion updates",
    });
  }
...
function buildForensicSummary(
  ...
  let summary =
    "The Forensic Investigation Summary provides a comprehensive assessment of the water damage incident and outlines the professional remediation approach required. ";
  summary +=
    "This assessment includes detailed analysis of the affected areas, contamination levels, and specialized remediation protocols necessary to restore the property to a safe and habitable condition. ";
  summary +=
    "The investigation incorporates IICRC standards and best practices to ensure complete remediation and prevent future issues.";
  ...
  if (hasMould) {
    summary +=
      " Biological/mould contamination: POSITIVE - Category 3 protocols required.";
  }
...
function extractWaterCategory(source: string): string {
  ...
  if (
    lower.includes("category 3") ||
    lower.includes("cat 3") ||
  ...
  if (
    ...
    lower.includes("cat 2") ||
    lower.includes("grey water")
  ) {
    return "Category 2";
  }
  return "Category 1";
