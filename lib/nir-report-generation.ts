...
export async function generateNIRPDF(
  ...
  // Called with no jurisdiction: this module has no country or state in scope,
  // because `propertyCountry` is not on the Inspection model (RA-1120). So the
  // footer resolves to the NCC edition in force in EVERY Australian jurisdiction
  // — correct everywhere, less precise than it could be.
  //
  // Be clear about what that means: reportStandardsFooterLine() can now omit NCC
  // for a New Zealand job, but this call site cannot yet tell it that a job IS in
  // New Zealand. The NZ defect is fixed in the function and still reachable here.
  // RA-1120 is what closes it; pass ("NZ") / ("AU", state) once it lands.
  text(
    `Generated by RestoreAssist. Standards: ${reportStandardsFooterLine()}.`,
    8,
    false,
    "center",
  );
