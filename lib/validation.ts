...
export function validateInitialDataEntry(data: {
  ...
  if (!data.propertyPostcode || !data.propertyPostcode.trim()) {
    errors.push({
      ...
      message:
        "Postcode is required for state detection and regulatory compliance",
      severity: "error",
      canOverride: false,
    });
  } else {
...
export function validateTier1Responses(responses: any): ValidationResult {
  ...
  if (
    !responses.T1_Q6_materialsAffected ||
    responses.T1_Q6_materialsAffected.length === 0
  ) {
    errors.push({
  ...
  if (!responses.T1_Q7_hazards || responses.T1_Q7_hazards.length === 0) {
    errors.push({
      field: "T1_Q7_hazards",
      message:
        'Hazard assessment is required (select "None identified" if no hazards)',
      severity: "error",
      canOverride: false,
    });
  } else {
  ...
  if (
    responses.T1_Q4_occupancyStatus?.includes("Vacant") &&
    responses.T1_Q4_petsPresent
  ) {
    warnings.push({
      field: "T1_Q4",
  ...
  if (
    responses.T1_Q2_constructionYear?.includes("Pre-1970") &&
    responses.T1_Q7_hazards?.includes("None identified")
  ) {
    warnings.push({
      field: "T1_Q7",
      message:
        'Pre-1970 construction typically has asbestos risk. Consider selecting "Suspected asbestos" or confirming "None identified".',
      severity: "warning",
      canOverride: true,
    });
  }
  ...
  if (
    ...
    warnings.push({
      field: "T3_Q3",
      message:
        "Category 3 water typically requires biohazard treatment. Verify selection.",
      severity: "warning",
      canOverride: true,
    });
  }
...
export function validateCostEstimation(costData: any): ValidationResult {
  ...
  if (subtotal > 50000) {
    warnings.push({
      field: "subtotal",
      message:
        "This is a high-value claim ($" +
        subtotal.toLocaleString() +
        "). Verify all quantities and rates.",
      severity: "warning",
      canOverride: true,
    });
  }
  ...
  if (subtotal < 2000) {
    warnings.push({
      field: "subtotal",
      message:
        "This estimate seems low for the described damage. Verify scope is complete.",
      severity: "warning",
      canOverride: true,
    });
  }
...
export function calculateCompletenessScore(report: any): number {
  ...
  if (report.tier1Responses) {
    ...
    const answered = tier1Fields.filter(
      (field) =>
        tier1[field] &&
        (typeof tier1[field] !== "object" ||
  ...
  if (report.tier2Responses) {
    ...
    const answered = tier2Fields.filter(
      (field) =>
        tier2[field] &&
        (typeof tier2[field] !== "object" ||
          (Array.isArray(tier2[field]) && tier2[field].length > 0) ||
          tier2[field].trim?.()),
    );
  ...
  if (report.tier3Responses) {
    ...
    const answered = tier3Fields.filter(
      (field) => tier3[field] && tier3[field].trim?.(),
    );
...
export function checkCompletenessBeforeGeneration(report: any): {
  ...
  if (!report.technicianFieldReport)
    missingItems.push("Technician field report");
  ...
  if (report.reportDepthLevel === "Enhanced" && !report.tier2Responses) {
    warnings.push(
      "Tier 2 questions not completed - report will be less detailed",
    );
  }

  if (report.reportDepthLevel === "Optimised" && !report.tier3Responses) {
    warnings.push(
      "Tier 3 questions not completed - cost estimation may be less precise",
    );
  }
  ...
  if (report.tier1Responses) {
    ...
    if (hazards.length > 0 && !hazards.includes("None identified")) {
      warnings.push(
        "Hazards identified - ensure STOP WORK flags are included in report",
      );
    }
  }
  ...
  if (report.tier3Responses) {
    ...
    if (tier3.T3_Q5_class4DryingAssessment?.includes("Class 4")) {
      warnings.push(
        "Class 4 drying confirmed - ensure specialist quote flag is included",
      );
    }
  }
