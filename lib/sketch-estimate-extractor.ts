...
const MATERIALS_PER_M2: { description: string; rate: number; unit: string }[] = [
  ...
  { description: "Plasterboard replacement (20% wastage)", rate: 1.2, unit: "m²" },
]
...
export function recommendEquipment(affectedAreaM2: number): {
  ...
  return {
    dehumidifiers: Math.max(1, Math.ceil(affectedAreaM2 / IICRC_RATIOS.dehumidifier)),
    airMovers:     Math.max(1, Math.ceil(affectedAreaM2 / IICRC_RATIOS.airMover)),
    airScrubbers:  Math.max(1, Math.ceil(affectedAreaM2 / IICRC_RATIOS.airScrubber)),
  }
}
 ...
 */
export function extractSketchEstimate(floors: SketchFloorData[]): SketchEstimate {
  const allLineItems: EstimateLineItem[] = []
  let totalRoomAreaM2 = 0
  let totalDamageAreaM2 = 0

  for (const floor of floors) {
    const { rooms, damage } = extractRoomsFromFabricJson(floor.sketchData, floor.floorLabel)
    ...
    const equip = extractEquipmentLineItems(floor.equipmentPoints, floor.floorLabel)
    ...
    if (floorDamageArea > 0 && equip.length === 0) {
      ...
      const equipRec: EstimateLineItem[] = [
        { id: `recequip-${floor.floorLabel}-dh`, category: "equipment", description: `Dehumidifier (IICRC recommended) — ${floor.floorLabel}`, quantity: rec.dehumidifiers, unit: "units", floor: floor.floorLabel, notes: `IICRC S500: 1 per ${IICRC_RATIOS.dehumidifier}m²` },
        { id: `recequip-${floor.floorLabel}-am`, category: "equipment", description: `Air Mover (IICRC recommended) — ${floor.floorLabel}`, quantity: rec.airMovers, unit: "units", floor: floor.floorLabel, notes: `IICRC S500: 1 per ${IICRC_RATIOS.airMover}m²` },
        { id: `recequip-${floor.floorLabel}-as`, category: "equipment", description: `Air Scrubber (IICRC recommended) — ${floor.floorLabel}`, quantity: rec.airScrubbers, unit: "units", floor: floor.floorLabel, notes: `IICRC S500: 1 per ${IICRC_RATIOS.airScrubber}m²` },
      ]
      allLineItems.push(...equipRec)
    }

    // Derived materials from damage area
    allLineItems.push(...deriveMaterialItems(floorDamageArea, floor.floorLabel))
  }
