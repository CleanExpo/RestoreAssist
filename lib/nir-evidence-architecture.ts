...
export const EVIDENCE_REGISTER: EvidenceClaim[] = [
  {
    ...
    value: '250,000',
    status: 'SOURCED',
    source: 'Insurance Council of Australia Annual Claims Report. Range: 230,000–280,000. Update annually.',
    lastUpdated: '2026-03',
  },
  {
    id: 'CLAIM-002',
    ...
    status: 'HYPOTHESIS',
    hypothesis: {
      measurementMethod: 'Track re-inspection costs, adjuster time (timed sessions), dispute resolution time across 50 pilot claims vs. 50 control claims using existing formats.',
      sampleSize: '50 NIR pilot claims + 50 control claims',
      measurementPhase: 'Phase 2 pilot',
      successCriteria: 'Measured cost difference is statistically significant (p<0.05)',
    },
    lastUpdated: '2026-03',
    notes: 'DO NOT cite as fact in customer materials until VALIDATED. Use range and label as pilot hypothesis.',
  },
  {
    id: 'CLAIM-003',
    ...
    status: 'HYPOTHESIS',
    hypothesis: {
      measurementMethod: 'Count re-inspection work orders per claim across pilot cohort. Compare NIR vs. control group.',
      sampleSize: '50 NIR + 50 control claims',
      measurementPhase: 'Phase 2 pilot',
      successCriteria: 'NIR re-inspection rate <10%, control rate measured for baseline',
    },
    lastUpdated: '2026-03',
    notes: 'DO NOT cite as established industry fact. State as practitioner-reported estimate pending validation.',
  },
  {
    id: 'CLAIM-004',
    ...
    status: 'HYPOTHESIS',
    hypothesis: {
      measurementMethod: 'Timed adjuster review sessions: NIR vs. 3 different company-specific formats. n=6 adjusters, 5 reports each format.',
      sampleSize: '6 adjusters × 20 reports = 120 timed sessions',
      measurementPhase: 'Phase 2 pilot',
      successCriteria: 'NIR average review time <45 minutes; >30% reduction vs. control formats',
    },
    lastUpdated: '2026-03',
  },
  {
    ...
    status: 'HYPOTHESIS',
    hypothesis: {
      measurementMethod: '5-point post-use survey after first 3 inspections. Minimum 20 technician respondents.',
  ...
  },
  {
    ...
    value: '$625M–$1.25B',
    status: 'DERIVED',
    source: 'Derived: CLAIM-001 (claims volume) × CLAIM-002 (per-claim saving). Will be updated when CLAIM-002 is validated.',
    lastUpdated: '2026-03',
    notes: 'Publish as scenario model (low/mid/high), not point estimate. Always disclose derivation methodology.',
  },
  {
    id: 'CLAIM-007',
    ...
    status: 'HYPOTHESIS',
    hypothesis: {
      measurementMethod: 'Track claim-open to scope-approval timeline. Pilot claims vs. same company historical average.',
      sampleSize: '50 pilot claims per company (minimum 3 companies)',
      measurementPhase: 'Phase 2 pilot',
      successCriteria: 'Median scope approval time <5 business days for NIR vs. company historical baseline',
    },
    lastUpdated: '2026-03',
  },
]
...
export function getPublishableClaims(): EvidenceClaim[] {
  return EVIDENCE_REGISTER.filter(c => c.status === 'SOURCED' || c.status === 'VALIDATED')
}
...
export function assertClaimsPublishable(claimIds: string[]): void {
  const unpublishable = claimIds.filter(id => {
    ...
    return !claim || (claim.status !== 'SOURCED' && claim.status !== 'VALIDATED')
  })
