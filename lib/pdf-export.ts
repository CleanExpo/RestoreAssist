...
function buildReportHtml(data: InspectionReportData): string {
  ...
  const table = (head: string[], rows: string[][]): string => {
    const thCells = head
      .map(
        (h) =>
          `<th style="padding:6px 10px;text-align:left;font-size:10px;color:#374151;font-weight:700;border-bottom:1px solid #e5e7eb;background:#f9fafb;">${h}</th>`,
      )
      .join("");
    const bodyRows = rows
      .map((row) => {
        const cells = row
          .map(
            (c) =>
              `<td style="padding:6px 10px;font-size:10px;color:#111827;border-bottom:1px solid #f3f4f6;">${c}</td>`,
          )
          .join("");
        return `<tr>${cells}</tr>`;
      })
...
<div style="background:#0e7490;color:#fff;padding:24px 32px 20px;display:flex;justify-content:space-between;align-items:flex-start;">
  ...
  const summaryItems: [string, string][] = [
    ["Status", statusBadge],
    ...(inspection.damageCategory
      ? [
          ["IICRC Category", `Category ${inspection.damageCategory}`] as [
            string,
            string,
          ],
        ]
    ...
    ...(inspection.damageClass
      ? [["IICRC Class", `Class ${inspection.damageClass}`] as [string, string]]
      : []),
    ...(inspection.affectedAreaM2 != null
      ? [
          ["Affected Area", `${inspection.affectedAreaM2} m²`] as [
            string,
            string,
          ],
        ]
    ...
    ...(inspection.completedAt
      ? [["Completed", fmt(inspection.completedAt)] as [string, string]]
      : []),
  ];

  const summaryGrid = summaryItems
    .map(
      ([label, value]) =>
...
<div style="padding:12px 16px 4px;">
  ...
  if (moistureReadings && moistureReadings.length > 0) {
    ...
    const moistureStatusColour = (s?: string): string => {
      ...
      if (lower.includes("warn") || lower.includes("elevated"))
        return "#d97706";
      return "#dc2626";
    };
...
<div style="padding:12px 16px 4px;">
  ...
  const workItems = (scopeItems ?? []).filter(
    (i) =>
      ...
      i.itemType !== "dehumidifier" &&
      i.itemType !== "air_mover",
  );
  let scopeHtml = "";
  if (workItems.length > 0) {
    const rows = workItems.map((item) => [
      item.description +
        (item.iicrcReference
          ? ` <span style="color:#0e7490;font-size:9px;">[${item.iicrcReference}]</span>`
          : ""),
      item.quantity != null
...
<div style="padding:12px 16px 4px;">
  ...
  // ── 7. Sign-off ─────────────────────────────────────────────────────────────
  const signOffDate = new Date().toLocaleDateString("en-AU", {
    day: "2-digit",
    month: "long",
    year: "numeric",
  });
