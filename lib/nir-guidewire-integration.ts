...
export interface NirPhotoManifest {
  ...
  photos: Array<{
    ...
    category: 'overview' | 'damage' | 'moisture-reading' | 'equipment' | 'content' | 'post-restoration'
    standardRef: string
  }>
}
...
export const GUIDEWIRE_FIELD_MAP = {
  ...
  claim: {
    ...
    lossType:              { nirSource: 'lossClassification.lossType',         gwField: 'claim.lossType',                    notes: 'Map NIR lossType to Guidewire LossType typecode (e.g. "PR_WaterDamage")' },
    claimNumber:           { nirSource: 'reportId',                            gwField: 'claim.externalClaimNumber',         notes: 'NIR reportId stored as external reference for audit trail' },
    lossDate:              { nirSource: 'inspectionDate',                      gwField: 'claim.lossDate',                    notes: 'Date loss was inspected — not the date damage occurred; insurer may adjust' },
    reportedDate:          { nirSource: 'submittedAt',                         gwField: 'claim.reportedDate',                notes: 'ISO 8601 timestamp NIR was submitted' },
    description:           { nirSource: 'lossClassification.primaryClassification', gwField: 'claim.description',           notes: 'Primary IICRC classification drives claim description' },
  },

  // ─── Loss location ─────────────────────────────────────────────────────────

  lossLocation: {
    addressLine1:          { nirSource: 'property.address',                    gwField: 'claim.lossLocation.addressLine1',   notes: '' },
    city:                  { nirSource: 'property.suburb',                     gwField: 'claim.lossLocation.city',           notes: '' },
    state:                 { nirSource: 'property.state',                      gwField: 'claim.lossLocation.state',          notes: 'Map AU state abbreviation to Guidewire state typecode' },
    postalCode:            { nirSource: 'property.postcode',                   gwField: 'claim.lossLocation.postalCode',     notes: '' },
  },
  ...
  exposure: {
     ...
     */
    primaryCoverage:       { nirSource: 'lossClassification.lossType',        gwField: 'exposure.primaryCoverage',          notes: 'Map to Guidewire Coverage typecode on the policy' },
    exposureState:         { nirSource: null,                                  gwField: 'exposure.state',                    notes: 'Set to "open" on creation; insurer workflow advances state' },
    closingReserve:        { nirSource: 'scopeLineItems[].unitRate (sum)',     gwField: 'exposure.closingReserve',           notes: 'Sum of all NIR scope line item rates — initial reserve estimate' },
  },
  ...
  serviceRequest: {
     ...
     */
    vendorName:            { nirSource: 'technician.name',                     gwField: 'serviceRequest.vendorContact.displayName', notes: 'Technician name as vendor contact' },
    vendorLicence:         { nirSource: 'technician.licenceNumber',            gwField: 'serviceRequest.vendorContact.licenceNumber', notes: 'Maps to Guidewire vendor profile if pre-registered' },
    specialInstructions:   { nirSource: 'standardsCitations (formatted)',      gwField: 'serviceRequest.specialInstructions', notes: 'Formatted IICRC citation list appended as special instructions' },
  },
  ...
  documents: {
     ...
     */
    nirReport:             { nirSource: 'reportId (PDF link)',                 gwField: 'document.name + document.mimeType', notes: 'Full NIR PDF uploaded as "Restoration Assessment Report"' },
    photoEvidence:         { nirSource: 'photoManifest.photos[]',              gwField: 'document (per photo)',              notes: 'Each photo uploaded individually; exif metadata preserved' },
  },

  // ─── Notes ─────────────────────────────────────────────────────────────────

  notes: {
    classificationNote:    { nirSource: 'lossClassification (formatted)',      gwField: 'note.text',                         notes: 'IICRC classification with clause refs appended as a claim note' },
    jurisdictionalNote:    { nirSource: 'property.locationFlags',              gwField: 'note.text',                         notes: 'Location risk flags (flood zone, BAL, cyclone) appended as advisory note' },
  },
} as const
...
export function transformNirToGuidewireClaim(
  ...
  if (locationFlags.isFloodZone)    locationNotes.push('ADVISORY: Property in identified flood zone')
  if (locationFlags.isBushfireProne) locationNotes.push('ADVISORY: Property in bushfire-prone area')
  if (locationFlags.isCycloneZone)  locationNotes.push('ADVISORY: Property in cyclone zone')
  if (locationFlags.isHeritageListed) locationNotes.push('ADVISORY: Heritage-dense area — confirm heritage listing before structural works')
  if (locationFlags.windRegion)     locationNotes.push(`Wind Region: ${locationFlags.windRegion}`)
  if (locationFlags.approximateBALZone) locationNotes.push(`Approximate BAL Zone: ${locationFlags.approximateBALZone}`)
...
export const MAJESCO_FIELD_MAP_STUB = {
  ...
  _nirContactForMapping: 'RestoreAssist technical team via docs.restoreassist.com.au/integrations',
  knownMappings: {
    claimReference: { nirSource: 'reportId', majescoField: 'claim.externalReference', notes: 'Confirmed equivalent to Guidewire externalClaimNumber' },
    lossDate:       { nirSource: 'inspectionDate', majescoField: 'claim.incidentDate', notes: 'Confirmed — Majesco uses incidentDate not lossDate' },
  },
} as const
