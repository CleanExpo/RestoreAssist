...
export async function pollHeygenRenders(): Promise<CronJobResult> {
  ...
  if (!apiKey) {
    return {
      itemsProcessed: 0,
      metadata: { error: "HEYGEN_API_KEY not configured" },
    };
  }
  ...
  if (renderingJobs.length === 0) {
    return {
      itemsProcessed: 0,
      metadata: { message: "No rendering jobs to poll" },
    };
  }
  ...
  for (const job of renderingJobs) {
    try {
      ...
      if (age > MAX_RENDER_AGE_MS) {
        ...
        failed++;
        console.warn(
          `[poll-heygen] Job ${job.id} timed out (${Math.round(age / 60000)}m)`,
        );
        continue;
      }
      ...
      if (!response.ok) {
        console.error(
          `[poll-heygen] HeyGen API ${response.status} for job ${job.id}`,
        );
      ...
      if (json.data?.status === "completed" && json.data.video_url) {
        ...
        completed++;
        console.log(
          `[poll-heygen] Job ${job.id} completed: ${json.data.video_url}`,
        );
