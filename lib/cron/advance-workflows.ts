...
export async function advanceWorkflows(): Promise<CronJobResult> {
  ...
  for (const wf of scheduledWorkflows) {
    try {
      ...
      console.error(
        `[CronAdvance] Failed to start scheduled workflow ${wf.id}:`,
        err,
      );
    }
  }
  ...
  for (const wf of staleWorkflows) {
    try {
      ...
      if (
        ...
        progress.nextExecutable.length === 0
      ) {
        ...
        await prisma.agentWorkflow.update({
          ...
          data: {
            ...
            errorMessage:
              "Workflow stalled: no executable tasks and no progress for 15+ minutes",
          },
        });
      ...
      console.error(
        `[CronAdvance] Failed to handle stale workflow ${wf.id}:`,
        err,
      );
    }
  }
