...
export async function distributeContent(): Promise<CronJobResult> {
  ...
  if (readyJobs.length === 0) {
    return { itemsProcessed: 0, metadata: { message: 'No videos ready for distribution' } }
  }
  ...
  for (const job of readyJobs) {
    try {
      ...
      // Upload to YouTube
      const result = await uploadToYouTube(systemUserId, job.videoUrl, metadata)
      ...
      uploaded++
      console.log(`[distribute-content] Uploaded job ${job.id} → ${result.youtubeUrl}`)
    } catch (err) {
      failed++
      const message = err instanceof Error ? err.message : String(err)
      console.error(`[distribute-content] Failed to upload job ${job.id}:`, message)
