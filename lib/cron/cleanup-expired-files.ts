          ...
          if (file.context && file.context.custom) {
            ...
            if (expiresAt) {
              ...
              if (now > expiryDate) {
                try {
                  console.log(
                    `[Cron] Deleting expired file: ${file.publicId} (expired: ${expiresAt})`,
                  );
                  await deleteFile(file.publicId, "raw");
                  stats.deleted++;
                } catch (deleteError) {
                  console.error(
                    `[Cron] Error deleting file ${file.publicId}:`,
                    deleteError,
                  );
                  stats.errors++;
                }
...
export async function cleanupOldFiles(
  ...
  try {
      ...
      try {
        ...
        for (const file of files) {
          ...
          if (createdDate < cutoffDate) {
            try {
              console.log(
                `[Cron] Deleting old file: ${file.publicId} (created: ${file.createdAt})`,
              );
              await deleteFile(file.publicId, "raw");
              stats.deleted++;
            } catch (deleteError) {
    ...
    console.log(
      `[Cron] Old files cleanup completed (${daysOld}+ days):`,
      stats,
    );
