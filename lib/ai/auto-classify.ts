...
export function ruleBasedClassify(input: {
  ...
  const text = `${input.description ?? ''} ${input.notes ?? ''} ${input.location ?? ''}`.toLowerCase()
  ...
  } else if (/storm|hail|wind|roof|cyclone|tempest|tornado|flood rain/.test(text)) {
    claimType = 'storm'
  ...
  if (claimType === 'water_damage') {
    ...
    if (/sewage|black water|toilet overflow|sewer|category.?3|cat.?3|biohazard|faecal|human waste/.test(text)) {
      damageCategory = 3
      categoryReasoning = 'Category 3 (black water/sewage) — biohazard protocols required'
    } else if (/grey water|gray water|washing machine|dishwasher|overflow|category.?2|cat.?2|contaminate/.test(text)) {
      damageCategory = 2
      categoryReasoning = 'Category 2 (grey water) — antimicrobial treatment required'
    } else {
      damageCategory = 1
    ...
    if (avgMoisture > 40 || /class.?4|soaked|saturated|concrete|dense material/.test(text)) {
      damageClass = 4
    } else if (avgMoisture > 25 || /class.?3|ceiling|insulation|entire room|extensive/.test(text)) {
      damageClass = 3
    } else if (avgMoisture > 15 || /class.?2|carpet|significant|walls and floor/.test(text)) {
      damageClass = 2
    } else {
      damageClass = 1
    }
  ...
  const confidence: 'high' | 'medium' | 'low' =
    text.length > 50 ? 'medium' :
    text.length > 20 ? 'low' : 'low'
...
export async function vectorClassify(input: {
  ...
  try {
    ...
    const categories = jobs.map((j: any) => j.waterCategory).filter(Boolean) as number[]
    const classes = jobs.map((j: any) => j.waterClass).filter(Boolean) as number[]

    const mode = (arr: number[]) => arr.length === 0 ? undefined :
      arr.sort((a, b) =>
