...
describe("llmClassify — happy path", () => {
  ...
  it("returns a parsed ClassificationResult from a well-formed LLM response", async () => {
    const llmPayload = {
      ...
      confidence: 0.92,
      reasoning:
        "Grey water ingress from dishwasher overflow per S500:2021 §10.4.1 and §10.6.3.",
      clauseRefs: ["S500:2021 §10.4.1", "S500:2021 §10.6.3"],
  ...
  it("sets humanReviewRequired=true when confidence < 0.7", async () => {
    const llmPayload = {
      ...
      confidence: 0.55,
      reasoning:
        "Ambiguous storm and water damage; roof damaged by wind per S500:2021 §10.4.1.",
