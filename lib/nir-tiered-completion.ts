...
export const CRITICAL_FIELDS: TieredField[] = [
  {
    ...
    rationale: 'Without a verifiable address the report cannot be linked to a policy or claim.',
  },
  {
    fieldName: 'propertyPostcode',
    label: 'Property postcode',
    rationale: 'Postcode drives jurisdictional matrix and building code requirements. Cannot be derived from address alone.',
  },
  {
    fieldName: 'inspectionDate',
    label: 'Inspection date',
    rationale: 'Required for IICRC time-escalation logic (S500 §7.1 — 48hr Cat 1→2 degradation). Absence invalidates classification.',
  },
  {
    fieldName: 'affectedAreas',
    label: 'At least one affected area',
    clauseRef: 'IICRC S500 §8.1–8.4',
    rationale: 'Affected area is required to determine water class. No area = no scope = report is incomplete.',
  },
  {
    fieldName: 'waterSource',
    label: 'Water source / loss type',
    clauseRef: 'IICRC S500 §7.1–7.3',
    rationale: 'Water source determines Category (1/2/3). Absent = no classification possible = no scope.',
  },
  {
    fieldName: 'photos',
    label: 'At least one photo',
    clauseRef: 'IICRC S500 §5.3',
    rationale: 'S500 §5.3 requires photo documentation as a minimum. One photo is the absolute floor for a submittable report.',
  },
]
...
export const SUPPLEMENTARY_FIELDS: TieredField[] = [
  {
    ...
    rationale: 'Required for drying target calculation (S500 §12.4) but can be captured at reinspection if equipment unavailable.',
  },
  {
    fieldName: 'moistureReadings',
    label: 'Moisture readings',
    clauseRef: 'IICRC S500 §12.3',
    rationale: 'Critical for classification accuracy but a partial set is acceptable in emergency conditions. Flagged for follow-up.',
  },
  {
    fieldName: 'equipmentSerialNumbers',
    label: 'Equipment serial numbers',
    clauseRef: 'IICRC S500 §14',
    rationale: 'S500 §14 requires equipment documentation but serial numbers can be recorded at placement.',
  },
  {
    fieldName: 'scopeItems',
    label: 'Full scope line items',
    rationale: 'Auto-generated from classification. If classification is present, scope can be generated post-submission.',
  },
  {
    fieldName: 'technicianSignature',
    label: 'Technician signature',
    rationale: 'Required for final report but may be captured digitally after field work is complete.',
  },
]
...
export function validateTieredCompletion(
  ...
  if (!inspection.propertyAddress?.trim()) {
    missingCritical.push(CRITICAL_FIELDS.find(f => f.fieldName === 'propertyAddress')!)
  }

  if (!inspection.propertyPostcode?.trim()) {
    missingCritical.push(CRITICAL_FIELDS.find(f => f.fieldName === 'propertyPostcode')!)
  }

  if (!inspection.inspectionDate) {
    missingCritical.push(CRITICAL_FIELDS.find(f => f.fieldName === 'inspectionDate')!)
  }

  if (!inspection.affectedAreas || inspection.affectedAreas.length === 0) {
    missingCritical.push(CRITICAL_FIELDS.find(f => f.fieldName === 'affectedAreas')!)
  }

  // Water source check — derived from first affected area
  const hasWaterSource = inspection.affectedAreasWithSource?.some(a => !!a.waterSource)
  if (!hasWaterSource) {
    missingCritical.push(CRITICAL_FIELDS.find(f => f.fieldName === 'waterSource')!)
  }

  if (!inspection.photos || inspection.photos.length === 0) {
    missingCritical.push(CRITICAL_FIELDS.find(f => f.fieldName === 'photos')!)
  }

  // ─── Supplementary field checks ────────────────────────────────────────────

  if (!inspection.environmentalData) {
    missingSupplementary.push(SUPPLEMENTARY_FIELDS.find(f => f.fieldName === 'environmentalData')!)
  }

  if (!inspection.moistureReadings || inspection.moistureReadings.length === 0) {
    missingSupplementary.push(SUPPLEMENTARY_FIELDS.find(f => f.fieldName === 'moistureReadings')!)
  }

  if (!inspection.scopeItems || inspection.scopeItems.length === 0) {
    missingSupplementary.push(SUPPLEMENTARY_FIELDS.find(f => f.fieldName === 'scopeItems')!)
  }
  ...
  if (photoCount === 1) {
    warnings.push('Only 1 photo recorded. S500 §5.3 requires overview, affected areas, moisture meter placement, and equipment placement photos. Additional photos required before final report.')
  }
  ...
  if (moistureCount > 0 && areaCount > 0 && moistureCount < areaCount) {
    warnings.push(`${moistureCount} moisture reading(s) recorded across ${areaCount} affected area(s). At least one reading per area is recommended per S500 §12.3.`)
  }

  if (!inspection.environmentalData && moistureCount > 0) {
    warnings.push('Moisture readings present but no environmental data (temperature/humidity). Drying target calculation (S500 §12.4) requires ambient conditions — record at reinspection.')
  }
  ...
  if (!canSubmit) {
    summaryParts.push(`${missingCritical.length} critical field(s) missing — submission blocked: ${missingCritical.map(f => f.label).join(', ')}.`)
  }
  if (missingSupplementary.length > 0) {
    summaryParts.push(`${missingSupplementary.length} supplementary field(s) absent — flagged in report for follow-up: ${missingSupplementary.map(f => f.label).join(', ')}.`)
  }
