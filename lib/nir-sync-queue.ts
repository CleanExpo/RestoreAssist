...
export type SyncStatus = 'SYNCED' | 'PENDING_SYNC' | 'SYNC_CONFLICT' | 'OFFLINE'
 ...
 */
export async function getPendingEntries(inspectionId?: string): Promise<SyncQueueEntry[]> {
  const db = await openDatabase()
 ...
 */
export async function resolveConflictWithLocal(conflictId: string): Promise<void> {
  const db = await openDatabase()

  const conflict = await new Promise<SyncConflict | undefined>((resolve, reject) => {
...
async function incrementRetry(db: IDBDatabase, entry: SyncQueueEntry): Promise<void> {
  const backoffMs = RETRY_BACKOFF_MS[Math.min(entry.retryCount, RETRY_BACKOFF_MS.length - 1)]
