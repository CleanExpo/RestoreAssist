...
export function logCacheMetrics(
  ...
  if (metrics.cacheCreationInputTokens) {
    ...
    console.log(
      `[Cache] ${agentName}: Created cache (${metrics.cacheCreationInputTokens} tokens)`,
    );
  }
  ...
  if (metrics.cacheReadInputTokens) {
    ...
    console.log(
      `[Cache] ${agentName}: Cache HIT (${metrics.cacheReadInputTokens} tokens, ${logData.cacheHitRate} hit rate)`,
    );
  }
...
export function prepareCachedMessageParams(
  ...
  const enableCache =
    params.enableCache !== false && params.cacheStrategy !== "none";
