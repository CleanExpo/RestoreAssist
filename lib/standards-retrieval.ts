...
async function analyzeFolderStructureWithAI(
  ...
  try {
    ...
    const fileList = folderItems.files
      .map((f) => ({
        name: f.name,
        mimeType: f.mimeType,
        id: f.id,
...
5. IICRC standard references and citations`;
    ...
    console.error(
      `[Standards Retrieval] Error in AI folder analysis:`,
      error.message,
    );
    return {
      relevantFiles: folderItems.files.slice(0, 10),
...
export async function retrieveRelevantStandards(
  ...
  try {
    ...
    try {
      ...
      console.error(
        `[Standards Retrieval] Error initializing Anthropic client:`,
        error,
      );
      return {
        documents: [],
    ...
    let folderStructure: { files: DriveFile[]; folders: any[] } = {
      files: [],
      folders: [],
    };
    ...
    try {
      ...
      const aiAnalysis = await analyzeFolderStructureWithAI(
        anthropic,
        folderStructure,
        query,
      );
