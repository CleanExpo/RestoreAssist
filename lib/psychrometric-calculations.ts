 ...
 */
export function calculateDryingPotential(
  ...
  const saturationVaporPressure =
    0.611 * Math.exp((17.27 * temperature) / (temperature + 237.3));
  ...
  const vaporPressureDifferential =
    saturationVaporPressure - actualVaporPressure;

  // Target vapor pressure differential (optimal conditions)
  const targetSaturationVP =
    0.611 * Math.exp((17.27 * target.temp) / (target.temp + 237.3));
  ...
  const rhProximity =
    humidity >= rhRange.min && humidity <= rhRange.max
  ...
  return {
    ...
    status,
    recommendation,
    vaporPressureDifferential:
      Math.round(vaporPressureDifferential * 100) / 100,
  };
}
...
export function calculateWaterRemovalTarget(
  ...
  const affectedAreaTarget =
    Math.max(0, affectedArea) * (litresPerM2PerDayByClass[waterClass] || 8);
