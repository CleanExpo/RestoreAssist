...
const COUNCIL_POSTCODE_RANGES: {
  [key: string]: { name: string; ranges: number[][] };
} = {
  Brisbane: {
...
export function getGeographicIntelligence(
  postcode: string,
): GeographicIntelligence | null {
...
function getWeatherPatterns(
  postcode: string,
  state: string,
): GeographicIntelligence["weatherPatterns"] {
  const numericPostcode = parseInt(postcode.replace(/\D/g, ""));

  // Cyclone-prone areas (Far North QLD, Northern WA, NT)
  const cycloneProne =
    (state === "QLD" && numericPostcode >= 4800 && numericPostcode <= 4899) ||
    (state === "WA" && numericPostcode >= 6700 && numericPostcode <= 6799) ||
    state === "NT";

  // Flood-prone areas (simplified)
  const floodProne =
    (state === "QLD" && numericPostcode >= 4000 && numericPostcode <= 4999) ||
...
function getBuildingRequirements(
  postcode: string,
  state: string,
): GeographicIntelligence["buildingRequirements"] {
  const numericPostcode = parseInt(postcode.replace(/\D/g, ""));
  ...
  if (
    ...
    return {
      floodLevel:
        "Check local flood mapping for property-specific requirements",
      specialConsiderations: [
