...
export function generateVerificationChecklist(
  ...
  items.push({
    ...
    verified: !!(
      inspection.propertyAddress?.trim() && inspection.propertyPostcode?.trim()
    ),
  ...
  const totalCost = costEstimates.reduce(
  ...
  const citedCount = standardsCitations.filter(
    (c) => c.status === "CITED",
  ).length;
  ...
  const hasEquipmentScope = scopeItems.some(
    (item) =>
