 ...
 */
export function calculateEquipment(input: EquipmentCalculatorInput): EquipmentCalculatorResult {
  ...
  items.push(buildItem("air_mover", airMoverQty, airMoverRatio, areaM2, "IICRC S500:2021 §9.3.2"))

  // 2. LGR dehumidifiers — always required
  const dehuRatio = DEHU_RATIO[damageClass]
  const dehuQty = calcQty(areaM2, dehuRatio)
  items.push(buildItem("lgr_dehumidifier", dehuQty, dehuRatio, areaM2, "IICRC S500:2021 §9.4.1"))

  // 3. Air scrubbers — Cat 2/3 mandatory; Cat 1 at Class 3/4
  const requiresScrubber =
    damageCategory === "CAT_2" ||
    damageCategory === "CAT_3" ||
    (damageCategory === "CAT_1" && (damageClass === "CLASS_3" || damageClass === "CLASS_4"))

  if (requiresScrubber) {
    const scrubberRatio = AIR_SCRUBBER_RATIO[damageClass]
    const scrubberQty = Math.max(1, calcQty(areaM2, scrubberRatio))
    items.push(buildItem("air_scrubber", scrubberQty, scrubberRatio, areaM2, "IICRC S500:2021 §9.5.1"))
  }

  // 4. Negative air machines — Cat 3 only
  if (damageCategory === "CAT_3") {
    const negAirQty = Math.max(1, calcQty(areaM2, NEGATIVE_AIR_RATIO_M2_PER_UNIT))
    items.push(
      buildItem("negative_air_machine", negAirQty, NEGATIVE_AIR_RATIO_M2_PER_UNIT, areaM2, "IICRC S500:2021 §9.5.3")
    )
  }
  ...
  const summary =
    `IICRC S500 Category ${catNum} / Class ${classNum}: ` + summaryParts.join(" + ")

  const iicrcClassification = `Category ${catNum} (${
    catNum === "1" ? "Clean Water" : catNum === "2" ? "Grey Water" : "Black Water"
  }) · Class ${classNum} (${
    classNum === "1"
      ? "Limited"
      : classNum === "2"
