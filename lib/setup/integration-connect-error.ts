...
export const ADDONS_URL = "/dashboard/addons";
...
export function integrationConnectFailure(
  ...
  if (status === 402 && code === "ADDON_REQUIRED") {
    ...
    return {
      ...
      action: { label: "View add-ons", href: ADDONS_URL },
    };
  }
  ...
  // Do not infer an upgrade action from the status alone. Unknown 402 codes
  // and 403 bodies without an explicit `upgradeRequired: true` may represent
  // a different policy denial, so keep the response generic and action-free.
