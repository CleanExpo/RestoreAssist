...
export function validateCitationAgainstStandards(
  ...
  return standardsContext.documents.some((doc) => {
    ...
    if (
      documentCode.includes("iicrc") ||
      documentCode.includes("s500") ||
      documentCode.includes("s520")
    ) {
      if (
        docName.includes("s500") ||
        docName.includes("s520") ||
        standardType.includes("s500") ||
        standardType.includes("s520")
      ) {
        return true;
      }
    }
    ...
    if (documentCode.includes("as/nzs") || documentCode.includes("asnzs")) {
      if (
        docName.includes("as/nzs") ||
        docName.includes("3000") ||
        docName.includes("3500")
      ) {
        return true;
      }
    }
    ...
    if (documentCode.includes("ncc") || documentCode.includes("qdc")) {
      if (
        docName.includes("ncc") ||
        docName.includes("construction") ||
        docName.includes("building")
      ) {
        return true;
      }
    }
...
export function getRecommendedCitations(
  ...
  return filtered.sort((a, b) => b.confidence - a.confidence).slice(0, 10); // Top 10 citations
}
...
export function formatCitationsForPrompt(
  ...
  prompt +=
    "═══════════════════════════════════════════════════════════════\n\n";
  prompt +=
    "The following citations have been extracted from the retrieved standards documents.\n";
  prompt +=
    "Use these citations when referencing standards in your report:\n\n";
