...
function getAscoraJobTypeName(damageType: NIRJobPayload['damageType']): string {
  switch (damageType) {
    case 'WATER': return process.env.ASCORA_JOB_TYPE_WATER || 'Water Damage'
    case 'FIRE':  return process.env.ASCORA_JOB_TYPE_FIRE  || 'Fire Damage'
    case 'MOULD': return process.env.ASCORA_JOB_TYPE_MOULD || 'Mould Remediation'
    default:      return process.env.ASCORA_JOB_TYPE_WATER || 'Water Damage'
  }
}
...
function parseAddress(address: string): {
  ...
  if (postcodeMatch) {
    ...
    const beforePostcode = address.slice(0, -4).trim().replace(/,?\s*$/, '')

    // Split remaining by last comma to get suburb
    const lastComma = beforePostcode.lastIndexOf(',')
    if (lastComma > 0) {
      result.suburb = beforePostcode.slice(lastComma + 1).trim()
...
export async function syncNIRJobToAscora(
  ...
  const ascoraPayload = {
    ...
    customFields: [
      ...
      ...(job.waterCategory ? [{ fieldName: 'Water Category', fieldValue: `Category ${job.waterCategory}` }] : []),
      ...(job.waterClass ? [{ fieldName: 'Water Class', fieldValue: `Class ${job.waterClass}` }] : []),
      ...(job.technician ? [{ fieldName: 'Technician', fieldValue: job.technician }] : []),
    ],
  }
  ...
  if (!res.ok) {
    ...
    await markIntegrationError(integrationId, `Ascora API ${res.status}: ${errBody}`)
    throw new Error(`Ascora API error: ${res.status} ${res.statusText} — ${errBody}`)
  }
