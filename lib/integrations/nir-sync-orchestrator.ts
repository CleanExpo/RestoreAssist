...
export async function syncNIRToAllConnectedIntegrations(
  ...
  return Promise.all(integrations.map(i => syncNIRToSpecificIntegration(i.id, payload, i.provider)))
}
...
export async function syncNIRToSpecificIntegration(
  ...
  if (!integration) return { integrationId, provider: providerHint || 'UNKNOWN', status: 'error', error: 'Integration not found' }
  if (integration.status !== 'CONNECTED') return { integrationId, provider: integration.provider, status: 'skipped', error: `Status: ${integration.status}` }
