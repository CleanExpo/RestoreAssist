...
export async function syncNIRJobToServiceM8(
  ...
  const jobRes = await fetch(`${SM8_BASE}/job.json`, {
    ...
    body: JSON.stringify({
      ...
      job_description: [
        `NIR: ${job.reportNumber}`, `Damage: ${job.damageType}`,
  ...
  await Promise.allSettled(job.scopeItems.map(item =>
    fetch(`${SM8_BASE}/jobmaterial.json`, {
      method: 'POST', headers,
      body: JSON.stringify({
        job_uuid: jobUuid,
        name: item.iicrcRef ? `${item.description} [${item.iicrcRef}]` : item.description,
