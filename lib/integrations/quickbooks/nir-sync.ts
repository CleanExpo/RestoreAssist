...
async function findOrCreateCustomer(
  ...
  const q = encodeURIComponent(
    `SELECT * FROM Customer WHERE DisplayName = '${job.clientName.replace(/'/g, "''")}'  MAXRESULTS 1`,
  );
  ...
  const createRes = await fetch(`${base}/customer`, {
    ...
    body: JSON.stringify({
      ...
      ...(job.clientEmail && {
        PrimaryEmailAddr: { Address: job.clientEmail },
      }),
      ...(job.clientPhone && {
        PrimaryPhone: { FreeFormNumber: job.clientPhone },
      }),
    }),
  });
  if (!createRes.ok)
...
export async function syncNIRJobToQuickBooks(
  ...
  if (!integration?.realmId)
    throw new Error("QuickBooks realm ID missing. Re-connect.");
  ...
  return {
    ...
    status: data.Invoice.EmailStatus || "NeedToSend",
  };
}
