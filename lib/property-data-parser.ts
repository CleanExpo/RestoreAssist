...
function parseFromNextData(
  ...
  const listing =
    (p.listing ?? p.property ?? p.data ?? p.propertyData) as Record<string, unknown> | undefined
  ...
  return {
    ...
    carSpaces: (listing.carSpaces ?? listing.garages ?? listing.parking) as number | undefined,
    landSizeM2: (listing.landSize ?? listing.landArea ?? listing.lotSize) as number | undefined,
    floorAreaM2: (listing.floorArea ?? listing.buildingArea ?? listing.houseSize) as number | undefined,
    propertyType: (listing.propertyType ?? listing.category) as string | undefined,
    address:
      typeof listing.address === "string"
        ? listing.address
        : (listing.displayAddress ?? listing.fullAddress) as string | undefined,
  }
}
...
function parseFromJsonLd(
  ...
  const obj = jsonLd.find(
    j => typeof j["@type"] === "string" && RESIDENCE_TYPES.includes(j["@type"] as string)
  )
  if (!obj) return null
  ...
  return {
    ...
    bathrooms: (obj.numberOfBathroomsTotal ?? obj.numberOfBathrooms) as number | undefined,
 ...
 */
export function parseOnTheHouseHTML(html: string, sourceUrl: string): ScrapedPropertyData {
  ...
  const bedrooms =
    merged.bedrooms ??
    extractNumber(stripped, [/(\d+)\s*bed(?:room)?s?/i, /bed(?:room)?s?\s*:\s*(\d+)/i])
  const bathrooms =
    merged.bathrooms ??
    extractNumber(stripped, [/(\d+)\s*bath(?:room)?s?/i, /bath(?:room)?s?\s*:\s*(\d+)/i])
  const carSpaces =
    merged.carSpaces ??
    extractNumber(stripped, [/(\d+)\s*car\s*(?:space|garage|park)/i, /garage\s*:\s*(\d+)/i])
  const landSizeM2 =
    merged.landSizeM2 ??
  ...
  const floorPlanImages = extractImages(html, base, (src, alt) =>
    /floor[-_]?plan|floorplan/i.test(src) || /floor[-_]?plan|floorplan/i.test(alt)
  )

  // General property images — skip icons, logos, tiny dimension thumbnails
  const propertyImages = extractImages(html, base, src =>
...
export function parseOnTheHouseSearchResults(html: string, baseUrl: string): string[] {
  ...
  if (nextData) {
    ...
    if (props && typeof props === "object") {
      const p = props as Record<string, unknown>
      const items = (p.properties ?? p.listings ?? p.results) as unknown[] | undefined
