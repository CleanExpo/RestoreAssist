...
export const scopeGenerationConfig: AgentConfig = {
  ...
  description:
    "Generates a scope of works based on IICRC classification, affected areas, and building code requirements.",
  version: "1.0.0",
  capabilities: [
    {
      name: "generate_scope",
      description:
        "Creates a list of required scope items for the restoration project",
...
export const scopeGenerationHandler: AgentHandler = async (
  input: TaskInput,
): Promise<TaskOutput> => {
  ...
  const category =
    (data.category as string) || classificationOutput?.data?.category || "1";
  const waterClass =
    (data.class as string) || classificationOutput?.data?.class || "2";
  const waterSource =
    (data.waterSource as string) ||
