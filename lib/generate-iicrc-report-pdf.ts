...
function drawPageFooter(w: PDFWriter) {
  ...
  p.drawText(
    `Generated ${new Date().toLocaleDateString("en-AU")} by RestoreAssist`,
    {
      x: A4_W - MARGIN - 145,
      y: 10,
      size: 7.5,
...
export async function generateIICRCReportPDF(
  ...
  const reportNum =
    data.reportNumber ?? `RA-${data.id.slice(0, 8).toUpperCase()}`;
  let pageNum = 1;
  ...
  w.kv(
    "Property Address:",
    `${data.propertyAddress ?? ""}${data.propertyPostcode ? `, ${data.propertyPostcode}` : ""}`,
  );
  w.kv("Client Name:", data.clientName ?? data.client?.name ?? null);
  w.kv(
    "Insurer Claim No.:",
    data.insurerClaimNumber ?? "(To be completed by insurer)",
  );
  w.kv("Date of Inspection:", fmtDate(data.inspectionDate));
  if (data.completionDate)
    w.kv("Completion Date:", fmtDate(data.completionDate));
  ...
  w.page.drawText("WATER CATEGORY", {
    x: MARGIN + 6,
    y: w.y + 12,
    size: 7,
    font: bold,
  ...
  w.page.drawText(sanitise(catLabel), {
    x: MARGIN + 6,
    y: w.y - 1,
    size: 8.5,
    font: bold,
  ...
  w.page.drawText(sanitise(classLabel), {
    x: bx + 6,
    y: w.y - 1,
    size: 8.5,
    font: bold,
  ...
  if (data.affectedArea != null)
    w.kv("Total Affected Area:", `${data.affectedArea} m²`);
  w.gap(8);
    ...
    eqArr.forEach((eq: any, i: number) => {
      w.tableRow(
        [
          ...
          (eq.deployedDate ?? eq.date)
            ? fmtDate(eq.deployedDate ?? eq.date)
            : "—",
        ],
        eWidths,
        false,
  ...
  if (data.scopeOfWorksDocument) {
    ...
    const scopeLines = data.scopeOfWorksDocument
      ...
      .replace(/\r/g, "\n")
      .split("\n");
    for (const raw of scopeLines.slice(0, 60)) {
      // cap at 60 lines in PDF
      const line = raw.trim();
      if (!line) {
        w.gap(4);
        continue;
  ...
  w.page.drawLine({
    start: { x: MARGIN + 220, y: sigY },
    end: { x: MARGIN + 420, y: sigY },
    thickness: 0.75,
    color: C_TEXT,
  });
  w.y -= 5;
  w.page.drawText("Technician Signature", {
    x: MARGIN,
    y: w.y,
    size: 8,
    font: regular,
