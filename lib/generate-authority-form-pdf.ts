 ...
 */
export async function generateAuthorityFormPDF(
  ...
  if (data.incidentDate) {
    const incidentDateStr = new Date(data.incidentDate).toLocaleDateString(
      "en-AU",
      {
        day: "2-digit",
        month: "long",
  ...
  const lastSigY =
    yPosition -
    (Math.ceil(data.signatures.length / signaturesPerRow) - 1) *
...
function drawRoundedRectangle(
  ...
  const { x, y, width, height, backgroundColor, borderColor, borderWidth } =
    options;
