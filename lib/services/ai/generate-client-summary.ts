 ...
 *
 * Service-layer migration of lib/ai/client-summary.ts. The retry +
 * validation + safe-template logic is COPIED (not imported) so this
 * service is self-contained against lib/ai/client-summary.ts being
 * retired in a follow-up.
 *
 * Composes lib/services/ai/anthropic-gateway.ts via callAnthropic.
 * Returns ServiceResult<{summary, fellBack, attempts}, AnthropicReason>.
 *
 * Semantics preserved verbatim:
 *   - up to 2 attempts (per RA-1461 spec)
 *   - IICRC citation regex /S\d+:\d+/
 *   - 120-160 word range
 *   - "What this means for you:" closer
 *   - Australian English banned-word list
 *   - deterministic safe-template fallback when both retries fail
 *   - Anthropic prompt-cache directive on the SYSTEM_PROMPT
 *
 * The route at app/api/reports/[id]/client-summary/route.ts owns auth,
 * subscription gate, rate-limit, ownership-check, cache, and BYOK key
 * resolution. The pre-resolved key is passed via the apiKey arg.
 *
...
export interface ClientSummaryServiceData {
  summary: string;
...
export async function generateClientSummaryService(args: {
  apiKey: string;
  input: ClientSummaryInput;
}): Promise<ServiceResult<ClientSummaryServiceData, AnthropicReason>> {
  const userMessage = buildUserMessage(args.input);
  const rejections: string[] = [];

  // Up to two attempts — per the ticket "retry once".
  for (let attempt = 1; attempt <= 2; attempt++) {
    const callResult = await callHaiku(
      args.apiKey,
      userMessage,
      attempt,
      rejections,
    );
    // Gateway failure short-circuits: propagate verbatim, no validation
    // retry. Only validation failures consume the second attempt.
    if (!callResult.ok) {
      return callResult;
    }

    const summary = callResult.data;
  ...
  const fallback = safeTemplate(args.input);
  console.warn(
    `[client-summary] All retries failed for report ${args.input.reportId}. ` +
...
async function callHaiku(
...
): Promise<ServiceResult<string, AnthropicReason>> {
  ...
  if (!first || first.type !== "text") {
    // Treat empty-content as an API error — the gateway succeeded but the
    // payload is unusable.
    return fail("API_ERROR", {
      detail: "client-summary: model returned no text block",
    });
  }
