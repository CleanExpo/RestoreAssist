...
export async function checkCompletion(
  ...
  const completionMap: Record<S500CompletionItemId, boolean> = {
    ...
    category_documented: inspection.classifications.some(
      (c) => (c as any).waterCategory !== null,
    ),

    class_with_area:
      inspection.classifications.some((c) => (c as any).damageClass !== null) &&
      inspection.affectedAreas.some(
        (a) => (a as any).area !== null && (a as any).area > 0,
      ),
