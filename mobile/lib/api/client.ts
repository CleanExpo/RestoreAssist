...
async function apiFetch<T>(path: string, options?: RequestInit): Promise<T> {
  ...
  if (!res.ok) {
    ...
    const err = new Error(
      (body as { error?: string }).error ?? `API error ${res.status}: ${path}`,
    );
...
export const api = {
  inspections: {
    ...
    saveEnvironmental: (
      ...
      apiFetch<{ environmentalData: any }>(
        `/api/inspections/${id}/environmental`,
        {
          method: "POST",
          body: JSON.stringify(data),
        },
  ...
  reports: {
    ...
    get: (id: string) =>
      apiFetch<{ report: import("@/shared/types").Report }>(
        `/api/reports/${id}`,
      ),
  },
};
