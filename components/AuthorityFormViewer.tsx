...
export default function AuthorityFormViewer({
  ...
  const handleSignSubmit = async () => {
    ...
    try {
      const response = await fetch(
        `/api/authority-forms/${formId}/signatures`,
        {
          method: "POST",
  ...
  const handleEmailAllParties = async () => {
    ...
    try {
      const response = await fetch(
        `/api/authority-forms/${formId}/send-completed`,
        {
          method: "POST",
        },
      );
      if (response.ok) {
        const data = await response.json();
        toast.success(
          `Signed PDF emailed to ${data.sent} ${data.sent === 1 ? "party" : "parties"}`,
        );
      } else {
        const data = await response.json().catch(() => ({}));
  ...
  const canEmail =
    ...
    form.signatures.some((s) => s.signatoryEmail && s.signedAt);
          ...
          <div className="max-w-8xl mx-auto px-6 py-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-brand-navy hover:bg-brand-navy text-white rounded-lg transition-colors"
              >
                <Download size={18} />
                Download PDF
              </button>
          ...
          <div className="space-y-5">
            ...
            <label className="flex items-start gap-3 cursor-pointer select-none">
              <input
                ...
                className="mt-1 h-4 w-4 rounded border-gray-300 text-brand-tan focus:ring-brand-navy"
              />
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-md text-sm bg-white dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-brand-navy"
              />
            </div>
