...
export default function VisualCostEstimationViewer({
  ...
  const chemicalCategories = Object.entries(categories || {})
    .filter(
      ([_, cat]: [string, any]) =>
        cat.name?.includes("Chemical") || cat.name?.includes("Treatment"),
    )
    .map(([_, cat]: [string, any]) => cat);
          ...
          <div className="mb-8 print:mb-6">
            ...
            <div className="bg-white border-2 border-slate-300 rounded-lg p-6 print:p-4 min-h-[120px] print:min-h-[100px]">
              <p className="text-sm print:text-xs text-slate-700 leading-relaxed">
                This preliminary cost estimation document provides a detailed
                ...
                and may vary based on actual conditions.
              </p>
            </div>
          </div>
          ...
          <div className="w-full p-0 px-4 space-y-8">
            ...
            <div className="space-y-6">
              ...
              {labourCategories.length > 0 &&
                labourCategories.map((category, catIdx) => (
                  <div
                    key={catIdx}
                    className="bg-blue-50 border border-blue-200 rounded-lg p-6 print:p-4"
                  >
                    <h3 className="text-xl font-bold text-slate-900 mb-4 print:mb-3">
                      {category.name}
                    </h3>
              ...
              {equipmentCategories.length > 0 &&
                equipmentCategories.map((category, catIdx) => (
                  <div
                    key={catIdx}
                    className="bg-purple-50 border border-purple-200 rounded-lg p-6 print:p-4"
                  >
                    <h3 className="text-xl font-bold text-slate-900 mb-4 print:mb-3">
                      {category.name}
                    </h3>
              ...
              {/* Chemical Categories */}
              {chemicalCategories.length > 0 &&
                chemicalCategories.map((category, catIdx) => (
                  <div
                    key={catIdx}
                    className="bg-amber-50 border border-amber-200 rounded-lg p-6 print:p-4"
                  >
                    <h3 className="text-xl font-bold text-slate-900 mb-4 print:mb-3">
                      {category.name}
                    </h3>
              ...
              {otherCategories.length > 0 &&
                otherCategories.map((category, catIdx) => (
                  <div
                    key={catIdx}
                    className="bg-slate-50 border border-slate-200 rounded-lg p-6 print:p-4"
                  >
                    <h3 className="text-xl font-bold text-slate-900 mb-4 print:mb-3">
                      {category.name}
                    </h3>
                ...
                <div className="space-y-4">
                  ...
                  {totals?.totalAdmin && totals.totalAdmin > 0 && (
                    <div className="mt-2 text-sm text-slate-600">
                      <p>
                        Note: Includes Administration, Call-Out, and Thermal
                        Imaging fees
                      </p>
                    </div>
                  )}
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <p className="text-slate-700 mt-1">
                        ${industryComparison.average?.min?.toFixed(2) || "0.00"}{" "}
                        - $
                        {industryComparison.average?.max?.toFixed(2) || "0.00"}
                      </p>
                    </div>
                    <div>
                      <span className="font-semibold text-slate-900">
                      ...
                      <p className="text-slate-700 mt-1">
                        ${industryComparison.estimated?.toFixed(2) || "0.00"}
                      </p>
                    </div>
                    <div>
                      <span className="font-semibold text-slate-900">
