...
export default function InspectionReportViewer({
  ...
  const preprocessReportContent = (content: string): string => {
    ...
    processed = processed.replace(
      /^(\*\*)?RestoreAssist\s+Inspection\s+Report(\*\*)?$/gim,
    ...
    processed = processed.replace(
      /^(?!###\s)(\*\*)?(KEY\s+[A-Z\s]+|SUBSECTION\s+[A-Z]|Subsection\s+[A-Z])(\*\*)?$/gm,
      "### $2",
    );

    // Convert "Subsection A:", "Subsection B:" etc. to H3
    processed = processed.replace(
      /^(?!###\s)(\*\*)?(Subsection\s+[A-E]:\s*[^\n]+)(\*\*)?$/gim,
      "### $2",
    );
  ...
  const handleGenerateReport = async (
    reportType: "basic" | "enhanced" = "enhanced",
  ) => {
    setGenerating(true);
    try {
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/reports/${reportId}/export-excel?includeScope=true&includeEstimate=true`,
        {
          method: "GET",
        },
      );
        ...
        <div>
          ...
          <p className="text-slate-400">
            {reportContent || visualData || structuredReportData
              ? "View and download your generated report"
              : "Generate your comprehensive inspection report"}
          </p>
        </div>
          ...
          {(reportContent || visualData || structuredReportData) && (
            <>
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-brand-bronze to-brand-bronze hover:from-green-700 hover:to-emerald-700 text-white rounded-lg transition-colors print:hidden disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {exportingExcel ? (
                  <>
      ...
      {!reportContent && !structuredReportData && !visualData && (
        <div className="p-6 rounded-lg border border-amber-500/50 bg-amber-500/10">
          ...
          <div className="flex gap-4 flex-wrap">
            ...
            {(!report?.reportDepthLevel ||
              report.reportDepthLevel === "Enhanced") && (
              <button
            ...
            {(report?.reportDepthLevel === "Optimised" ||
              report?.reportDepthLevel === "Optimized") && (
              <button
                onClick={() => handleGenerateReport("enhanced")}
                disabled={generating}
                className="px-6 py-3 bg-gradient-to-r from-brand-bronze to-brand-bronze hover:from-brand-bronze hover:to-brand-bronze text-white rounded-lg font-medium transition-colors disabled:opacity-50"
              >
            ...
            {(reportContent || visualData || structuredReportData) && (
              <button
                onClick={handleExportExcel}
                disabled={exportingExcel}
                className="px-6 py-3 bg-gradient-to-r from-brand-bronze to-brand-bronze hover:from-green-700 hover:to-emerald-700 text-white rounded-lg font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
              >
                {exportingExcel ? (
                  <>
                    <Loader2 className="w-4 h-4 animate-spin" />
          ...
          >
            ...
            <div className="w-full p-0 px-4 space-y-8">
              {isBasicReport && structuredReportData ? (
                <RestorationInspectionReportViewer
                ...
                <div className="p-6">
                  <textarea
                    ...
                    rows={30}
                    className="w-full px-4 py-2 bg-slate-700/50 border border-slate-600 rounded-lg focus:outline-none focus:border-brand-bronze font-mono text-sm text-slate-300"
                  />
                </div>
              ) : (
      ...
      {report && (
        <div
          className={cn(
            ...
            "border-neutral-200 dark:border-slate-700/50 bg-neutral-50 dark:bg-slate-800/30",
          )}
        >
          <h3
            className={cn(
          ...
          <div className="grid md:grid-cols-2 gap-2 text-sm">
            <div>
              ...
              <span
                className={cn("ml-2", "text-neutral-900 dark:text-slate-300")}
              >
                {report.reportDepthLevel || "Not set"}
              </span>
            </div>
            <div>
              <span className={cn("text-neutral-600 dark:text-slate-400")}>
