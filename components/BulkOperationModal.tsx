...
export function BulkOperationModal({
  ...
  const handleDuplicate = async () => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(data.message || data.error || "Duplication failed");
      setSuccess(true);
      toast.success(
        `Successfully duplicated ${data.duplicated || selectedCount} report(s)`,
      );
      setTimeout(() => {
        onRefresh();
        onClose();
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      ...
      setSuccess(true);
      toast.success(
        `Successfully updated ${data.updated || selectedCount} report(s)`,
      );
      setTimeout(() => {
        onRefresh();
        onClose();
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setSuccess(true);
      toast.success(
        `Successfully deleted ${data.deletedCount || selectedCount} report(s)`,
      );
      setTimeout(() => {
        onRefresh();
        onClose();
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      if (!listData.reports || listData.reports.length === 0) {
        throw new Error(
          "No Excel reports found for selected reports. Please generate Excel reports first.",
        );
      }
      ...
      toast.loading(
        `Downloading ${listData.count} Excel file(s) and creating ZIP...`,
        { id: "excel-export" },
      );
      ...
      if (
        ...
        document.body.contains(downloadLinkRef.current)
      ) {
        try {
      ...
      setSuccess(true);
      toast.success(
        `Successfully downloaded ${listData.count} Excel report(s) as ZIP!`,
        { id: "excel-export" },
      );
      setTimeout(() => {
        onClose();
              ...
              <div className="space-y-3">
                ...
                <div className="max-h-48 overflow-y-auto space-y-2">
                  ...
                  {excelReports.map((report) => (
                    ...
                    >
                      <FileSpreadsheet className="w-3 h-3 text-brand-navy shrink-0" />
                ...
                className={`flex-1 px-4 py-2 rounded-lg transition-all duration-200 text-sm font-semibold flex items-center justify-center gap-2 shadow-md hover:shadow-lg disabled:hover:shadow-md hover:scale-[1.02] active:scale-[0.98] disabled:hover:scale-100 group ${
                  operationType === "delete"
                    ...
                    : operationType === "duplicate"
                      ...
                      : "bg-gradient-to-r from-brand-bronze to-purple-700 hover:from-purple-700 hover:to-purple-800 disabled:bg-brand-bronze/50 text-white hover:shadow-brand-bronze/30"
                } disabled:opacity-50`}
              >
                {isLoading ? (
                  <>
                  ...
                  <>
                    ...
                    {operationType === "duplicate" && (
                      <Copy className="w-4 h-4 transition-transform duration-200 group-hover:scale-110 group-hover:rotate-12" />
                    )}
                    {operationType === "status-update" && (
                      <FileCheck className="w-4 h-4 transition-transform duration-200 group-hover:scale-110 group-hover:rotate-6" />
                    )}
                    <span>Confirm</span>
                  </>
                )}
