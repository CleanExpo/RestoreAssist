  ...
  return (
    <motion.div
      ...
      className={`relative p-8 rounded-2xl border transition-all duration-500 ${
        popular
          ? "border-brand-navy/50 bg-gradient-to-br from-slate-800/60 to-slate-800/80 ring-2 ring-brand-navy/20 transform scale-105 hover:scale-110"
    ...
    >
        ...
        <motion.div
          className={`absolute -top-4 left-1/2 transform -translate-x-1/2 px-6 py-2 rounded-full text-sm font-medium ${
            popular
              ...
              : "bg-gradient-to-r from-brand-bronze to-brand-tan"
          }`}
          initial={{ scale: 0 }}
          animate={{ scale: 1 }}
      ...
      <ul className="space-y-4 mb-8">
        {features.map((feature, i) => (
          <li key={i} className="flex items-center gap-3 text-slate-300">
            <CheckCircle size={20} className="text-brand-tan flex-shrink-0" />
      ...
      <motion.button
        className={`w-full py-4 rounded-xl font-medium text-lg transition-all duration-300 ${
          popular
            ? "bg-gradient-to-r from-blue-500 to-brand-tan hover:shadow-2xl hover:shadow-blue-500/50"
