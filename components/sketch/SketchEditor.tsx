  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const floorSketch = sketches.find(
        (s: { floorNumber: number; sketchData: object }) =>
          s.floorNumber === activeFloor.floorNumber,
      );
  ...
  useEffect(() => {
    ...
    const handleClick = async (opt: unknown) => {
      ...
      if (toolMode === "room") {
        ...
        const fabric = await import("fabric");
        const { IText } = fabric as unknown as {
  ...
  const addFloor = () => {
    ...
    setFloors((prev) => [
      ...prev,
      {
        ...
        canvasRef: {
          current: null,
        } as React.MutableRefObject<FabricCanvasRef | null>,
      },
    ]);
  ...
  return (
    <div className={cn("flex flex-col gap-2", className)}>
      ...
      <div className="flex items-center gap-1 overflow-x-auto pb-1">
        <Layers
          size={15}
          className="flex-shrink-0 text-neutral-400 dark:text-slate-500 mr-1"
        />
            ...
            {floors.length > 1 && (
              <button
                ...
                className={cn(
                  ...
                  idx === activeFloorIdx
                    ? "bg-brand-navy text-white border-brand-navy hover:bg-cyan-700"
        ...
        {!readonly && (
          <button
            onClick={addFloor}
            className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-sm text-neutral-500 dark:text-slate-400 border border-dashed border-neutral-300 dark:border-slate-600 hover:border-brand-navy hover:text-brand-tan transition-all flex-shrink-0"
          >
            <Plus size={13} />
            Add Floor
          </button>
        ...
        <div className="ml-auto flex items-center gap-2 flex-shrink-0">
          ...
          {inspectionId && (
            <button
              ...
              className="flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-medium border border-neutral-200 dark:border-slate-600 text-neutral-500 dark:text-slate-400 hover:border-brand-navy hover:text-brand-tan disabled:opacity-40 disabled:cursor-not-allowed transition-all"
            >
              {exportingPdf ? (
      ...
      {!readonly && (
        <FloorPlanUnderlayLoader
          ...
          autoFetch={
            !!propertyAddress && !backgroundUrls[activeFloor?.id ?? ""]
          }
        />
      )}
