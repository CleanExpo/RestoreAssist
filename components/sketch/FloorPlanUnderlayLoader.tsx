...
export function FloorPlanUnderlayLoader({
  ...
  return (
    ...
    >
      ...
      >
        <Layers size={14} className="text-brand-bronze flex-shrink-0" />
        <span className="font-medium text-neutral-700 dark:text-slate-300 flex-1 text-left">
          Floor Plan Underlay
        </span>
        {hasBackground && !expanded && (
          <span className="flex items-center gap-1 text-xs text-brand-bronze dark:text-brand-navy">
        ...
        <div className="px-3 pb-3 pt-1 border-t border-neutral-100 dark:border-slate-700/50 space-y-3">
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex gap-1.5">
              <input
                ...
                className="flex-1 min-w-0 text-sm px-3 py-1.5 rounded-lg border border-neutral-200 dark:border-slate-600 bg-white dark:bg-slate-900 text-neutral-800 dark:text-slate-200 placeholder:text-neutral-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-brand-bronze/30 focus:border-brand-navy"
              />
              <button
                type="button"
                onClick={fetchListing}
                disabled={loading || !address.trim()}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium bg-brand-bronze text-white hover:bg-brand-bronze disabled:opacity-40 disabled:cursor-not-allowed transition-colors flex-shrink-0"
              >
                {loading ? (
          ...
          <div>
            ...
            <button
              ...
              className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm border border-dashed border-neutral-300 dark:border-slate-600 text-neutral-500 dark:text-slate-400 hover:border-brand-navy hover:text-brand-bronze transition-colors"
            >
              <Upload size={13} />
              Choose image file…
            </button>
                ...
                {allImages.slice(0, 12).map((img, i) => (
                  <button
                    ...
                    className={cn(
                      ...
                      selectedImage === img
                        ? "border-brand-bronze shadow-md shadow-brand-bronze/20"
                  ...
                  >
                    ...
                    {results?.floorPlanImages.includes(img) && (
                      <span className="absolute bottom-0 left-0 right-0 text-center text-white text-[9px] font-bold bg-brand-bronze/70 py-0.5">
                        Floor Plan
                      </span>
                    )}
                  </button>
          ...
          {selectedImage && !results && (
            <div className="flex items-center gap-2 text-xs text-brand-bronze dark:text-brand-navy">
              <ImageIcon size={13} />
              Local file selected
            </div>
          )}
          ...
          <div className="flex gap-2 pt-1">
            <button
              ...
              className="flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-lg text-sm font-medium bg-brand-bronze text-white hover:bg-brand-bronze disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >
              <Layers size={13} />
              Apply to Canvas
            </button>
