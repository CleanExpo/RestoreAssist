...
export function SketchViewer({
  ...
  return (
    ...
    >
      ...
      {/* ── Footer ───────────────────────────────────────── */}
      <div className="px-5 py-2.5 border-t border-neutral-100 dark:border-slate-700 flex items-center justify-between gap-4">
        <div className="min-w-0">
          ...
          {(() => {
            const line = formatProvenanceLegend(
              summarizeSketchProvenance(activeFloor?.sketchData ?? null),
            );
            return line ? (
              <p className="truncate text-xs text-neutral-400 dark:text-slate-500">
                {line}
              </p>
            ) : null;
          })()}
        </div>
        <p className="shrink-0 text-xs text-neutral-400 dark:text-slate-500">
