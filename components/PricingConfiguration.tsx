...
export default function PricingConfiguration({
    ...
    try {
      ...
      if (profileResponse.ok) {
        ...
        currentSubscriptionStatus =
          profileData.profile?.subscriptionStatus || null;
    ...
    try {
      ...
      Object.keys(customFields).forEach((category) => {
        ...
        if (fields && fields.length > 0) {
          cleanedCustomFields[category as CustomFieldsCategory] = fields.filter(
            (f) => f.name.trim() !== "",
          );
        }
      });
        ...
        if (isOnboarding) {
          ...
          if (onboardingResponse.ok) {
            ...
            if (onboardingData.nextStep) {
              ...
              if (nextStepRoute) {
                toast.success(
                  "Step 3 complete! Redirecting to create your first report...",
                  { duration: 2000 },
                );
                setTimeout(() => {
  ...
  const updateCustomField = (
    ...
    setCustomFields((prev) => ({
      ...prev,
      [category]: (prev[category] || []).map((f, i) =>
        i === index ? { ...f, ...field } : f,
      ),
    }));
  };
              ...
              <div key={index} className="flex gap-3 items-center">
                <div className="flex-1">
                  <input
                    ...
                    value={field.name}
                    onChange={(e) =>
                      updateCustomField(category, index, {
                        name: e.target.value,
                      })
                    }
                    disabled={!canEdit}
                    className="w-full px-3 py-2 bg-gray-100 dark:bg-slate-700/50 border border-gray-300 dark:border-slate-600 rounded-lg focus:outline-none focus:border-brand-navy disabled:opacity-50 disabled:cursor-not-allowed text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-slate-400"
                  />
                </div>
                <div className="w-32">
                  <div className="relative">
                    ...
                    <input
                      ...
                      disabled={!canEdit}
                      className="w-full pl-8 pr-3 py-2 bg-gray-100 dark:bg-slate-700/50 border border-gray-300 dark:border-slate-600 rounded-lg focus:outline-none focus:border-brand-navy disabled:opacity-50 disabled:cursor-not-allowed text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-slate-400"
                    />
                  </div>
                </div>
                {canEdit && (
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-blue-500 to-brand-tan rounded-lg font-medium hover:shadow-lg hover:shadow-blue-500/50 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          >
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      disabled={!canEdit}
                      className="w-full pl-8 pr-4 py-2 bg-white dark:bg-slate-700/50 border border-gray-300 dark:border-slate-600 rounded-lg focus:outline-none focus:border-brand-navy disabled:opacity-50 disabled:cursor-not-allowed text-gray-900 dark:text-white"
                    />
                  </div>
                </div>
                <div>
