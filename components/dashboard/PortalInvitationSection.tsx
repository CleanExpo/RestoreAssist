...
export default function PortalInvitationSection({
  clientId,
  clientEmail,
  clientName,
}: PortalInvitationSectionProps) {
  ...
  const fetchInvitations = async () => {
    try {
      const response = await fetch(
        `/api/portal/invitations?clientId=${clientId}`,
      );
      if (response.ok) {
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          ...
          <div className="text-sm text-slate-400">
            <p>
              {clientName} accepted their invitation on{" "}
              {new Date(latestInvitation.acceptedAt!).toLocaleDateString()}.
            </p>
            <p className="mt-1">
              They can log in at:{" "}
        ...
        <div className="text-center py-4">
          ...
          <p className="text-slate-400 text-sm">
            No portal invitations sent yet.
          </p>
          <p className="text-slate-500 text-xs mt-1">
            Send an invitation to give this client access to their restoration
            reports.
          </p>
        </div>
      ) : (
            ...
            >
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  <span
                    className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${getStatusBadge(invitation.status).bg} ${getStatusBadge(invitation.status).text}`}
                  >
            ...
            <div className="space-y-4 mb-6">
              ...
              <div className="text-xs text-slate-400 bg-slate-700/30 p-3 rounded">
                ...
                <ul className="list-disc list-inside space-y-1">
                  ...
                  <li>
                    Once registered, they can view their restoration reports
                  </li>
