...
export type TestResultsSummaryComponentProps = HTMLAttributes<HTMLDivElement>;

export const TestResultsSummary = ({
  className,
  children,
  ...props
}: TestResultsSummaryComponentProps) => {
  const { summary } = useContext(TestResultsContext);
  ...
  return (
    <div className={cn("flex items-center gap-3", className)} {...props}>
      {children ?? (
        <>
          <Badge
            className="gap-1 bg-green-100 text-success dark:bg-green-900/30 dark:text-success"
            variant="secondary"
          >
          ...
          {summary.failed > 0 && (
            <Badge
              className="gap-1 bg-red-100 text-destructive dark:bg-red-900/30 dark:text-destructive"
              variant="secondary"
            >
          ...
          {summary.skipped > 0 && (
            <Badge
              className="gap-1 bg-yellow-100 text-warning dark:bg-yellow-900/30 dark:text-warning"
              variant="secondary"
            >
...
const statusStyles: Record<TestStatusValue, string> = {
  ...
  skipped: "text-warning dark:text-warning",
};

const statusIcons: Record<TestStatusValue, React.ReactNode> = {
...
const TestStatusIcon = ({ status }: { status: TestStatusValue }) => (
...
export type TestStatusComponentProps = HTMLAttributes<HTMLSpanElement>;

export const TestStatus = ({
  className,
  children,
  ...props
}: TestStatusComponentProps) => {
