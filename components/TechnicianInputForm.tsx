  ...
  const handleConversationComplete = (conversation: any[]) => {
    ...
    for (let i = 0; i < conversation.length - 1; i++) {
      ...
      if (systemMsg.role === "system" && nextClientMsg?.role === "client") {
        ...
        if (
          (systemContent.includes("name") ||
            systemContent.includes("your name") ||
            systemContent.includes("full name")) &&
          !extractedName
        ) {
        ...
        if (
          (systemContent.includes("address") ||
            systemContent.includes("property address") ||
            systemContent.includes("where")) &&
          !extractedAddress
        ) {
          if (
            clientContent &&
            clientContent.length > 5 &&
        ...
        if (
          (systemContent.includes("email") ||
            systemContent.includes("e-mail")) &&
          !extractedEmail
        ) {
        ...
        if (
          (systemContent.includes("phone") ||
            systemContent.includes("contact number") ||
            systemContent.includes("mobile")) &&
          !extractedPhone
        ) {
    ...
    if (
      ...
      for (
        let i = clientMessages.length - 1;
        i >= Math.max(0, clientMessages.length - 5);
        i--
      ) {
        ...
        if (
          ...
          const cleanName = message
            .replace(/^(my name is|i'm|i am|name is)\s+/i, "")
            .trim();
    ...
    setActiveTab("notes");
    toast.success(
      "Client information collected! You can now add your technician notes.",
    );
  };
    ...
    <Card className="border-slate-700 bg-slate-800/30">
      <CardHeader>
        ...
        <CardDescription className="text-slate-300">
          First gather information from the client, then enter your technician
          notes. The system will automatically generate a comprehensive
          professional report.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <Tabs
          value={activeTab}
          onValueChange={(value) => setActiveTab(value as "qa" | "notes")}
          className="w-full"
        >
          <TabsList className="grid w-full grid-cols-2 mb-6 bg-slate-800">
            <TabsTrigger value="qa" className="data-[state=active]:bg-brand-bronze">
              <MessageCircle className="mr-2 h-4 w-4" />
              Client Q&A
            </TabsTrigger>
            <TabsTrigger
              value="notes"
              className="data-[state=active]:bg-brand-bronze"
            >
          ...
          <TabsContent value="notes" className="mt-0 space-y-6">
            ...
            {(clientName || propertyAddress || clientEmail || clientPhone) && (
              <div className="p-4 bg-brand-bronze/10 border border-brand-bronze/30 rounded-lg space-y-3">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                className="bg-slate-900 border-slate-600 text-white placeholder:text-slate-500 min-h-[80px] focus:border-brand-bronze focus:ring-brand-bronze"
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                className="bg-slate-900 border-slate-600 text-white placeholder:text-slate-500 min-h-[300px] font-mono text-sm focus:border-brand-bronze focus:ring-brand-bronze"
              />
              <p className="text-xs text-slate-300">
                Enter all relevant information from your inspection. Include
                areas affected, equipment used, observations, and any other
                details.
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <label className="w-20 h-20 border-2 border-dashed border-slate-600 rounded flex items-center justify-center cursor-pointer hover:border-brand-bronze transition-colors bg-slate-900/50">
            ...
            <Button
              ...
              className="w-full bg-brand-bronze hover:bg-cyan-700 text-white"
              size="lg"
            >
              {generating ? (
                <>
