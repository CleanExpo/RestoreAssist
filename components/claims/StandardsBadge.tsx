  ...
  const getStandardType = (ref: string) => {
    ...
    if (upper.includes("NCC")) {
      return {
        ...
        icon: Shield,
        color:
          "bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-brand-navy border-brand-navy dark:border-purple-700",
        label: ref.match(/NCC\s+\d{4}/i)?.[0] || ref,
      };
    }

    if (
      upper.includes("AS/NZS") ||
      upper.includes("AS ") ||
      upper.includes("NZS ")
    ) {
      return {
        ...
        icon: Scale,
        color:
          "bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-brand-navy border-brand-navy dark:border-emerald-700",
        label:
    ...
    return {
      ...
      icon: FileText,
      color:
        "bg-slate-100 text-slate-800 dark:bg-slate-900/30 dark:text-slate-400 border-slate-300 dark:border-slate-700",
      label: ref,
    };
  };
...
export function StandardsGroup({
  ...
  return (
    <div className={cn("flex flex-wrap items-center gap-1.5", className)}>
      {visible.map((ref, idx) => (
        <StandardsBadge key={idx} standardRef={ref} size={size} />
      ))}
      {remaining > 0 && (
        <Badge
          variant="outline"
