...
export function NotificationBell({ className }: NotificationBellProps) {
  ...
  const getTypeStyles = (type: Notification["type"]) => {
    const styles = {
      info: "bg-brand-bronze/10 text-brand-bronze",
      success: "bg-brand-bronze/10 text-brand-bronze",
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          ...
          {unreadCount > 0 && (
            <span className="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-brand-bronze text-xs font-medium text-white">
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-neutral-200 dark:border-neutral-800">
          ...
          {unreadCount > 0 && (
            <Button
              variant="ghost"
              size="sm"
              className="text-xs text-brand-bronze hover:text-brand-navy"
              ...
              {notifications.map((notification) => (
                <div
                  ...
                  className={cn(
                    ...
                    !notification.read && "bg-brand-bronze/5",
                  )}
                >
        ...
        {notifications.length > 0 && (
          <div className="p-2 border-t border-neutral-200 dark:border-neutral-800">
            <Button
              variant="ghost"
              className="w-full text-sm text-brand-bronze hover:text-brand-navy"
