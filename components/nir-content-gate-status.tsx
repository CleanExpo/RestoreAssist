...
const GATE_STATUS_STYLES: Record<GateStatus, {
  pill: string
  label: string
  icon: string
  border: string
...
function GateStatusPill({ status }: { status: GateStatus }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-semibold ${styles.pill}`}>
  ...
  return (
    <div className="mt-3 rounded-md border border-slate-200 bg-slate-50 p-3">
      <div className="flex items-start justify-between gap-2">
        <div className="text-xs font-medium text-slate-700">Certification requirement</div>
...
function ClaimRow({ claim, blocked }: {
  claim: { id: string; claim: string; status: string; source?: string; notes?: string }
  blocked: boolean
}) {
  return (
    <li className={`flex items-start gap-2 py-1.5 text-xs ${blocked ? 'text-red-700' : 'text-green-700'}`}>
      <span className="mt-0.5 shrink-0 font-mono font-bold">{claim.id}</span>
      <span className={`shrink-0 rounded px-1.5 py-0.5 font-medium uppercase tracking-wide text-[10px] ${
      ...
      <div className="px-4 py-3 space-y-3">
        ...
        {result.requiredActions.length > 0 && (
          <div className="rounded-md bg-slate-50 border border-slate-200 p-3">
            <p className="text-xs font-semibold text-slate-700 mb-2">Required to open gate</p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-slate-900">NIR Content Gate</h2>
          <p className="text-sm text-slate-500 mt-0.5">
            Evidence-based publication gate — all domains must be open before customer-facing
            content referencing NIR capabilities can be published.
          </p>
        </div>
        <div className="text-right">
          <div className={`text-2xl font-bold tabular-nums ${
            openCount === totalCount ? 'text-green-600' : openCount > 0 ? 'text-amber-600' : 'text-red-600'
          }`}>
      ...
      {openCount === 0 && (
        ...
        >
          <strong>All content gates blocked.</strong> No customer-facing content referencing NIR
          capabilities, IICRC standards, or cost savings claims may be published. See{' '}
          <code className="rounded bg-red-100 px-1 py-0.5 text-xs">docs/CONTENT-GATE.md</code>{' '}
      ...
      <p className="text-xs text-slate-400 text-right">
        ...
        {' · '}
        Source of truth:{' '}
        <code className="rounded bg-slate-100 px-1">lib/nir-content-gate.ts</code>
      </p>
    </div>
  )
}
