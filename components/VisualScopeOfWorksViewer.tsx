...
export default function VisualScopeOfWorksViewer({
  ...
  * {
    ...
    background-image: none !important;
      ...
      <div
        id="scope-of-works-content"
        className="bg-white text-slate-900 print-content"
      >
        {/* PAGE 1: PROFESSIONAL COVER PAGE */}
        <div
          className="min-h-[297mm] print:min-h-[297mm] max-w-5xl mx-auto flex flex-col print-page"
        ...
        >
          ...
          <div className="mb-8 print:mb-6">
            <p className="text-base print:text-sm text-slate-700">
              ...
              {formatDate(
                incident?.technicianAttendanceDate || header?.dateGenerated,
              )}
            </p>
            {header?.claimReference && (
              <p className="text-base print:text-sm text-slate-700 mt-2 print:mt-1">
                <span className="font-semibold">Based on:</span> Inspection
                Report {header.claimReference}
              </p>
            )}
          </div>
          ...
          <div className="mb-8 print:mb-6">
            ...
            <div className="bg-white border-2 border-slate-300 rounded-lg p-6 print:p-4 min-h-[120px] print:min-h-[100px]">
              <p className="text-sm print:text-xs text-slate-700 leading-relaxed">
                This preliminary scope of works document outlines the
                restoration works required to address water damage at the
                property. The scope includes {phases.length} remediation phases,{" "}
                {lineItems.length} restoration work line items
                {licensedTrades.length > 0
                  ? `, and ${licensedTrades.length} licensed trade requirement${licensedTrades.length > 1 ? "s" : ""}`
                  : ""}
                . All works will be conducted in accordance with IICRC S500
                standards and relevant Australian regulations.
              </p>
            </div>
          </div>
          ...
          <div className="mt-auto pt-8 print:pt-6 border-t-2 border-slate-300">
            <div className="text-center text-xs print:text-[10px] text-slate-600 space-y-1">
              ...
              {header?.businessAddress && <p>{header.businessAddress}</p>}
          ...
          <div className="w-full p-0 px-4 space-y-8">
              ...
              {lineItems.length > 0 ? (
                    ...
                    <tbody>
                      {lineItems.map((item, idx) => (
                        <tr key={idx} className="hover:bg-slate-50">
                          <td className="border border-slate-300 px-4 py-3 font-mono text-sm text-slate-700">
                            {item.code}
                          </td>
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
                  <p className="text-amber-800">
                    No restoration works line items available.
                  </p>
                </div>
              )}
            </div>
            ...
            {insuranceBreakdown && (
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  {insuranceBreakdown.buildingClaim &&
                    insuranceBreakdown.buildingClaim.length > 0 && (
                  ...
                  {insuranceBreakdown.contentsClaim &&
                    insuranceBreakdown.contentsClaim.length > 0 && (
                  ...
                  {insuranceBreakdown.additionalLivingExpenses &&
                    insuranceBreakdown.additionalLivingExpenses.length > 0 && (
                      <div className="bg-amber-50 border border-amber-200 rounded-lg p-6">
                        <h3 className="text-xl font-bold text-slate-900 mb-3">
                          ADDITIONAL LIVING EXPENSES (if property uninhabitable)
                        </h3>
                        <ul className="list-disc list-inside space-y-1 text-slate-700">
                          {insuranceBreakdown.additionalLivingExpenses.map(
                            (item, idx) => (
                              <li key={idx}>{item}</li>
                            ),
                          )}
            ...
            <div className="space-y-6 border-t border-slate-300 pt-6">
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-lg p-6">
                <p className="text-slate-700">
                  <strong>Note:</strong> All line items, quantities, rates, and
                  calculations can be edited by the admin before finalising.
                  System maintains calculation formulas but allows manual
                  override.
                </p>
              </div>
            </div>
