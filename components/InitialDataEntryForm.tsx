...
export default function InitialDataEntryForm({
  ...
  return (
    <div className="max-w-full mx-auto">
      ...
      {/* Review Page - Shows all data before report type selection */}
      {showReview && (
        <div className={cn(
          ...
          <div className="mb-6">
            <h2 className={cn("text-2xl font-semibold mb-2 flex items-center gap-2", "text-neutral-900 dark:text-neutral-50")}>
              <FileText className="w-6 h-6 text-primary-600 dark:text-primary-400" />
              Review All Data
            </h2>
            <p className={cn("text-neutral-600 dark:text-neutral-400")}>
              Please review all entered data before proceeding to report type selection and PDF generation.
            </p>
          </div>

          {/* Client Information */}
          <div className={cn("p-6 rounded-lg border", "bg-white dark:bg-neutral-900/50", "border-neutral-200 dark:border-neutral-700")}>
            <h3 className={cn("text-lg font-semibold mb-4 flex items-center gap-2", "text-neutral-900 dark:text-neutral-50")}>
              <User className="w-5 h-5" />
              Client Information
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div>
                <span className={cn("text-neutral-600 dark:text-neutral-400")}>Client Name:</span>
                <p className={cn("font-medium", "text-neutral-900 dark:text-neutral-50")}>{formData.clientName || "—"}</p>
              </div>
              <div>
                <span className={cn("text-neutral-600 dark:text-neutral-400")}>Contact Details:</span>
                <p className={cn("font-medium", "text-neutral-900 dark:text-neutral-50")}>{formData.clientContactDetails || "—"}</p>
              </div>
            </div>
          </div>
          ...
          <div className={cn("p-6 rounded-lg border", "bg-white dark:bg-neutral-900/50", "border-neutral-200 dark:border-neutral-700")}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div>
                <span className={cn("text-neutral-600 dark:text-neutral-400")}>Address:</span>
                <p className={cn("font-medium", "text-neutral-900 dark:text-neutral-50")}>{formData.propertyAddress || "—"}</p>
              </div>
              <div>
                <span className={cn("text-neutral-600 dark:text-neutral-400")}>Postcode:</span>
                <p className={cn("font-medium", "text-neutral-900 dark:text-neutral-50")}>{formData.propertyPostcode || "—"}</p>
              </div>
              {formData.buildingAge && (
                <div>
                  <span className={cn("text-neutral-600 dark:text-neutral-400")}>Building Age:</span>
                  <p className={cn("font-medium", "text-neutral-900 dark:text-neutral-50")}>{formData.buildingAge}</p>
                </div>
              )}
              {formData.structureType && (
                <div>
                  <span className={cn("text-neutral-600 dark:text-neutral-400")}>Structure Type:</span>
                  <p className={cn("font-medium", "text-neutral-900 dark:text-neutral-50")}>{formData.structureType}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className={cn("p-6 rounded-lg border", "bg-white dark:bg-neutral-900/50", "border-neutral-200 dark:border-neutral-700")}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              {formData.claimReferenceNumber && (
                <div>
                  <span className={cn("text-neutral-600 dark:text-neutral-400")}>Claim Reference:</span>
                  <p className={cn("font-medium", "text-neutral-900 dark:text-neutral-50")}>{formData.claimReferenceNumber}</p>
                </div>
              )}
              {formData.incidentDate && (
                <div>
                  <span className={cn("text-neutral-600 dark:text-neutral-400")}>Incident Date:</span>
                  <p className={cn("font-medium", "text-neutral-900 dark:text-neutral-50")}>{formData.incidentDate}</p>
                </div>
              )}
              {formData.technicianName && (
                <div>
                  <span className={cn("text-neutral-600 dark:text-neutral-400")}>Technician:</span>
                  <p className={cn("font-medium", "text-neutral-900 dark:text-neutral-50")}>{formData.technicianName}</p>
                </div>
              )}
              {formData.technicianAttendanceDate && (
                <div>
                  <span className={cn("text-neutral-600 dark:text-neutral-400")}>Attendance Date:</span>
                  <p className={cn("font-medium", "text-neutral-900 dark:text-neutral-50")}>{formData.technicianAttendanceDate}</p>
                </div>
              )}
            </div>
            {formData.technicianFieldReport && (
              <div className="mt-4">
                <span className={cn("text-neutral-600 dark:text-neutral-400")}>Field Report:</span>
                <p className={cn("mt-1 p-3 rounded-lg bg-neutral-50 dark:bg-neutral-800", "text-neutral-900 dark:text-neutral-50")}>
                  {formData.technicianFieldReport}
                </p>
              </div>
            )}
          ...
          {nirMoistureReadings.length > 0 && (
            <div className={cn("p-6 rounded-lg border", "bg-white dark:bg-neutral-900/50", "border-neutral-200 dark:border-neutral-700")}>
              ...
              <div className="space-y-2">
                {nirMoistureReadings.map((reading) => (
                  <div key={reading.id} className={cn("flex items-center justify-between p-3 rounded-lg", "bg-neutral-50 dark:bg-neutral-800")}>
                    <div className="flex items-center gap-4 text-sm">
                      <span className={cn("font-medium", "text-neutral-900 dark:text-neutral-50")}>{reading.location}</span>
                      <span className={cn("text-neutral-600 dark:text-neutral-400")}>{reading.surfaceType}</span>
                      <span className={cn("font-semibold text-cyan-600 dark:text-cyan-400")}>{reading.moistureLevel}%</span>
                      <span className={cn("text-neutral-600 dark:text-neutral-400")}>{reading.depth}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {nirAffectedAreas.length > 0 && (
            <div className={cn("p-6 rounded-lg border", "bg-white dark:bg-neutral-900/50", "border-neutral-200 dark:border-neutral-700")}>
              ...
              <div className="space-y-3">
                {nirAffectedAreas.map((area) => (
                  <div key={area.id} className={cn("p-4 rounded-lg", "bg-neutral-50 dark:bg-neutral-800")}>
                    <div className="flex items-center gap-3 mb-2">
                      <span className={cn("font-medium text-base", "text-neutral-900 dark:text-neutral-50")}>{area.roomZoneId}</span>
                      <span className={cn("text-xs px-2 py-1 bg-cyan-500/20 text-cyan-600 dark:text-cyan-400 rounded")}>
      ...
      {/* Report Type Selection - Appears after review (outside form) */}
