...
export default function IICRCDashboard({ reports }: IICRCDashboardProps) {
  ...
  const calculateComplianceStats = () => {
    ...
    const stats = {
      ...
      category1Reports: filteredReports.filter(
        (r) => r.waterCategory === "Category 1",
      ).length,
      category2Reports: filteredReports.filter(
        (r) => r.waterCategory === "Category 2",
      ).length,
      category3Reports: filteredReports.filter(
        (r) => r.waterCategory === "Category 3",
      ).length,
  ...
  const calculateAverageDryingTime = (reports: any[]) => {
    ...
    const totalTime = reportsWithDryingTime.reduce(
      (sum, report) => sum + (report.estimatedDryingTime || 0),
      0,
    );
  ...
  const getCategoryDistribution = () => {
    ...
    return [
      ...
      },
      {
        ...
        percentage: Math.round(
          (complianceStats.category2Reports / total) * 100,
        ),
  ...
  const getClassDistribution = () => {
    ...
    return Object.entries(classCounts).map(([waterClass, count]) => ({
      ...
      percentage: Math.round(
        ((count as number) / complianceStats.totalReports) * 100,
      ),
    }));
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400">
            Professional water damage restoration standards tracking
          </p>
        </div>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <FileText className="text-brand-tan" size={24} />
        ...
        >
          ...
          <div className="space-y-1">
            <p className="text-3xl font-bold text-orange-400">
              {complianceStats.averageDryingTime}h
            </p>
            <p className="text-slate-400 text-sm font-medium">
              Avg Drying Time
            </p>
        ...
        >
          ...
          <div className="space-y-3">
            {getClassDistribution().map((item, index) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-3 h-3 rounded-full bg-brand-navy/20" />
                  <span className="text-slate-300">{item.class}</span>
                </div>
                <div className="flex items-center gap-3">
                  <span className="font-bold text-brand-tan">{item.count}</span>
                  <span className="text-slate-400 text-sm">
                    ({item.percentage}%)
                  </span>
                </div>
