...
export default function InsuranceDashboard({
  ...
  const calculateInsuranceStats = () => {
    ...
    const stats = {
      ...
      propertyCoverage: calculateCoveragePercentage(
        filteredReports,
        "propertyCover",
      ),
      contentsCoverage: calculateCoveragePercentage(
        filteredReports,
        "contentsCover",
      ),
      liabilityCoverage: calculateCoveragePercentage(
        filteredReports,
        "liabilityCover",
      ),
      businessInterruption: calculateCoveragePercentage(
        filteredReports,
    ...
    stats.averageCoverage = Math.round(
      (stats.propertyCoverage +
        stats.contentsCoverage +
        stats.liabilityCoverage +
        stats.businessInterruption) /
        4,
    );
  ...
  const getInsuranceSummary = () => {
    return {
      totalValue: reports.reduce(
        (sum, report) => sum + (report.totalCost || 0),
        0,
      ),
      averageClaim:
        reports.length > 0
          ? reports.reduce((sum, report) => sum + (report.totalCost || 0), 0) /
            reports.length
          : 0,
      coverageGaps: reports.filter(
        (report) =>
          !report.propertyCover &&
          !report.contentsCover &&
          !report.liabilityCover,
      ).length,
    };
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400">
            Small business insurance coverage analysis and insights
          </p>
        </div>
        ...
        <div className="flex gap-2">
          <select
            ...
            className="px-4 py-2 bg-slate-700/50 border border-slate-600 rounded-lg focus:outline-none focus:border-brand-navy"
          >
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <Shield className="text-brand-tan" size={24} />
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <DollarSign className="text-brand-bronze" size={24} />
            <div className="w-2 h-2 bg-brand-bronze rounded-full animate-pulse" />
          </div>
          <div className="space-y-1">
        ...
        >
          ...
          <div className="space-y-1">
            <p className="text-3xl font-bold text-orange-400">
              ${Math.round(summary.averageClaim).toLocaleString()}
            </p>
            <p className="text-slate-400 text-sm font-medium">
              Avg Claim Value
            </p>
          ...
          <div className="space-y-4">
            {coverageDistribution.map((coverage, index) => (
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  <span className={`font-bold ${coverage.color}`}>
                    {coverage.percentage}%
                  </span>
            ...
            <div className="flex items-center justify-between p-3 bg-slate-700/30 rounded-lg">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-blue-400 font-bold">
                  {insuranceStats.propertyCoverage}%
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between p-3 bg-slate-700/30 rounded-lg">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-amber-400 font-bold">
                  {insuranceStats.liabilityCoverage}%
                </span>
              </div>
            </div>
      ...
      >
        ...
        <div className="space-y-3">
          {recentReports.map((report, index) => {
            ...
            const hasContents =
              report.contentsCover && JSON.parse(report.contentsCover);
            const hasLiability =
              report.liabilityCover && JSON.parse(report.liabilityCover);
            const hasInterruption =
              report.businessInterruption &&
