...
export default function ScopingEngine({
  reportId,
  reportData,
  initialScopeData,
  onScopeComplete,
  ...
  const [scopeData, setScopeData] = useState({
    ...
    chemicalApplication: {
      chemicals: [] as Array<{ type: string; area: number }>,
    },
  ...
  const chemicalRates = {
    "Anti-microbial": 1.5,
    "Bio-decontamination": 2.5,
    "Mould Remediation": 1.5,
    "Odor Control": 1.0,
  };
    ...
    if (initialScopeData) {
      try {
        const parsedData = {
          ...
          siteVariables:
            typeof initialScopeData.siteVariables === "string"
          ...
          labourParameters:
            typeof initialScopeData.labourParameters === "string"
          ...
          timeCalculations:
            typeof initialScopeData.timeCalculations === "string"
              ...
              : initialScopeData.timeCalculations || {
                  ...
                  totalManHours:
                    initialScopeData.timeCalculations?.totalManHours || 0,
  ...
  useEffect(() => {
    ...
    scopeData.chemicalApplication,
    scopeData.timeCalculations,
  ]);

  // Update project duration when calculations change
  useEffect(() => {
    if (
      reportData?.affectedArea &&
      scopeData.timeCalculations.baseProductivity
    ) {
      const affectedArea = reportData.affectedArea || 0;
      let efficiency = 1;
      if (scopeData.labourParameters.modifiers.confinedSpace)
        efficiency *= 0.75;
      if (scopeData.labourParameters.modifiers.ppe) efficiency *= 0.85;
      if (scopeData.labourParameters.modifiers.heatStress) efficiency *= 0.8;
      if (scopeData.labourParameters.modifiers.teamSize === "large")
        efficiency *= 0.9;

      const adjustedProductivity =
        scopeData.timeCalculations.baseProductivity * efficiency;
    ...
    reportData?.affectedArea,
    scopeData.timeCalculations.baseProductivity,
    scopeData.labourParameters.modifiers,
  ]);
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="bg-slate-800/50 p-4 rounded-lg">
          ...
          <p className="text-slate-300">
            <strong>Address:</strong> {reportData?.propertyAddress}
          </p>
          <p className="text-slate-300">
            <strong>Category:</strong> {reportData?.waterCategory}
          </p>
          <p className="text-slate-300">
            <strong>Class:</strong> {reportData?.waterClass}
          </p>
          <p className="text-slate-300">
            <strong>Affected Area:</strong> {reportData?.affectedArea} sqm
          </p>
        </div>
      </div>

      <div>
        <Label htmlFor="scopeType" className="text-white mb-2">
          Scope Type *
        </Label>
        <Select
          value={scopeData.scopeType}
          onValueChange={(value) =>
            setScopeData((prev) => ({ ...prev, scopeType: value }))
          }
        >
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <Label htmlFor="structure" className="text-white mb-2">
              Structure Type
            </Label>
            <Input
              id="structure"
          ...
          <div>
            <Label htmlFor="materials" className="text-white mb-2">
          ...
          <div>
            <Label htmlFor="floors" className="text-white mb-2">
              Floors Affected
            </Label>
            <Input
              id="floors"
          ...
          <div>
            <Label htmlFor="condition" className="text-white mb-2">
              Site Condition
            </Label>
            <Input
              id="condition"
              ...
              onCheckedChange={(checked) =>
                setScopeData((prev) => ({
                  ...
                  labourParameters: {
                    ...prev.labourParameters,
                    modifiers: {
                      ...prev.labourParameters.modifiers,
                      weekend: checked as boolean,
                    },
                  },
                }))
              }
              ...
              onCheckedChange={(checked) =>
                setScopeData((prev) => ({
                  ...
                  labourParameters: {
                    ...
                    modifiers: {
                      ...prev.labourParameters.modifiers,
                      confinedSpace: checked as boolean,
                    },
                  },
                }))
              }
            />
            <Label htmlFor="confinedSpace" className="text-white">
              Confined Space (+10%)
            </Label>
          </div>
          <div className="flex items-center space-x-2">
              ...
              onCheckedChange={(checked) =>
                setScopeData((prev) => ({
                  ...
                  labourParameters: {
                    ...
                    modifiers: {
                      ...prev.labourParameters.modifiers,
                      ppe: checked as boolean,
                    },
                  },
                }))
              }
            />
            <Label htmlFor="ppe" className="text-white">
              PPE Required (+15%)
            </Label>
          </div>
          <div className="flex items-center space-x-2">
              ...
              onCheckedChange={(checked) =>
                setScopeData((prev) => ({
                  ...
                  labourParameters: {
                    ...
                    modifiers: {
                      ...prev.labourParameters.modifiers,
                      heatStress: checked as boolean,
                    },
                  },
                }))
              }
            />
            <Label htmlFor="heatStress" className="text-white">
              Heat Stress (+10%)
            </Label>
          </div>
        </div>
        <div className="mt-4">
          <Label htmlFor="teamSize" className="text-white mb-2">
            Team Size
          </Label>
          <Select
      ...
      <div>
        ...
        <div className="border border-slate-700 rounded-lg overflow-hidden">
          <Table>
            ...
            <TableBody>
              {scopeData.equipmentParameters.equipment.map(
                (eq: any, index: number) => {
                  const tiers =
                    equipmentRates[eq.type as keyof typeof equipmentRates];
                  let cost = 0;
                  if (tiers) {
                    if (eq.duration <= 7) {
                      cost = tiers.day * eq.quantity * eq.duration;
                    ...
                    <TableRow key={index} className="bg-slate-800/50">
                      <TableCell>
                        <Select
                          ...
                          onValueChange={(value) => {
                            const newEquip = [
                              ...scopeData.equipmentParameters.equipment,
                            ];
                      ...
                      <TableCell>
                        <Button
                          ...
                          onClick={() => {
                            const newEquip =
                              scopeData.equipmentParameters.equipment.filter(
        ...
        <Button
          onClick={() => {
            const newEquip = [
              ...scopeData.equipmentParameters.equipment,
              { type: "Air Mover", quantity: 1, duration: 1 },
            ];
            setScopeData((prev) => ({
              ...prev,
              equipmentParameters: { equipment: newEquip },
          ...
          className="mt-4 bg-brand-navy hover:bg-cyan-700"
        >
          Add Equipment
        </Button>
      </div>
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="border border-slate-700 rounded-lg overflow-hidden">
          <Table>
            ...
            <TableBody>
              {scopeData.chemicalApplication.chemicals.map(
                (chem: any, index: number) => {
                  const rate =
                    chemicalRates[chem.type as keyof typeof chemicalRates] || 0;
                  return (
                    <TableRow key={index} className="bg-slate-800/50">
                      <TableCell>
                        <Select
                          ...
                          onValueChange={(value) => {
                            const newChems = [
                              ...scopeData.chemicalApplication.chemicals,
                            ];
                      ...
                      <TableCell>
                        <Button
                          ...
                          onClick={() => {
                            const newChems =
                              scopeData.chemicalApplication.chemicals.filter(
                                (_: any, i: number) => i !== index,
                              );
                            setScopeData((prev) => ({
                              ...prev,
      ...
      <div className="bg-slate-800/50 p-4 rounded-lg">
        <p className="text-slate-300 text-sm">
          <strong>Note:</strong> Areas are pulled from inspection data. You can
          modify room sizes or surface types manually above.
        </p>
      </div>
    </div>
  );
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            ...
            <Card className="bg-slate-800 border-slate-700">
              ...
              <CardContent>
                <p className="text-2xl font-bold text-brand-tan">
                  {adjustedProductivity.toFixed(2)} sqm/hr
                </p>
                <p className="text-sm text-slate-400 mt-1">
                  After efficiency factors
                </p>
              </CardContent>
            </Card>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              <Label htmlFor="timeNotes" className="text-white mb-2">
                Notes
              </Label>
              <Textarea
                id="timeNotes"
      ...
      <div>
        ...
        <div className="mt-6">
          <Card className="bg-slate-800 border-slate-700">
            ...
            <CardContent>
              <p className="text-4xl font-bold text-brand-tan">
                $
                {(
                  scopeData.summary.labourCostTotal +
                  scopeData.summary.equipmentCostTotal +
                  scopeData.summary.chemicalCostTotal
                ).toFixed(2)}
              </p>
  ...
  const renderStep7 = () => (
    <div className="space-y-6">
      <div>
        <Label htmlFor="complianceNotes" className="text-white mb-2">
          Compliance Notes
        </Label>
        <Textarea
          id="complianceNotes"
      ...
      <div>
        ...
        <Textarea
          ...
          onChange={(e) =>
            setScopeData((prev) => ({ ...prev, assumptions: e.target.value }))
          }
          className="bg-slate-800 border-slate-700 text-white"
          rows={6}
      ...
      <div className="bg-slate-800/50 p-4 rounded-lg">
        ...
        <ul className="space-y-2 text-slate-300 text-sm">
          <li className="flex items-center space-x-2">
            ...
            <span>
              Required fields completed (location, category, technician rates)
            </span>
          </li>
          <li className="flex items-center space-x-2">
            <input type="checkbox" className="rounded" />
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="bg-slate-800 rounded-lg p-6 space-y-4">
          <div>
            <h4 className="text-brand-tan font-semibold mb-2">
              1. Project Summary
            </h4>
            <div className="text-slate-300 text-sm space-y-1">
              ...
              <p>
                <strong>Scope Type:</strong> {scopeData.scopeType}
              </p>
              <p>
          ...
          <div>
            ...
            <div className="text-slate-300 text-sm space-y-1">
              <p>
                ...
                {scopeData.siteVariables.structure || "—"}
              </p>
              <p>
                <strong>Materials:</strong>{" "}
                {scopeData.siteVariables.materials || "—"}
              </p>
              <p>
                <strong>Floors:</strong> {scopeData.siteVariables.floors || "—"}
              </p>
              <p>
                <strong>Condition:</strong>{" "}
                {scopeData.siteVariables.condition || "—"}
              </p>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="text-slate-300 text-sm">
              <p>
                <strong>Total Chemical Cost:</strong> $
                {scopeData.summary.chemicalCostTotal.toFixed(2)}
              </p>
            </div>
          </div>
          ...
          <div>
            <h4 className="text-brand-tan font-semibold mb-2">8. Cost Totals</h4>
            <div className="text-slate-300 text-sm space-y-1">
              <p>
              ...
              <p className="text-lg font-semibold text-brand-tan mt-2">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-slate-400">
            Convert inspection data into structured scope of works
          </p>
        </div>
            ...
            return (
              ...
              >
                <div className="flex flex-col items-center flex-1">
                  ...
                  <span
                    className={`text-xs mt-2 text-center ${isActive ? "text-brand-tan" : "text-slate-400"}`}
                  >
                    {step.number}
                  </span>
                  <span
                    className={`text-xs mt-1 text-center hidden md:block ${isActive ? "text-white" : "text-slate-500"}`}
                  >
                    {step.title.split(" ")[0]}
                  </span>
                </div>
