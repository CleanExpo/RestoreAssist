...
const PHOTO_CATEGORIES = [
  {
    id: "site_damage",
    label: "Site & Damage Photography",
    max: 15,
    description: "Damage assessment, technical findings, evidence",
  },
  {
    id: "moisture_mapping",
    label: "Moisture Mapping & Thermal Imaging",
    max: 10,
    description: "Moisture readings, thermal images, mapping data",
  },
  {
    id: "equipment_deployment",
    label: "Equipment Deployment",
    max: 8,
    description: "Equipment setup, placement, monitoring",
  },
  {
    id: "remediation_progress",
    label: "Remediation Progress",
    max: 10,
    description: "Work in progress, drying progress, restoration steps",
  },
  {
    id: "structural_assessment",
    label: "Structural Assessment",
    max: 8,
    description: "Structural damage, integrity concerns, building elements",
  },
  {
    id: "final_verification",
    label: "Final Verification & Handover",
    max: 5,
    description: "Completion photos, verification, handover documentation",
  },
];
...
export default function Tier3Questions({
  reportId,
  onComplete,
}: Tier3QuestionsProps) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const initialDataFields = {
          phase1StartDate: data.phase1StartDate
            ? new Date(data.phase1StartDate).toISOString().split("T")[0]
            : "",
          phase1EndDate: data.phase1EndDate
            ? new Date(data.phase1EndDate).toISOString().split("T")[0]
            : "",
          phase2StartDate: data.phase2StartDate
            ? new Date(data.phase2StartDate).toISOString().split("T")[0]
            : "",
          phase2EndDate: data.phase2EndDate
            ? new Date(data.phase2EndDate).toISOString().split("T")[0]
            : "",
          phase3StartDate: data.phase3StartDate
            ? new Date(data.phase3StartDate).toISOString().split("T")[0]
            : "",
          phase3EndDate: data.phase3EndDate
            ? new Date(data.phase3EndDate).toISOString().split("T")[0]
            : "",
        };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updateResponse = await fetch(`/api/reports/${reportId}`, {
        ...
        body: JSON.stringify({
          phase1StartDate: responses.phase1StartDate
            ? new Date(responses.phase1StartDate).toISOString()
            : null,
          phase1EndDate: responses.phase1EndDate
            ? new Date(responses.phase1EndDate).toISOString()
            : null,
          phase2StartDate: responses.phase2StartDate
            ? new Date(responses.phase2StartDate).toISOString()
            : null,
          phase2EndDate: responses.phase2EndDate
            ? new Date(responses.phase2EndDate).toISOString()
            : null,
          phase3StartDate: responses.phase3StartDate
            ? new Date(responses.phase3StartDate).toISOString()
            : null,
          phase3EndDate: responses.phase3EndDate
            ? new Date(responses.phase3EndDate).toISOString()
            : null,
        }),
      });
      ...
      if (photos.length > 0) {
        ...
        photos.forEach((photo) => {
          ...
          if (photo.description) {
            formData.append(
              `photoDescription_${photo.file.name}`,
              photo.description,
            );
          }
        });
    ...
    <div className="space-y-6">
      <div className="p-4 rounded-lg border-2 border-green-500/50 bg-green-50 dark:bg-green-500/10">
        ...
        <p className="text-sm text-slate-700 dark:text-slate-300">
          These questions optimise cost estimation and timeline prediction. All
      ...
      <div className="p-4 rounded-lg border-2 border-green-500/50 bg-green-50 dark:bg-green-500/10 mb-6">
        <div className="flex items-center justify-between mb-2">
          <div>
            ...
            <p className="text-sm text-slate-700 dark:text-slate-300">
              These advanced questions optimise cost estimation and timeline
              prediction.
            </p>
          </div>
          <button
            ...
            onClick={() => {
              setResponses((prev) => ({
                ...
                T3_Q4_totalAffectedArea:
                  "Master Bedroom: 25 sqm, Ensuite: 8 sqm, Hallway: 12 sqm = 45 sqm total",
                T3_Q5_class4DryingAssessment:
          ...
          {responses.T3_Q1_timelineRequirements === "Other" && (
            <input
              ...
              onChange={(e) =>
                setResponses((prev) => ({
                  ...prev,
                  T3_Q1_timelineRequirementsOther: e.target.value,
                }))
              }
      ...
      <div className="p-6 rounded-lg border border-slate-200 dark:border-slate-700/50 bg-slate-50 dark:bg-slate-800/30">
        ...
        <p className="text-sm text-slate-600 dark:text-slate-400 mb-4">
          Example: 'Kitchen 15 sqm, Lounge 40 sqm, 2 Bedrooms 50 sqm total,
          Hallway 20 sqm = 125 sqm total.' Used for chemical treatment costing
          ($/sqm) and labour estimation.
        </p>
        <input
          type="text"
          ...
          onChange={(e) =>
            setResponses((prev) => ({
              ...prev,
              T3_Q4_totalAffectedArea: e.target.value,
            }))
          }
      ...
      <div className="p-6 rounded-lg border-2 border-green-500/50 bg-green-50 dark:bg-green-500/10">
        ...
        <p className="text-sm text-slate-700 dark:text-slate-300 mb-4">
          Categorise photos for proper placement in the report (35-45 images
          recommended)
        </p>
        ...
        <div className="space-y-4">
          {PHOTO_CATEGORIES.map((category) => {
            ...
            return (
              <div
                key={category.id}
                className="p-4 rounded-lg border border-slate-200 dark:border-slate-600/50 bg-white dark:bg-slate-900/30"
              >
                <div className="flex items-center justify-between mb-2">
                  <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-2">
                  {categoryPhotos.map((photo, index) => (
                    <div key={index} className="relative">
                      <img
                        ...
                        className="w-full h-20 object-cover rounded border-2 border-slate-300 dark:border-slate-600"
                      />
                      <button
                        type="button"
                        onClick={() => {
                            ...
                            photos.filter(
                              (p, i) =>
                                !(
                                  ...
                                  i === photos.findIndex((ph) => ph === photo)
                                ),
                            ),
                          );
                  ...
                  {remaining > 0 && (
                    <label className="w-full h-20 border-2 border-dashed border-slate-300 dark:border-slate-600 rounded flex items-center justify-center cursor-pointer hover:border-green-500 transition-colors bg-slate-50 dark:bg-slate-900/50">
                      <div className="text-center">
                        <Camera className="w-4 h-4 text-slate-500 dark:text-slate-400 mx-auto mb-1" />
                        <span className="text-xs text-slate-600 dark:text-slate-400">
                          Add
                        </span>
                      </div>
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (files.length > 0) {
                            ...
                            if (
                              categoryPhotos.length + files.length >
                              category.max
                            ) {
                              toast.error(
                                `Maximum ${category.max} photos allowed for ${category.label}`,
                              );
                              return;
                            }
                            setPhotos([...photos, ...newPhotos]);
                            toast.success(
                              `${files.length} photo(s) added to ${category.label}`,
                            );
                          }
                        }}
                        className="hidden"
      ...
      {/* Submit Button */}
      <div className="sticky bottom-0 z-10 flex justify-end py-4 bg-white dark:bg-slate-950 border-t border-neutral-200 dark:border-slate-800">
        <button
          onClick={handleSave}
          disabled={loading}
