...
export default function ScopeOfWorksViewer({
  reportId,
  onScopeGenerated,
}: ScopeOfWorksViewerProps) {
  ...
  const convertToVisualScopeData = (
    data: any,
    ...
    const phases = [
      ...
      },
      {
        ...
        duration: "Variable",
        activities: data.licensedTrades?.map((t: any) => t.trade) || [
          "None required",
        ],
    ...
    const formattedLineItems = (data.lineItems || []).map((item: any) => ({
      ...
      unit: item.unit || "",
      rate:
        item.rate || (item.subtotal && item.qty ? item.subtotal / item.qty : 0),
    ...
    const formattedLicensedTrades = (data.licensedTrades || []).map(
      (trade: any) => ({
    ...
    const insuranceBreakdown = {
      buildingClaim: [
        ...
        ...(data.licensedTrades || [])
          .filter((t: any) =>
            ["Plumbing", "Electrical", "Builder/Carpenter"].includes(
              t.trade || t.name,
            ),
          )
    ...
    if (
      data.licensedTrades?.some((t: any) => (t.trade || t.name) === "Plumbing")
    ) {
      coordinationNotes.push("Plumbing must be completed BEFORE drying begins");
    }
    if (
      data.licensedTrades?.some(
        (t: any) => (t.trade || t.name) === "Electrical",
      )
    ) {
      coordinationNotes.push(
        "Electrical clearance required BEFORE equipment activation",
      );
    }
    if (data.hasClass4Drying) {
      coordinationNotes.push(
        "Class 4 drying: Specialist assessment takes priority",
      );
    }
    if (
      data.licensedTrades?.some((t: any) =>
        (t.trade || t.name)?.includes("Mould"),
      )
    ) {
      coordinationNotes.push(
        "Mould remediation: Work stops immediately; restoration resumes post-clearance",
      );
    }
    if (
      data.licensedTrades?.some((t: any) =>
        (t.trade || t.name)?.includes("Asbestos"),
      )
    ) {
      coordinationNotes.push(
        "Asbestos abatement: All work suspended; WorkSafe clearance mandatory",
      );
    }
    coordinationNotes.push(
    ...
    return {
      header: {
        ...
        claimReference:
          data.claimReference || report?.claimReferenceNumber || "Reference",
      ...
      insuranceBreakdown,
      coordinationNotes,
      totalCost: formattedLineItems.reduce(
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400">
            {scopeDocument
              ? "View your generated scope of works document"
              : "Generate your comprehensive scope of works document"}
          </p>
        </div>
      ...
      {!scopeDocument && (
        <div className="p-6 rounded-lg border border-amber-500/50 bg-amber-500/10">
          ...
          <p className="text-slate-300 mb-4">
            Generate your comprehensive scope of works document with all
            remediation phases, restoration works line items, licensed trades
            requirements, and coordination notes.
          </p>
          <button
    ...
    background-image: none !important;
          ...
          >
            ...
            <div className="w-full p-0 px-4 space-y-8">
              {scopeData ? (
                <VisualScopeOfWorksViewer
                  data={convertToVisualScopeData(
                    scopeData,
                    businessInfo,
                    report,
                  )}
            ...
            <div className="p-4 rounded-lg border border-slate-700/50 bg-slate-800/30 print:hidden">
              ...
              <div className="grid md:grid-cols-3 gap-2 text-sm">
                ...
                <div>
                  ...
                  <span className="text-slate-300 ml-2">
                    {scopeData.dryingDuration || 7} days
                  </span>
                </div>
              </div>
            </div>
