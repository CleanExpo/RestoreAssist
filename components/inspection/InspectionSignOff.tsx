...
import { PenLine, CheckCircle, Loader2, AlertCircle, Shield } from 'lucide-react'
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(`/api/inspections/${inspectionId}/sign`, {
        ...
        body: JSON.stringify({ signatoryName: signatoryName.trim(), role: role.trim() }),
      })
    ...
    <div className="p-4 rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 space-y-4">
      ...
      <div className="p-3 rounded-lg bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/50">
        <p className="text-sm text-amber-700 dark:text-amber-400">
          By signing, you certify that inspection <strong>{inspectionNumber}</strong> is
      ...
      <div className="space-y-3">
        ...
        <label className="flex items-start gap-2.5 cursor-pointer">
          ...
          <span className="text-sm text-neutral-600 dark:text-neutral-400">
            I confirm I have authority to sign this inspection report on behalf of
            the company, and that all information is accurate and complete.
          </span>
        </label>
      </div>
