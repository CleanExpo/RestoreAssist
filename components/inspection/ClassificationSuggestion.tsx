...
import { ruleBasedClassify, type ClassificationResult, type ClaimType } from '@/lib/ai/auto-classify'
import { cn } from '@/lib/utils'
import { Lightbulb, X, Check } from 'lucide-react'

// All selectable claim types displayed as additional chips when multi-select is open
const ALL_CLAIM_TYPES: ClaimType[] = ['water_damage', 'fire_smoke', 'storm', 'mould', 'contents']
...
export default function ClassificationSuggestion({ description, averageMoistureReading, onApply }: Props) {
  ...
  const selectedTypes: ClaimType[] = [primaryType, ...Array.from(extraTypes).filter((t) => t !== primaryType)]
  ...
  if (selectedTypes.length === 1) {
    if (suggestion.damageCategory) summaryParts.push(`Category ${suggestion.damageCategory}`)
    if (suggestion.damageClass) summaryParts.push(`Class ${suggestion.damageClass}`)
  }
  ...
  return (
    ...
    >
      ...
      {multiSelectOpen && (
        <div className="flex flex-wrap gap-2 pt-1" role="group" aria-label="Select additional claim types">
