...
function toMaterial(surfaceType: string): string {
  ...
  if (s.includes("timber") || s.includes("hardwood") || s.includes("wood"))
    return "timber";
  if (s.includes("softwood") || s.includes("pine")) return "softwood";
  if (
    s.includes("plasterboard") ||
    s.includes("drywall") ||
    s.includes("gypsum")
  )
    return "plasterboard";
  if (s.includes("concrete") || s.includes("masonry") || s.includes("slab"))
    return "concrete";
  if (s.includes("carpet")) return "carpet";
  if (s.includes("vinyl") || s.includes("lvt") || s.includes("lvp"))
    return "vinyl";
  if (
    s.includes("particleboard") ||
    s.includes("mdf") ||
    s.includes("chipboard")
  )
...
function buildChartData(readings: DryingMoistureReading[], startMs: number) {
  ...
  for (const r of sorted) {
    const dayNum =
      Math.floor((new Date(r.recordedAt).getTime() - startMs) / 86_400_000) + 1;
  ...
  return {
    data: Object.values(dayMap).sort(
      (a, b) => (a.dayNum as number) - (b.dayNum as number),
    ),
...
function getLatestReadings(readings: DryingMoistureReading[]) {
  ...
  for (const r of readings) {
    ...
    if (
      !latest[key] ||
      new Date(r.recordedAt) > new Date(latest[key].recordedAt)
    ) {
      latest[key] = r;
    }
  }
...
function CustomTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 border border-neutral-200 dark:border-slate-700 rounded-xl p-3 shadow-lg text-xs space-y-1.5 min-w-[180px]">
      <p className="font-semibold text-neutral-700 dark:text-slate-200 mb-1">
        {label}
      </p>
        ...
        return (
          <div key={entry.dataKey} className="flex items-start gap-2 py-0.5">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-neutral-600 dark:text-slate-400 truncate">
                {location}
              </p>
...
export default function DryingProgressChart({
  ...
  const dryCount = series.filter((s) => {
    ...
    if (!r) return false;
    return (
      getMoistureStatus(r.moistureLevel, toMaterial(r.surfaceType)) === "dry"
    );
  }).length;
  const dryingCount = series.filter((s) => {
    ...
    if (!r) return false;
    return (
      getMoistureStatus(r.moistureLevel, toMaterial(r.surfaceType)) === "drying"
    );
  }).length;
  ...
  return (
    <div className={cn("space-y-4", className)}>
      ...
      <div className="flex flex-wrap gap-2">
        {dryCount > 0 && (
          <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-emerald-50 dark:bg-brand-bronze/10 text-emerald-700 dark:text-brand-navy border border-emerald-200 dark:border-brand-bronze/30">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left">
                <th className="px-4 py-2.5 text-xs font-medium text-neutral-500 dark:text-slate-500">
                  Location
                </th>
                <th className="px-4 py-2.5 text-xs font-medium text-neutral-500 dark:text-slate-500">
                  Material
                </th>
                <th className="px-4 py-2.5 text-xs font-medium text-neutral-500 dark:text-slate-500 text-right">
                  Reading
                </th>
                <th className="px-4 py-2.5 text-xs font-medium text-neutral-500 dark:text-slate-500 text-right">
                  Target
            ...
            <tbody className="divide-y divide-neutral-100 dark:divide-slate-700">
              {series.map((s) => {
                ...
                return (
                  <tr
                    key={s}
                    className="bg-white dark:bg-slate-800/20 hover:bg-neutral-50 dark:hover:bg-slate-800/40 transition-colors"
                  >
