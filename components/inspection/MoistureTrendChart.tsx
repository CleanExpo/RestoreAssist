...
type SeriesData = Record<
  string,
  { time: number; label: string; [location: string]: number | string }
>;
...
function buildChartData(readings: MoistureReading[]) {
  ...
  return {
    data: Object.values(byTime).sort(
...
function getAlerts(readings: MoistureReading[]) {
  ...
  for (const r of readings) {
    if (
      !latest[r.location] ||
      new Date(r.recordedAt) > new Date(latest[r.location].recordedAt)
    ) {
      latest[r.location] = r;
    }
  }
...
export default function MoistureTrendChart({
  ...
  for (const r of readings) {
    ...
    if (
      ...
      new Date(r.recordedAt) >
        new Date(
          readings.find(
            (x) => x.location === r.location && x.moistureLevel === existing,
          )?.recordedAt ?? "",
        )
    ) {
  ...
  const dryCount = Object.values(latestByLocation).filter(
    (v) => v <= DRY_STANDARD,
  ).length;
    ...
    <div className={cn("space-y-4", className)}>
      ...
      {alerts.length > 0 ? (
        ...
        <div className="flex items-center gap-2 p-3 rounded-lg bg-emerald-50 dark:bg-brand-bronze/10 border border-emerald-200 dark:border-brand-bronze/30 text-sm">
          <CheckCircle2
            size={16}
            className="text-brand-bronze dark:text-brand-navy"
          />
          <span className="text-emerald-700 dark:text-brand-navy font-medium">
              ...
              {Object.entries(latestByLocation).map(([loc, level]) => (
                <tr key={loc} className="bg-white dark:bg-slate-800/20">
                  ...
                  <td className="px-4 py-2.5 text-right">
                    {level <= DRY_STANDARD ? (
                      <span className="inline-flex items-center gap-1 text-xs text-brand-bronze dark:text-brand-navy bg-emerald-50 dark:bg-brand-bronze/10 px-2 py-0.5 rounded-full">
