...
import { Droplets, Loader2, X, CheckCircle2, AlertTriangle, Info } from "lucide-react"
...
export function MoistureReadingEntryForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!hasLevel) { setError("Moisture level is required"); return }
    if (level < 0 || level > 100) { setError("Moisture level must be between 0 and 100"); return }
    ...
    <form onSubmit={handleSubmit} className={cn("space-y-4", className)}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-gray-900 dark:text-white text-sm">Add Moisture Reading</h3>
        </div>
        {onCancel && (
          <button type="button" onClick={onCancel} className="p-1 hover:bg-gray-100 dark:hover:bg-slate-700 rounded text-gray-400">
            <X size={16} />
          </button>
        )}
      </div>
        ...
        <div>
          ...
          >
            {IICRC_DRY_STANDARDS.map(s => (
              <option key={s.material} value={s.material}>{s.label}</option>
            ))}
          </select>
        </div>
        <div>
        ...
        <div className="flex items-center gap-2">
          ...
          {status && statusColors && (
            <span className={cn(
              ...
              {status === "dry" ? <CheckCircle2 size={12} /> : <AlertTriangle size={12} />}
      ...
      <div className="flex items-start gap-2 p-3 bg-blue-50 dark:bg-blue-500/10 rounded-lg text-xs text-blue-700 dark:text-blue-400">
        ...
        <div>
          <span className="font-semibold">IICRC S500 dry standard for {std.label}: </span>
          ≤{std.dryThreshold}% = Dry · ≤{std.wetThreshold}% = Drying · &gt;{std.wetThreshold}% = Wet
          {std.notes && <span className="block text-blue-600 dark:text-blue-500 mt-0.5">{std.notes}</span>}
        </div>
      </div>
      ...
      <div className="flex gap-2 pt-1">
        ...
        >
          {submitting ? <Loader2 size={14} className="animate-spin" /> : <Droplets size={14} />}
