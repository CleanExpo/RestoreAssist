...
export default function MoistureMappingCanvas({
  ...
  const [equipmentPoints, setEquipmentPoints] = useState<EquipmentPoint[]>(
    initialEquipmentPoints,
  );
  const [selectedPoint, setSelectedPoint] = useState<MoisturePoint | null>(
    null,
  );
  const [selectedEquipment, setSelectedEquipment] =
    useState<EquipmentPoint | null>(null);
  ...
  const [placingReading, setPlacingReading] = useState<MoistureReading | null>(
    null,
  );
  ...
  // Equipment totals
  const equipmentCounts = EQUIPMENT_TYPES.reduce(
    (acc, eq) => {
      acc[eq.type] = equipmentPoints.filter(
        (p) => p.equipmentType === eq.type,
      ).length;
      return acc;
    },
  ...
  const MODE_TABS: { mode: SketchMode; label: string; description: string }[] =
    [
      ...
      },
      {
        mode: "moisture",
        label: "Moisture Map",
        description: "Place moisture readings",
      },
      {
        mode: "equipment",
        label: "Equipment",
        description: "Place drying equipment",
      },
    ];
    ...
    <div className={cn("space-y-3", className)}>
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 bg-brand-navy text-white rounded-lg text-sm hover:bg-brand-navy disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        >
          <Download size={14} />
          Export PNG
        </button>
      ...
      {sketchMode === "moisture" && (
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 px-3 py-2 rounded-lg bg-neutral-50 dark:bg-slate-800/50 border border-neutral-200 dark:border-slate-700/50 text-xs">
          ...
          <span className="text-neutral-400 dark:text-slate-500 ml-auto italic">
            Thresholds vary by material type
          </span>
        </div>
      )}
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 px-3 py-2 rounded-lg bg-neutral-50 dark:bg-slate-800/50 border border-neutral-200 dark:border-slate-700/50 text-xs">
          ...
          {EQUIPMENT_TYPES.map((eq) => (
            <span key={eq.type} className="flex items-center gap-1.5">
              ...
              <span className="text-neutral-400 dark:text-slate-400">
                {eq.iicrcRatio}
              </span>
            </span>
          ))}
      ...
      <div className="flex gap-4">
        ...
        >
          <svg
            ...
            className={cn(
              "w-full h-auto",
              isCrosshair ? "cursor-crosshair" : "cursor-default",
            )}
            ...
            <>
              {selectedPoint && (
                <div className="p-3 rounded-xl border-2 border-brand-navy bg-cyan-50/50 dark:bg-cyan-900/10 space-y-2">
                  <div className="text-xs font-semibold text-brand-tan uppercase">
                    Selected Reading
                  </div>
                    ...
                    <span
                      ...
                      style={{
                        backgroundColor: getMoistureColor(
                          ...
                          selectedPoint.reading.surfaceType,
                        ),
                      }}
                    >
                      {selectedPoint.reading.moistureLevel}%
                  ...
                  <div className="text-xs text-neutral-500">
                    ...
                    {selectedPoint.reading.notes && (
                      <p>Notes: {selectedPoint.reading.notes}</p>
                    )}
                  </div>
                </div>
              )}
                    ...
                    {unplacedReadings.map((reading) => (
                      <button
                        ...
                        className={cn(
                          ...
                          placingReading?.id === reading.id
                            ? "border-brand-navy bg-cyan-50 dark:bg-cyan-900/20 ring-2 ring-brand-navy/30"
                      ...
                      >
                        ...
                        <div className="text-xs text-neutral-400 mt-0.5">
                          {reading.surfaceType} / {reading.depth}
                        </div>
                      </button>
                    ))}
                  </div>
              ...
              {unplacedReadings.length === 0 && points.length > 0 && (
                <div className="p-3 rounded-xl bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-200 dark:border-emerald-800/50 text-center">
                  <Droplets
                    ...
                    size={20}
                  />
                  <p className="text-xs text-brand-bronze dark:text-brand-bronze font-medium">
                    All readings placed
                  </p>
                </div>
              )}
            ...
            <>
                ...
                <div className="space-y-1.5">
                  {EQUIPMENT_TYPES.map((eq) => (
                    <button
                      ...
                      onClick={() =>
                        setPlacingEquipment(
                          placingEquipment === eq.type ? null : eq.type,
                        )
                      }
                      disabled={readonly}
                      className={cn(
                    ...
                    >
                      ...
                      <div className="text-xs text-neutral-400 mt-0.5 ml-8">
                        {eq.iicrcRatio}
                      </div>
                    </button>
                  ))}
              ...
              {selectedEquipment && (
                <div
                  ...
                  style={{
                    borderColor: getEquipmentConfig(
                ...
                >
                  ...
                  <div className="text-sm font-medium">
                    {getEquipmentConfig(selectedEquipment.equipmentType).label}
                  </div>
                  <div className="text-xs text-neutral-500">
                    Position: ({Math.round(selectedEquipment.x)},{" "}
                    {Math.round(selectedEquipment.y)})
                  </div>
                </div>
              )}
                ...
                <div className="p-3 rounded-xl bg-neutral-50 dark:bg-slate-800/50 border border-neutral-200 dark:border-slate-700/50">
                  ...
                  <div className="space-y-1">
                    {EQUIPMENT_TYPES.filter(
                      (eq) => (equipmentCounts[eq.type] ?? 0) > 0,
                    ).map((eq) => (
                      <div
                        key={eq.type}
