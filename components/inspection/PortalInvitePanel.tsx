...
export default function PortalInvitePanel({ inspectionId, preselectedClientId }: PortalInvitePanelProps) {
  const [clients, setClients] = useState<Client[]>([])
  const [selectedClientId, setSelectedClientId] = useState(preselectedClientId ?? '')
  const [message, setMessage] = useState('')
  const [loading, setLoading] = useState(true)
  const [sending, setSending] = useState(false)
  const [inviteStatus, setInviteStatus] = useState<'idle' | 'sent' | 'existing_access' | 'pending'>('idle')
  ...
  const handleSendInvite = async () => {
    ...
    try {
      const res = await fetch('/api/portal/invitations', {
        ...
        body: JSON.stringify({ clientId: selectedClientId, message: message || undefined }),
      })
  ...
  return (
    <div className="p-4 rounded-xl border border-neutral-200 dark:border-slate-700/50 bg-white dark:bg-slate-900/50">
      ...
      <p className="text-xs text-neutral-500 dark:text-slate-400 mb-4">
        Give your client read-only access to track their restoration job progress.
      </p>
      ...
      {inviteStatus === 'idle' && (
        <div className="space-y-3">
          ...
          <div>
            <label className="text-xs text-neutral-500 mb-1 block">Personal message (optional)</label>
            <input
              type="text"
              value={message}
