...
import { getSyncStatus, getQueueStats, initSyncOnReconnect, type SyncStatus, type QueueStats } from '@/lib/nir-sync-queue'
...
export function NirOfflineProvider({ children }: NirOfflineProviderProps) {
  ...
  const refreshStatus = useCallback(async () => {
    const [status, stats] = await Promise.all([getSyncStatus(), getQueueStats()])
  ...
  return (
    <NirOfflineContext.Provider value={{ syncStatus, queueStats, isServiceWorkerReady, triggerSync }}>
      {children}
