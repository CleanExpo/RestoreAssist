...
export default function DetailedReportViewer({
  ...
  if (signatureMatch) {
    ...
    const signatureText = signatureMatch[0]
      ...
      .map((l) => l.trim())
      .filter(
        (l) => l && !l.match(/^(Technician Name|Date|Position|Company):/i),
      )
  ...
  const handleDownloadEnhancedPDF = async () => {
    try {
      ...
      const response = await fetch(
        `/api/reports/${reportId}/download-enhanced`,
      );
  ...
  const handleGenerateReport = async () => {
    setGenerating(true);
    try {
      toast.loading("Regenerating enhanced report...", {
      ...
      if (data.report?.detailedReport) {
        ...
        router.refresh();
        toast.success(
          "Enhanced report regenerated successfully! Refreshing...",
          { id: "regenerate-enhanced" },
        );
      } else {
        toast.error("Failed to regenerate enhanced report", {
      ...
      <div className="bg-slate-800/50 rounded-lg border border-slate-700 p-6">
        ...
        <div className="text-center py-8">
          ...
          <p className="text-sm text-slate-500 mb-4">
            The AI-powered detailed report generation may have failed or is
            still processing.
          </p>
          <button
            onClick={handleGenerateReport}
            disabled={generating}
            className="flex items-center gap-2 px-4 py-2 bg-brand-bronze hover:bg-cyan-700 text-white rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed mx-auto"
          >
            <RefreshCw
    ...
    <div className="flex flex-col h-[calc(100vh-8rem)] bg-slate-900 rounded-lg border border-slate-700/50 shadow-xl overflow-hidden">
      ...
      <div className="flex items-center justify-between px-8 py-6 border-b border-slate-700/50 bg-gradient-to-r from-slate-800/50 to-slate-800/30 backdrop-blur-sm">
        <div className="flex items-center gap-4">
          <div className="p-3 bg-gradient-to-br from-brand-bronze/20 to-brand-bronze/20 rounded-lg border border-brand-bronze/30">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 px-5 py-2.5 text-sm font-semibold bg-gradient-to-r from-brand-bronze to-brand-bronze hover:from-cyan-700 hover:to-brand-bronze text-white rounded-lg transition-all duration-200 shadow-lg shadow-brand-bronze/20 hover:shadow-brand-bronze/30 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {downloading ? (
              <>
          ...
          <div className="max-w-6xl mx-auto">
            <div className="prose prose-invert prose-lg max-w-none">
              <ReactMarkdown
                components={{
                  h1: ({ children }) => (
                    <h1 className="text-3xl font-bold text-white mb-6 mt-8 first:mt-0 border-b border-slate-700/50 pb-3">
                      {children}
                    </h1>
                  ),
                  h2: ({ children }) => {
                    const isSignature =
                      ...
                      children.toLowerCase().includes("signature");
                    if (isSignature) {
                      return null; // We'll handle signature separately
                    }
                    return (
                      <h2 className="text-2xl font-bold text-brand-navy mb-4 mt-8 border-l-4 border-brand-bronze pl-4">
                        {children}
                      </h2>
                    );
                  },
                  h3: ({ children }) => (
                    <h3 className="text-xl font-bold text-brand-navy mb-3 mt-6">
                      {children}
                    </h3>
                  ),
                  h4: ({ children }) => (
                    <h4 className="text-lg font-bold text-brand-navy mb-2 mt-4">
                      {children}
                    </h4>
                  ),
                  p: ({ children, ..._props }) => {
                        ...
                        return children
                          .map((c) => {
                            ...
                            if (typeof c === "object" && c?.props?.children)
                              return getText(c.props.children);
                            return "";
                          })
                          .join("");
                      }
                    ...
                    if (lines.length >= 2) {
                      ...
                      if (
                        ...
                        if (content && content.length > 0) {
                          return (
                            <div className="mb-5">
                              <div className="font-bold text-brand-navy text-base mb-2 tracking-wide">
                                {firstLine}
                              </div>
                              <div className="text-slate-300 leading-7 text-base pl-4 border-l-2 border-brand-bronze/30 whitespace-pre-line">
                                {content}
                              </div>
                            </div>
                          );
                    ...
                    if (
                      ...
                      return (
                        <div className="mb-5">
                          ...
                          <div className="text-slate-300 leading-7 text-base pl-4 border-l-2 border-brand-bronze/30">
                            {content.trim()}
                          </div>
                        </div>
                      );
                    ...
                    return (
                      <p className="text-slate-300 mb-4 leading-7 text-base">
                        {children}
                      </p>
                    );
                  },
                  ul: ({ children }) => (
                    <ul className="text-slate-300 mb-4 ml-6 space-y-2 list-disc">
                      {children}
                    </ul>
                  ),
                  ol: ({ children }) => (
                    <ol className="text-slate-300 mb-4 ml-6 space-y-2 list-decimal">
                      {children}
                    </ol>
                  ),
                  li: ({ children }) => (
                    <li className="mb-1 leading-6">{children}</li>
                  ),
                  strong: ({ children }) => (
                    <strong className="text-white font-bold">{children}</strong>
                  ),
                  em: ({ children }) => (
                    <em className="text-brand-navy italic">{children}</em>
                  ),
                  code: ({ children }) => (
                    <code className="bg-slate-800/50 text-brand-navy px-2 py-1 rounded text-sm font-mono border border-slate-700/50">
                      {children}
                    </code>
                  ),
                  pre: ({ children }) => (
                    <pre className="bg-slate-800/50 text-slate-300 p-4 rounded-lg mb-4 overflow-x-auto border border-slate-700/50">
                      {children}
                    </pre>
                  ),
                  blockquote: ({ children }) => (
                    <blockquote className="border-l-4 border-brand-bronze pl-4 text-slate-400 italic mb-4 bg-slate-800/30 py-2 rounded-r">
                      {children}
                    </blockquote>
                  ),
                  table: ({ children }) => (
                  ...
                  th: ({ children }) => (
                    <th className="border border-slate-700/50 px-4 py-2 bg-slate-800/50 text-left text-brand-navy font-semibold">
                      {children}
                    </th>
                  ),
                  td: ({ children }) => (
                    <td className="border border-slate-700/50 px-4 py-2 text-slate-300">
                      {children}
                    </td>
                  ),
                }}
