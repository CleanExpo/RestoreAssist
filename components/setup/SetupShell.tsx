...
function jobStatusToHydrationState(
  status: string,
): "pending" | "running" | "ready" | "error" | "manual" {
  const s = status.toLowerCase();
  if (s === "ready" || s === "error" || s === "manual" || s === "running")
    return s as "ready" | "error" | "manual" | "running";
  return "pending";
}
...
export function SetupShell({ initial }: { initial: InitialPayload }) {
  ...
  useEffect(() => {
    ...
    for (const job of hydrationJobs) {
      setSectionStatus(
        jobKindToSectionKey(job.kind),
        jobStatusToHydrationState(job.status),
      );
    }
          ...
          fetch("/api/setup/state")
            ...
            .then((data) => {
              if (data?.data?.organization) {
                ...
                setOrg({
                  ...orgOnly,
                  setupStartedAt: orgOnly.setupStartedAt
                    ? new Date(orgOnly.setupStartedAt).toISOString()
                    : null,
                  setupCompletedAt: orgOnly.setupCompletedAt
                    ? new Date(orgOnly.setupCompletedAt).toISOString()
                    : null,
                });
              }
            })
