...
export default function RestorationInspectionReportViewer({
  ...
  const generateObservations = () => {
    ...
    if (data.summary.roomsAffected > 0) {
      observations.push(
        `${data.summary.roomsAffected} room(s) affected by water damage`,
      );
    }
    ...
    if (data.summary.averageMoisture) {
      observations.push(
        `Average moisture reading: ${data.summary.averageMoisture.toFixed(1)}%`,
      );
    }

    if (data.hazards.biologicalMouldDetected) {
      observations.push(
        `Biological mould detected${data.hazards.biologicalMouldCategory ? ` (${data.hazards.biologicalMouldCategory})` : ""}`,
      );
    }
    ...
    if (data.summary.estimatedDuration) {
      observations.push(
        `Estimated restoration duration: ${data.summary.estimatedDuration} days`,
      );
    }
  ...
  const generateProfessionalSummary = () => {
    ...
    if (data.incident.dateOfLoss) {
      summaryParts.push(
        `The water damage incident occurred on ${formatDateLong(data.incident.dateOfLoss)}.`,
      );
    }

    if (data.incident.waterSource) {
      summaryParts.push(
        `The source of water ingress has been identified as ${data.incident.waterSource.toLowerCase()}.`,
      );
    }
    ...
    if (data.incident.waterCategory && data.incident.waterClass) {
      summaryParts.push(
        `In accordance with IICRC S500 classification standards, the water has been categorised as ${data.incident.waterCategory} and classified as ${data.incident.waterClass}.`,
      );
    }
    ...
    if (data.summary.roomsAffected > 0) {
      summaryParts.push(
        `The inspection has identified ${data.summary.roomsAffected} room${data.summary.roomsAffected > 1 ? "s" : ""} affected by water damage.`,
      );
    }
    ...
    if (data.summary.averageMoisture !== null) {
      ...
      if (moistureLevel > 20) {
        summaryParts.push(
          `Moisture readings indicate significant saturation with an average moisture level of ${moistureLevel.toFixed(1)}%, requiring immediate remediation in accordance with IICRC S500 protocols.`,
        );
      } else if (moistureLevel > 15) {
        summaryParts.push(
          `Moisture assessment reveals elevated moisture levels averaging ${moistureLevel.toFixed(1)}%, necessitating structured drying procedures per IICRC S500 guidelines.`,
        );
      } else {
        summaryParts.push(
          `Moisture readings show average levels of ${moistureLevel.toFixed(1)}%, indicating moderate water impact requiring monitoring and controlled drying.`,
        );
      }
    }
    ...
    if (data.psychrometric?.dryingStatus) {
      summaryParts.push(
        `Psychrometric analysis indicates a ${data.psychrometric.dryingStatus.toLowerCase()} drying condition.`,
      );
    }
    ...
    if (data.hazards.biologicalMouldDetected) {
      summaryParts.push(
        `Biological mould growth has been detected${data.hazards.biologicalMouldCategory ? `, categorised as ${data.hazards.biologicalMouldCategory}` : ""}, requiring compliance with IICRC S520 (Mould Remediation) standards and appropriate containment protocols.`,
      );
    }
    ...
    if (data.equipment && data.equipment.length > 0) {
      const totalEquipment = data.equipment.reduce(
        (sum, item) => sum + item.quantity,
        0,
      );
      summaryParts.push(
        `Remediation equipment has been deployed, including ${totalEquipment} unit${totalEquipment > 1 ? "s" : ""} of specialised drying equipment, in compliance with IICRC S500 equipment placement and operational guidelines.`,
      );
    }
    ...
    if (data.summary.estimatedDuration) {
      summaryParts.push(
        `The estimated restoration duration is ${data.summary.estimatedDuration} day${data.summary.estimatedDuration > 1 ? "s" : ""}, subject to ongoing monitoring and verification per IICRC S500 completion criteria.`,
      );
    }
    ...
    // Closing
    summaryParts.push(
      `Detailed findings, recommendations, and compliance documentation are provided in the following sections of this report.`,
    );
  ...
  return (
    <>
      ...
      <div
        ...
        className="bg-white text-slate-900 max-w-5xl mx-auto p-8 print:p-0"
      >
        ...
        <div className="min-h-[297mm] print:min-h-0 flex flex-col print-page">
          ...
          <div className="mb-8 print:mb-6">
            ...
            <table className="w-full border-collapse">
              <tbody>
                <tr>
                  <td className="py-2 print:py-1.5 pr-4 print:pr-3 font-bold text-slate-900 w-1/3 text-sm print:text-xs border-b border-slate-200">
                    CLIENT NAME:
                  </td>
                  <td className="py-2 print:py-1.5 text-slate-700 text-sm print:text-xs border-b border-slate-200">
                    {data.property.clientName ||
                      data.property.clientCompany ||
                      "Not provided"}
                  </td>
                </tr>
                ...
                <tr>
                  <td className="py-2 print:py-1.5 pr-4 print:pr-3 font-bold text-slate-900 w-1/3 text-sm print:text-xs">
                    SITE ADDRESS:
                  </td>
        ...
        <div className="print-page-break print-page min-h-[297mm] print:min-h-0 flex flex-col">
          ...
          <div className="mb-6 print:mb-4">
            <h3 className="text-lg print:text-base font-semibold text-slate-900 mb-3 print:mb-2">
              Inspector Information
            </h3>
            <table className="w-full border-collapse border border-slate-300">
              <tbody>
                <tr>
                  <td className="p-3 print:p-2 border border-slate-300 bg-slate-50 font-semibold text-sm print:text-xs w-1/2">
                    Inspector Name
                  </td>
                  <td className="p-3 print:p-2 border border-slate-300 text-sm print:text-xs">
                    {data.incident.technicianName || "Not provided"}
                  </td>
                </tr>
                <tr>
                  <td className="p-3 print:p-2 border border-slate-300 bg-slate-50 font-semibold text-sm print:text-xs">
                    Inspector ID
                  </td>
                  <td className="p-3 print:p-2 border border-slate-300 text-sm print:text-xs">
                    {data.header.reportNumber || "N/A"}
                  </td>
                </tr>
              </tbody>
            </table>
          ...
          <div className="mb-6 print:mb-4">
            ...
            <div className="space-y-3 print:space-y-2">
              <div>
                <label className="block text-sm print:text-xs font-semibold text-slate-700 mb-1">
                  Address
                </label>
                ...
                <div className="flex gap-4 print:gap-2 text-sm print:text-xs">
                  <label className="flex items-center gap-1">
                    <input
                      type="checkbox"
                      className="print:hidden"
                      defaultChecked={data.property.structureType
                        ?.toLowerCase()
                        .includes("tough")}
                    />
              ...
              <div className="space-y-3 print:space-y-2">
                <div>
                  ...
                  <div className="border border-slate-300 rounded p-2 print:p-1.5 bg-white text-sm print:text-xs inline-block">
                    {data.maintenanceHistory.lastInspectionDate
                      ? formatDate(data.maintenanceHistory.lastInspectionDate)
                      : "Not provided"}
                  </div>
                </div>
                ...
                <div className="space-y-2 print:space-y-1">
                  <div className="flex items-center gap-2 print:gap-1">
                    <span className="text-sm print:text-xs font-semibold text-slate-700">
                  ...
                  <div className="flex items-center gap-2 print:gap-1">
                    <span className="text-sm print:text-xs font-semibold text-slate-700">
                      Attach copy of guarantee or previous works.
                    </span>
                    <span className="text-xs print:text-[10px] text-slate-500 italic">
                      (See attachments)
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-2 print:gap-1">
                    <span className="text-sm print:text-xs font-semibold text-slate-700">
                      Has any scope of work to the building changed since last
                      inspection?
                    </span>
                    <div className="flex gap-3 print:gap-2 text-sm print:text-xs">
                      <label className="flex items-center gap-1">
                        <input
                          type="radio"
                          name="building_changed"
                          className="print:hidden"
                          defaultChecked={data.maintenanceHistory.buildingChangedSinceLastInspection
                            ?.toLowerCase()
                            .includes("yes")}
                        />
                        <span>Yes</span>
                  ...
                  <div className="flex items-center gap-2 print:gap-1">
                    <span className="text-sm print:text-xs font-semibold text-slate-700">
                      Has any changes, additions or new penetrations been made
                      to roof since last inspection?
                    </span>
                    <div className="flex gap-3 print:gap-2 text-sm print:text-xs">
                      <label className="flex items-center gap-1">
                        <input
                          ...
                          className="print:hidden"
                          defaultChecked={data.maintenanceHistory.structureChangesSinceLastInspection
                            ?.toLowerCase()
                            .includes("yes")}
                        />
                        <span>Yes</span>
                  ...
                  <div className="flex items-center gap-2 print:gap-1">
                    <span className="text-sm print:text-xs font-semibold text-slate-700">
                      Was there previous leakage?
                    </span>
                    <div className="flex gap-3 print:gap-2 text-sm print:text-xs">
                      <label className="flex items-center gap-1">
                        <input
                          type="radio"
                          name="previous_leakage"
                          className="print:hidden"
                          defaultChecked={data.maintenanceHistory.previousLeakage
                            ?.toLowerCase()
                            .includes("yes")}
                        />
                        <span>Yes</span>
                    ...
                    <div className="flex gap-3 print:gap-2 text-sm print:text-xs">
                      <label className="flex items-center gap-1">
                        <input
                          ...
                          className="print:hidden"
                          defaultChecked={data.maintenanceHistory.emergencyRepairPerformed
                            ?.toLowerCase()
                            .includes("yes")}
                        />
                        <span>Yes</span>
        ...
        <div className="print-page-break print-page flex flex-col">
          ...
          <div className="mb-6 print:mb-4">
            <h3 className="text-lg print:text-base font-semibold text-slate-900 mb-3 print:mb-2 flex items-center gap-2">
              <Building2 className="w-5 h-5 print:w-4 print:h-4 text-brand-bronze" />
              Property Information
            </h3>
            <div className="space-y-3 print:space-y-2">
              {data.property.propertyId && (
          ...
          <div className="mb-6 print:mb-4">
            <h3 className="text-lg print:text-base font-semibold text-slate-900 mb-3 print:mb-2 flex items-center gap-2">
              <Calendar className="w-5 h-5 print:w-4 print:h-4 text-brand-bronze" />
              Incident Details
            </h3>
            <div className="space-y-3 print:space-y-2">
              <div className="grid grid-cols-2 gap-3 print:gap-2">
          ...
          {data.tier1 &&
            (data.reportDepthLevel === "Enhanced" ||
              data.reportDepthLevel === "Optimised") && (
              <div className="mb-6 print:mb-4">
                <h3 className="text-lg print:text-base font-semibold text-slate-900 mb-3 print:mb-2 flex items-center gap-2">
                  <CheckCircle className="w-5 h-5 print:w-4 print:h-4 text-brand-bronze" />
                ...
                <div className="space-y-3 print:space-y-2">
                      ...
                      <div className="border border-slate-300 rounded p-2 print:p-1.5 bg-white text-sm print:text-xs">
                        ...
                        {data.tier1.T1_Q4_petsPresent && (
                          <span className="text-slate-600 ml-2">
                            (Pets: {data.tier1.T1_Q4_petsPresent})
                          </span>
                        )}
                      </div>
                    </div>
                  ...
                  {data.tier1.T1_Q6_materialsAffected &&
                    Array.isArray(data.tier1.T1_Q6_materialsAffected) &&
                    data.tier1.T1_Q6_materialsAffected.length > 0 && (
                      <div>
                        <label className="block text-sm print:text-xs font-semibold text-slate-700 mb-1">
                          Materials Affected
                        </label>
                        <div className="border border-slate-300 rounded p-2 print:p-1.5 bg-white text-sm print:text-xs">
                          <ul className="list-disc list-inside space-y-1">
                            {data.tier1.T1_Q6_materialsAffected.map(
                  ...
                  {data.tier1.T1_Q7_hazards &&
                    Array.isArray(data.tier1.T1_Q7_hazards) &&
                    data.tier1.T1_Q7_hazards.length > 0 && (
                      <div>
                        <label className="block text-sm print:text-xs font-semibold text-slate-700 mb-1">
                          Hazards Identified
                        </label>
            ...
            (data.reportDepthLevel === "Enhanced" ||
              ...
              <div className="mb-6 print:mb-4">
                ...
                <div className="space-y-3 print:space-y-2">
                  ...
                  {data.tier2.T2_Q5_structuralConcerns &&
                    Array.isArray(data.tier2.T2_Q5_structuralConcerns) &&
                    data.tier2.T2_Q5_structuralConcerns.length > 0 && (
                      <div>
                        <label className="block text-sm print:text-xs font-semibold text-slate-700 mb-1">
                          Structural Concerns
                        </label>
                        <div className="border border-slate-300 rounded p-2 print:p-1.5 bg-white text-sm print:text-xs">
                          <ul className="list-disc list-inside space-y-1">
                            {data.tier2.T2_Q5_structuralConcerns.map(
                  ...
                  {data.tier2.T2_Q6_buildingServicesAffected &&
                    Array.isArray(data.tier2.T2_Q6_buildingServicesAffected) &&
                    data.tier2.T2_Q6_buildingServicesAffected.length > 0 && (
                      <div>
                        <label className="block text-sm print:text-xs font-semibold text-slate-700 mb-1">
                        ...
                        <div className="border border-slate-300 rounded p-2 print:p-1.5 bg-white text-sm print:text-xs">
                          <ul className="list-disc list-inside space-y-1">
                            {data.tier2.T2_Q6_buildingServicesAffected.map(
        ...
        <div className="print-page-break print-page flex flex-col">
          ...
          {(data.environmental ||
            ...
            <section className="print-avoid-break mb-6 print:mb-4">
              <h2 className="text-2xl print:text-xl font-bold text-slate-900 mb-4 print:mb-3 flex items-center gap-2">
                <Thermometer className="w-6 h-6 print:w-5 print:h-5 text-brand-bronze" />
                Environmental Conditions & Classification
              </h2>
                  ...
                  <div className="grid grid-cols-3 gap-4 print:gap-3">
                    {data.environmental.ambientTemperature !== null && (
                      <div className="bg-white rounded-lg p-4 print:p-3 text-center border border-blue-100">
                        ...
                        <p className="text-xs print:text-[10px] text-slate-600 mb-1 font-medium">
                          Temperature
                        </p>
                        <p className="text-2xl print:text-xl font-bold text-slate-900">
                          {data.environmental.ambientTemperature}°C
                        </p>
                      </div>
                    )}
                    {data.environmental.humidityLevel !== null && (
                      <div className="bg-white rounded-lg p-4 print:p-3 text-center border border-blue-100">
                        ...
                        <p className="text-2xl print:text-xl font-bold text-slate-900">
                          {data.environmental.humidityLevel}%
                        </p>
                      </div>
                    )}
                ...
                <div className="bg-gradient-to-br from-amber-50 to-yellow-50 rounded-lg p-6 print:p-4 border-2 border-amber-200 mb-4 print:mb-3">
                  ...
                  <div className="grid md:grid-cols-2 gap-4 print:gap-3">
                    {data.classification.category && (
                      <div>
                        <p className="text-sm print:text-xs text-slate-600 mb-1 font-semibold">
                          Category
                        </p>
                    ...
                    {data.classification.justification && (
                      <div className="md:col-span-2">
                        ...
                        <p className="text-slate-900 text-sm print:text-xs">
                          {data.classification.justification}
                        </p>
                      </div>
                    )}
                    {data.classification.standardReference && (
                      <div className="md:col-span-2">
                        ...
                        <p className="text-slate-900 text-sm print:text-xs font-mono">
                          {data.classification.standardReference}
                        </p>
                      </div>
                    )}
              ...
              {data.psychrometric && (
                <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-lg p-6 print:p-4 border-2 border-green-200">
                  <h3 className="text-lg print:text-base font-bold text-slate-900 mb-4 print:mb-3 flex items-center gap-2">
                    <Wind className="w-5 h-5 print:w-4 print:h-4 text-brand-bronze" />
                    Psychrometric Assessment
                  </h3>
                  <div className="grid md:grid-cols-2 gap-4 print:gap-3">
                    {data.psychrometric.dryingIndex !== null && (
                      <div>
                        ...
                        <p className="font-semibold text-slate-900 text-2xl print:text-xl">
                          {data.psychrometric.dryingIndex}
                        </p>
                      </div>
                    )}
          ...
          {data.affectedAreas && data.affectedAreas.length > 0 && (
            <section className="print-avoid-break mb-6 print:mb-4">
              <h2 className="text-2xl print:text-xl font-bold text-slate-900 mb-4 print:mb-3 flex items-center gap-2">
                <Droplet className="w-6 h-6 print:w-5 print:h-5 text-brand-bronze" />
                Affected Areas
              </h2>
              <div className="space-y-4 print:space-y-3">
                {data.affectedAreas.map((area, index) => (
                  <div
                    key={index}
                    className="bg-slate-50 rounded-lg p-6 print:p-4 border border-slate-200"
                  >
                    ...
                    {area.moistureReadings &&
                      area.moistureReadings.length > 0 && (
                        <div>
                          <p className="text-sm print:text-xs text-slate-600 mb-2 font-semibold">
                            Moisture Readings:
                          </p>
                          <div className="grid grid-cols-2 gap-2 print:gap-1">
                            {area.moistureReadings.map((reading, rIndex) => (
                              <div
                                key={rIndex}
                                className="bg-white rounded p-2 print:p-1.5 border border-slate-200"
                              >
                                <p className="text-xs print:text-[10px] text-slate-600">
                                  {reading.location}
                                </p>
                                <p className="text-sm print:text-xs font-semibold text-slate-900">
          ...
          {data.moistureReadings && data.moistureReadings.length > 0 && (
            <section className="print-avoid-break mb-6 print:mb-4">
              <h2 className="text-2xl print:text-xl font-bold text-slate-900 mb-4 print:mb-3 flex items-center gap-2">
                <FileText className="w-6 h-6 print:w-5 print:h-5 text-brand-bronze" />
                Detailed Moisture Readings
              </h2>
                ...
                <table className="w-full border-collapse border border-slate-300">
                  <thead>
                    <tr className="bg-slate-100">
                      <th className="p-3 print:p-2 border border-slate-300 text-left text-sm print:text-xs font-semibold text-slate-900">
                        Location
                      </th>
                      <th className="p-3 print:p-2 border border-slate-300 text-left text-sm print:text-xs font-semibold text-slate-900">
                        Surface Type
                      ...
                      <div>
                        ...
                        {data.hazards.biologicalMouldCategory && (
                          <p className="text-sm print:text-xs text-slate-700">
                            Category: {data.hazards.biologicalMouldCategory}
                          </p>
                        )}
                        <p className="text-sm print:text-xs text-slate-600 mt-1">
                          <strong>Compliance:</strong> IICRC S520 (Mould
                          Remediation) standards apply. Appropriate containment
                          and remediation protocols required.
                        </p>
                      </div>
                    </div>
                  )}
          ...
          {data.equipment && data.equipment.length > 0 && (
            <section className="print-avoid-break mb-6 print:mb-4">
              <h2 className="text-2xl print:text-xl font-bold text-slate-900 mb-4 print:mb-3 flex items-center gap-2">
                <Wrench className="w-6 h-6 print:w-5 print:h-5 text-brand-bronze" />
                Equipment Deployment
              </h2>
              <div className="overflow-x-auto">
                <table className="w-full border-collapse border border-slate-300">
                  <thead>
                    <tr className="bg-slate-100">
                      ...
                      <th className="p-3 print:p-2 border border-slate-300 text-center text-sm print:text-xs font-semibold text-slate-900">
                        Quantity
                      </th>
                      <th className="p-3 print:p-2 border border-slate-300 text-center text-sm print:text-xs font-semibold text-slate-900">
                        Daily Rate
                      ...
                      <th className="p-3 print:p-2 border border-slate-300 text-right text-sm print:text-xs font-semibold text-slate-900">
                        Total Cost
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.equipment.map((item, index) => (
                      <tr key={index}>
                        ...
                        <td className="p-3 print:p-2 border border-slate-300 text-center text-sm print:text-xs">
                          {item.quantity}
                        </td>
                        <td className="p-3 print:p-2 border border-slate-300 text-center text-sm print:text-xs">
                        ...
                        <td className="p-3 print:p-2 border border-slate-300 text-right text-sm print:text-xs font-semibold">
                    ...
                    <tr className="bg-slate-100 font-semibold">
                      <td
                        colSpan={4}
                        className="p-3 print:p-2 border border-slate-300 text-right text-sm print:text-xs"
                      >
          ...
          <section className="print-avoid-break mb-6 print:mb-4">
            <h2 className="text-2xl print:text-xl font-bold text-slate-900 mb-4 print:mb-3 flex items-center gap-2">
              <ClipboardCheck className="w-6 h-6 print:w-5 print:h-5 text-brand-bronze" />
              Compliance & Standards
            </h2>
              ...
              <div className="space-y-2 print:space-y-1">
                {getComplianceStandards().map((standard, index) => (
                  <div key={index} className="flex items-start gap-2">
                    <CheckCircle className="w-4 h-4 print:w-3 print:h-3 text-brand-bronze mt-0.5 flex-shrink-0" />
          ...
          {data.photos && data.photos.length > 0 && (
            <section className="print-avoid-break mb-6 print:mb-4">
              <h2 className="text-2xl print:text-xl font-bold text-slate-900 mb-4 print:mb-3 flex items-center gap-2">
                <ImageIcon className="w-6 h-6 print:w-5 print:h-5 text-brand-bronze" />
                        ...
                        <div key={categoryId} className="print-avoid-break">
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 print:gap-3">
                            {categoryPhotos.map((photo, index) => (
                              <div
                                key={index}
                                className="relative bg-slate-100 rounded-lg overflow-hidden border border-slate-300"
                              >
                                <img
                                  src={photo.url}
                                  alt={
                                  ...
                                  onError={(e) => {
                                    console.error(
                                      `[RestorationInspectionReportViewer] Failed to load image: ${photo.url}`,
                                    );
                                  ...
                                  onLoad={() => {
                                    console.log(
                                      `[RestorationInspectionReportViewer] Successfully loaded image: ${photo.url}`,
                                    );
                                  }}
                                />
