...
class FreeTrialService {
  ...
  private async checkDeviceFingerprint(fingerprintHash: string): Promise<FraudCheckResult> {
    ...
    try {
      ...
      // If device fingerprint check fails (e.g., table doesn't exist),
      // log warning and allow trial to proceed
      console.warn('⚠️  Device fingerprint check skipped (table may not exist):', error);
  ...
  private async checkEmailValidity(email: string, userId: string): Promise<FraudCheckResult> {
    ...
    try {
      ...
      // If email check fails (e.g., tables don't exist), log warning and allow trial
      console.warn('⚠️  Email validation check skipped (tables may not exist):', error);
      return { allowed: true, fraudScore: 0, flags };
    }
  }
  ...
  private async checkIpAddress(ipAddress: string, userId: string): Promise<FraudCheckResult> {
    ...
    try {
      ...
      // If IP check fails, log warning and allow trial
      console.warn('⚠️  IP address check skipped (tables may not exist):', error);
      return { allowed: true, fraudScore: 0, flags };
    }
  }
  ...
  private async checkPaymentVerification(userId: string): Promise<FraudCheckResult> {
    ...
    try {
      ...
      if (!paymentVerification) {
        ...
        // This is OPTIONAL for trials when Stripe is not configured
        return { allowed: true, fraudScore: 0, flags };
      }
      ...
      // If payment verification check fails (e.g., table doesn't exist when Stripe not configured),
      // log warning and allow trial to proceed
      console.warn('⚠️  Payment verification check skipped (Stripe may not be configured):', error);
      return { allowed: true, fraudScore: 0, flags };
    }
  }
  ...
  private async checkUsagePatterns(userId: string, fingerprintHash: string): Promise<FraudCheckResult> {
    ...
    try {
      ...
      // If usage check fails, log warning and allow trial
      console.warn('⚠️  Usage pattern check skipped (tables may not exist):', error);
      return { allowed: true, fraudScore: 0, flags };
    }
  }
  ...
  private async checkTimeLockouts(userId: string, fingerprintHash: string): Promise<FraudCheckResult> {
    ...
    try {
      ...
      // If lockout check fails, log warning and allow trial
      console.warn('⚠️  Time lockout check skipped (tables may not exist):', error);
      return { allowed: true, fraudScore: 0, flags };
    }
  }
  ...
  async activateTrial(request: TrialActivationRequest): Promise<TrialActivationResult> {
    ...
    try {
      ...
      // Run fraud detection (with graceful fallbacks for missing tables/Stripe)
      ...
      // Save fraud flags to database (optional - don't fail if table doesn't exist)
      if (fraudCheck.flags.length > 0) {
        for (const flag of fraudCheck.flags) {
          try {
            ...
            console.warn('⚠️  Failed to save fraud flag (table may not exist):', flagError);
            // Continue without blocking trial activation
          }
        }
      }
      ...
      // Update or create device fingerprint (optional - don't fail if table doesn't exist)
      try {
        ...
        console.warn('⚠️  Failed to save device fingerprint (table may not exist):', deviceError);
        // Continue without blocking trial activation
      }
      ...
      console.log(`✅ Trial activated successfully for user ${userId} (${user.email})`);
