...
|---|---|---|---|---|---|---|---|
| 1 | Forensic PDF + close-letters cite fabricated **AS-IICRC S500:2021 "Australian edition, Standards Australia + IICRC + RIA"**; `applyCitationGuard` auto-asserts S500 compliance on every water close | compliance | **P0** | `lib/generate-forensic-report-pdf.ts:1694` (footer), `:1658-1660`; `lib/ai/lifecycle/on-close.ts:150,164`; `lib/standards-retrieval.ts:432` | Replace all `S500:2021`/`AS-IICRC`/co-publisher literals with verified `ANSI/IICRC S500-2021 (5th ed.)` from one registry constant; make `applyCitationGuard` conditional on recorded compliance | [CV] | NEW |
