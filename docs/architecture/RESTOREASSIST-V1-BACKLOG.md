...
### P1-12 — Stable SketchRoom identity

- **Decision:** D-009. **Acceptance:** a redraw never deletes a room that owns evidence; rooms removed from the current canvas are detached with history preserved, while empty stale rooms may be deleted. **Verification:** migration + route unit tests. **Status:** DONE — `20260826100000_sketch_room_detach_not_delete`, #2060.

### P1-13 — ScopeItem canonical sketch-room link

- **Decision:** D-009. **Acceptance:** `ScopeItem.sketchRoomId` points to the canonical sketch twin; legacy `ScopeItem.roomId` retires only after production row-count and backfill evidence. **Verification:** migration dry-run + integration + orphan guard. **Status:** TODO — depends on P1-12.
...
### P3-6 — AU/NZ organisation locale and business identity

- **Decision:** D-021. **Acceptance:** setup writes an authoritative AU/NZ country, jurisdiction-appropriate ABN/NZBN, region and valid timezone; server-side activation applies the same country-aware requirements. **Verification:** schema + API + component + activation tests. **Status:** IN PROGRESS.

### P3-7 — GST call-site consolidation

- **Decision:** D-021. **Acceptance:** invoice, estimate, credit-note and accounting paths derive rate/currency from `getGstTreatment()` and contain no AU-only fallbacks. **Verification:** unit + static planted-defect guard. **Status:** TODO — depends on P3-6.
...
### P4-5 — Cross-job continuity exception inbox

- **Decision:** D-020. **Acceptance:** authorised admin/management roles can triage completeness findings across jobs from one navigable surface; findings remain projections of P4-3 and cannot author state. **Verification:** isolation + integration + accessibility. **Status:** TODO — depends on P4-3.

### P4-6 — Consumer plain-language restoration views

- **Acceptance:** homeowner views describe status, affected rooms, decisions and next actions without exposing technical Cat/Class labels, raw moisture readings or area measurements; insurer/reviewer evidence retains technical detail. **Verification:** component tests + portal route regression. **Status:** IN PROGRESS.
...
### P5-3 — Accessibility ratchet

- **Acceptance:** WCAG 2.2 AA checks become a blocking ratchet for the new Job Continuity surfaces, including keyboard, focus, contrast, semantics and 44px field touch targets. **Verification:** lint + axe + browser checks. **Status:** TODO.

### P5-4 — Reconcile design source with shipped tokens

- **Acceptance:** `.claude/DESIGN.md`, `app/globals.css` and the automated token guard describe and enforce one visual system. **Verification:** `npm run check:design-tokens`. **Status:** DONE — #2063.
