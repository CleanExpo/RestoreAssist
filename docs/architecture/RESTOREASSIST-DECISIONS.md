...
- **Consequences:** Classify is propose; accept writes `ai_suggested` job fields; Confirm is the person. Accept merges AI labels into existing technician columns and must never drop `ASBESTOS_SUSPECT`. Classify re-reads `metadata` at write time so an overlapping no-ACM run cannot reset `aiRaisedAcm`. Reject does not overwrite a `HUMAN_TECH` `labelledBy`. Evidence submission is never blocked (RA-7076). Strip-out stays blocked until a recorded WHS pathway.
