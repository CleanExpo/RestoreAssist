...
- **Amendment (RA-7641, 27/09/2026):** Minting a share link (`POST /api/reports/[id]/share-link`) is a **read**. Anyone who can read the report under the read reach above may share it; the route resolves the report through `resolveReportReach`, merged with `AND`, instead of its own `userId` clause, which 404'd an owner pressing Share on a technician's report. A user in another organisation still gets 404. Default taken per RA-7641 recommendation; founder may tighten to MANAGER+.
...
- **Consequences:** Classify is propose; accept writes `ai_suggested` job fields; Confirm is the person. Accept merges AI labels into existing technician columns and must never drop `ASBESTOS_SUSPECT`. Classify (and classification-review) lock the `InspectionPhoto` row with `SELECT … FOR UPDATE` then re-read `metadata` so an overlapping no-ACM run cannot reset `aiRaisedAcm`. Reject does not overwrite a `HUMAN_TECH` `labelledBy`. Evidence submission is never blocked (RA-7076). Strip-out stays blocked until a recorded WHS pathway.
