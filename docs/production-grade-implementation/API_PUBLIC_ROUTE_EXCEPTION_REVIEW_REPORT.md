...
| --- | ---: | --- |
| Public token/signing links | 6 | Hardened where clear; requires owner sign-off |
...
| `app/api/invites/[token]/route.ts` | Team invite preview/accept must work before the invitee has an account/session. | Per-IP preview/accept rate limits, expiry/used-token checks, CSRF validation on POST, 12-character password minimum for email path, AU mobile validation, headshot magic-byte and size validation, invite consumed after account creation. | Confirm invite preview fields and public acceptance flow remain acceptable; consider token-shape precheck as a future narrowing. |
| `app/api/portal/invitations/verify/route.ts` | Client portal invite preview must work before a client account exists. | Per-IP rate limit, status/expiry/revocation checks, returns limited client/contractor display fields. | Confirm preview disclosure is acceptable; consider token-shape precheck as a future narrowing. |
| `app/api/portal/invitations/accept/route.ts` | Client portal account creation must work before a client account exists. | Per-IP rate limit, required token/password/name validation, invitation status/expiry/revocation checks, single client-user existence guard, transaction for account creation and invite acceptance. | Confirm 8-character password policy and public account creation flow; consider CSRF and token-shape precheck as future hardening. |
...
Fix: run a formal public route exception review with product/security ownership, then either approve the exception registry or require route-specific auth/token policy changes.
