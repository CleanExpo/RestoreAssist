...
- Offline mutation idempotency foundation is client-tested, JSON mutation replay is backed by durable database idempotency through `withIdempotency`, and multipart evidence-photo replay dedupe is backed by durable fingerprint-based idempotency. Domain-specific `ClientMutation`/`FieldCaptureEvent` models remain open.
