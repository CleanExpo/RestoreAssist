...
- Offline mutation idempotency foundation is client-tested, JSON mutation replay is backed by durable database idempotency through `withIdempotency`, multipart evidence-photo replay dedupe is backed by durable fingerprint-based idempotency, and the `ClientMutation`/`FieldCaptureEvent` model spine now exists. Server write-path integration, conflict handling, and network-toggle/device integration coverage remain open.
