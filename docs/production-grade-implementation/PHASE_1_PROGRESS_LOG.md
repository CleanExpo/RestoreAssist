...
This does not complete MOB-001. Durable server-side idempotency now backs JSON mutation routes that use `withIdempotency`, multipart evidence-photo replay now uses a server-side idempotency fingerprint over the file hash plus form fields, and the inspection evidence JSON mutation path writes `ClientMutation` ledger rows when mobile sends `X-RestoreAssist-Mutation-Id`. Broader server write-path coverage, conflict handling, and network-toggle/device integration coverage remain open.
