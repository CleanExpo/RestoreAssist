...
- API audit integration metrics/health slice: added deterministic ordering, explicit `take` caps, and narrower selects to user-scoped integration metrics and health reads. Advisory audit warnings reduced from 32 to 30. Final validation: `pnpm exec vitest run scripts/__tests__/audit-api-routes.test.ts` PASS with 1 file / 7 tests, `pnpm exec tsx scripts/audit-api-routes.ts --json` PASS with 442 routes / 30 warnings / 0 errors, `pnpm type-check` PASS, `pnpm lint` PASS with 0 errors and 839 warnings, `git diff --check` PASS.
...
Error: advisory API route scan reports 0 error-severity findings and 30 warning-severity findings.
...
- API route audit is advisory only. It has identified inherited route-hardening debt and these slices reduced warnings from 76 to 30, but remaining public/token and heavier query warnings still need review.
