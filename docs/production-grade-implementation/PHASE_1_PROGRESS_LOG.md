...
- it warns on documentation/comment references to `NODE_TLS_REJECT_UNAUTHORIZED` so known Ascora comments remain visible without blocking the local repo gate.
- current local result is 0 errors and 2 warnings, both the known Ascora route comments. Live Vercel Production still remains blocked because the encrypted production env var exists.
...
- SEC-002 local env audit slice: added `scripts/audit-env.ts` and fixture coverage for forbidden TLS bypass assignments, comment-only warnings, missing required env examples, and public service-role env names. Final validation: `pnpm exec vitest run scripts/__tests__/audit-env.test.ts scripts/__tests__/audit-api-routes.test.ts` PASS with 2 files / 12 tests, `pnpm exec tsx scripts/audit-env.ts --json` PASS with 0 errors / 2 warnings for known Ascora comments, `pnpm exec tsx scripts/audit-api-routes.ts --json` PASS with 442 routes / 14 warnings / 0 errors, `pnpm type-check` PASS, `pnpm lint` PASS with 0 errors and 838 warnings, `git diff --check` PASS, `pnpm exec vitest run` PASS with 212 files / 1842 tests passed and 16 files / 81 tests skipped, `pnpm build` PASS, `pnpm audit --audit-level=high --prod` PASS for high-severity gate with 3 moderate vulnerabilities reported.
