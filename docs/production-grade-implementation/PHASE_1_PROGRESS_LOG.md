...
This does not complete MOB-001. Durable server-side idempotency now backs JSON mutation routes that use `withIdempotency`, and multipart evidence-photo replay now uses a server-side idempotency fingerprint over the file hash plus form fields. Server write-path integration for `ClientMutation`/`FieldCaptureEvent`, conflict handling, and network-toggle/device integration coverage remain open.
