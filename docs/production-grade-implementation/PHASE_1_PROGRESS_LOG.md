...
This does not complete MOB-001. Durable server-side idempotency now backs JSON mutation routes that use `withIdempotency`, but dedicated `ClientMutation`/`FieldCaptureEvent` domain models, multipart evidence-photo replay dedupe, conflict handling, and network-toggle/device integration coverage remain open.
...
- MOB-001 is only partially covered. The client queue exists and JSON mutation replay through `withIdempotency` is backed by durable database idempotency, but multipart evidence-photo replay dedupe and domain-specific `ClientMutation`/`FieldCaptureEvent` models remain open.
