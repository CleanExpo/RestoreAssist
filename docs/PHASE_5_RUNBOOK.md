...
> document is the *checklist*. The actual `prisma migrate deploy`
...
|---|------|-----------------|-------|
| 0.1 | Confirm `main` HEAD matches expectation | `git log origin/main --oneline -1` → `0dd11c2 feat(claims): RA-1717 — deep-link…` | ☐ |
...
| 2.4 | `psql $SHADOW_DATABASE_URL -c '\d "WorkspaceAiBudget"'` (RA-1707) | ☐ |
| 2.5 | `psql $SHADOW_DATABASE_URL -c '\d "ProgressConsentToken"'` (RA-1708) | ☐ |
