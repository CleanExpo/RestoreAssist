...
async function createInspection(page: Page, title: string): Promise<string> {
  ...
  await page.getByRole("button", { name: /create|save|next/i }).first().click()
  // Redirect to /dashboard/inspections/<id>
  await expect(page).toHaveURL(/\/dashboard\/inspections\/[a-z0-9-]+(?:\/|$)/, { timeout: 10_000 })
...
test.describe("V2 Workflow — with property scraper", () => {
  test("property lookup returns data for a known address", async ({ request }) => {
  ...
  test("domain.com.au fallback returns data when requested", async ({ request }) => {
    const res = await request.post("/api/properties/scrape", {
      data: {
        address: "20 Martin Place",
        postcode: "2000",
  ...
  test("property lookup is cached on second request", async ({ request }) => {
    ...
    const res2 = await request.post("/api/properties/scrape", { data: payload })
  ...
  test("scrape API rejects unauthenticated requests", async ({ request: anonRequest }) => {
  ...
  test("property lookup data appears in inspection detail UI", async ({ page }) => {
...
test.describe("V2 Workflow — manual sketch (no scraper)", () => {
  ...
  test("sketch toolbar renders expected tool buttons", async ({ page }) => {
    ...
    const toolBtns = page.getByRole("button").filter({ hasText: /select|room|line|text|pan/i })
  ...
  test("can switch floor tabs in sketch editor", async ({ page }) => {
    ...
    // "Add floor" button
    const addFloor = page.getByRole("button", { name: /add floor|new floor|\+/i })
    if (await addFloor.isVisible({ timeout: 3_000 }).catch(() => false)) {
      await addFloor.click()
      // A new floor tab should appear
      await expect(page.getByRole("tab", { name: /floor 2|first floor|level/i })).toBeVisible({
        timeout: 5_000,
      })
    }
  })
  ...
  test("floor plan underlay loader panel toggles open", async ({ page }) => {
    ...
    const underlayToggle = page.getByRole("button", { name: /underlay|floor plan image|background/i })
  ...
  test("sketch estimate API returns structure for empty inspection", async ({ request }) => {
    // Without an inspectionId we cannot hit the API directly; verify auth guard
    const res = await request.get("/api/inspections/nonexistent-id/sketches/estimate")
  ...
  test("sketch-preview page renders homeowner view", async ({ page }) => {
    ...
    await expect(page.getByText(/RestoreAssist/i)).toBeVisible({ timeout: 8_000 })
    // Should have a PDF download button
    await expect(page.getByRole("button", { name: /download pdf/i })).toBeVisible({ timeout: 8_000 })
  })
})
...
test.describe("V2 Moisture Mapping", () => {
  test("moisture mapping tab/section is accessible from inspection", async ({ page }) => {
  ...
  test("moisture API endpoint requires authentication", async ({ request: anonRequest }) => {
...
test.describe("V2 Sketch-to-Estimate Pipeline", () => {
  test("estimate endpoint returns correct structure", async ({ request }) => {
    ...
    const res = await request.get("/api/inspections/00000000-0000-0000-0000-000000000000/sketches/estimate")
    expect([401, 404]).toContain(res.status())
  })
  ...
  test("sketch PDF endpoint rejects invalid data URL", async ({ request }) => {
    const res = await request.post(
      "/api/inspections/00000000-0000-0000-0000-000000000000/sketches/pdf",
      {
        data: {
          floors: [
            { label: "Ground Floor", pngDataUrl: "not-a-valid-data-url", fabricJson: {} },
          ],
        },
      }
    )
...
test.describe("V2 Mobile/Tablet — iPad Pro", () => {
  ...
  test("inspection list is usable on iPad", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /inspection/i })).toBeVisible({ timeout: 8_000 })
...
test.describe("V2 Performance Benchmarks", () => {
  ...
  test("sketch estimate API responds within 2 s for empty inspection", async ({ request }) => {
    const start = Date.now()
    await request.get("/api/inspections/00000000-0000-0000-0000-000000000000/sketches/estimate")
    const elapsed = Date.now() - start
  ...
  test("sketch PDF endpoint responds within 10 s for minimal payload", async ({ request }) => {
    ...
    await request.post(
      ...
      {
        data: {
          floors: [{ label: "Ground Floor", pngDataUrl: TINY_PNG, fabricJson: { objects: [] } }],
        },
        timeout: 12_000,
      }
    )
