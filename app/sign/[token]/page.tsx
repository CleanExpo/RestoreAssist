    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-3 text-slate-600">Loading signing page...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-semibold text-slate-900">
                {signatory?.name}{" "}
                <span className="text-xs font-normal px-2 py-0.5 bg-slate-100 rounded-full text-slate-500">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
            <div className="flex items-start gap-2">
              ...
              <span className="text-slate-900">{form?.clientName}</span>
            </div>
            <div className="flex items-start gap-2">
              <MapPin className="h-4 w-4 text-slate-400 shrink-0 mt-0.5" />
              <span className="text-slate-900">{form?.clientAddress}</span>
            </div>
            {form?.companyPhone && (
              <div className="flex items-center gap-2">
                <Phone className="h-4 w-4 text-slate-400 shrink-0" />
                <span className="text-slate-900">{form.companyPhone}</span>
              </div>
            )}
            {form?.companyEmail && (
              <div className="flex items-center gap-2">
                <Mail className="h-4 w-4 text-slate-400 shrink-0" />
                <span className="text-slate-900">{form.companyEmail}</span>
              </div>
            )}
            {form?.incidentDate && (
              <div className="flex items-center gap-2">
              ...
              <div className="space-y-1.5">
                {form.signatures.map((sig) => (
                  <div key={sig.id} className="flex items-center gap-2 text-sm">
                    ...
                    <span
                      className={
                        sig.signedAt ? "text-slate-900" : "text-slate-500"
                      }
                    >
                      {sig.signatoryName}
                    </span>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 space-y-5">
          <h3 className="font-semibold text-slate-900">Sign Below</h3>
          ...
          <label className="flex items-start gap-3 cursor-pointer">
            <input
              ...
              className="mt-1 h-4 w-4 text-cyan-500 border-slate-300 rounded focus:ring-cyan-500"
            />
