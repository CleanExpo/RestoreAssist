...
export default function LoginPage() {
  ...
  // This also ensures the NextAuth CSRF cookie is set
  useEffect(() => {
    getCsrfToken().then(token => {
      console.log('[Login] CSRF token obtained:', token ? 'Yes' : 'No')
      setCsrfToken(token)
    }).catch(err => {
      console.error('[Login] Failed to get CSRF token:', err)
    })
  }, [])
  ...
  useEffect(() => {
    const checkOnboardingAndRedirect = async () => {
      if (shouldRedirect) {
        // TEMPORARY: Skip onboarding check when database is not configured
        // Go directly to dashboard for testing
        const timer = setTimeout(() => {
          window.location.href = "/dashboard"
        }, 1000)
        return () => clearTimeout(timer)

        /* ORIGINAL CODE - Restore when database is configured:
        try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!csrfToken) {
      console.error('[Login] No CSRF token available, fetching...')
    ...
    console.log('[Login] Attempting sign in with credentials')
    ...
    try {
      ...
      console.log('[Login] SignIn result keys:', result ? Object.keys(result) : 'null/undefined')
