...
function LoginForm() {
  ...
  // RA-1842 — Apple App Review (build 1.0(4), 2026-05-04) rejected on
  // 2.1(a) because "Continue with Apple" launches Apple OAuth but the
  // NextAuth callback redirects back to /login instead of /dashboard.
  // Until the callback is fixed, hide BOTH third-party buttons on the
  // iOS Capacitor shell — with NO third-party logins offered there,
  // guideline 4.8 (which would otherwise mandate Sign in with Apple)
  // does not apply. Web users keep Google + Apple as before.
  ...
  // Detect iOS Capacitor shell on mount; hide third-party auth buttons
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 flex items-center justify-center px-4">
      ...
      >
        ...
        >
          ...
          {/* Third-party auth (Google + Apple). Hidden inside the iOS
              Capacitor shell because the Apple OAuth callback is broken
              (RA-1842 / 1.0(4) rejection). With NO third-party logins
              offered on iOS, guideline 4.8 does not apply. Pre-hydration
              we render the buttons so SSR + non-iOS users see them with
              no flash; the gate kicks in once `authHydrated` flips. */}
          {(!authHydrated || !hideThirdPartyAuth) && (
            <>
              ...
              {/* RA-1842 Ground 2 — Sign in with Apple. Apple guideline 4.8
              requires this option whenever a third-party login is offered.
              Button only renders when the Apple provider is configured
              (APPLE_CLIENT_ID + APPLE_CLIENT_SECRET in env). */}
