...
export default function InterviewPage() {
  ...
  const [interviewStatus, setInterviewStatus] = useState<
    "in_progress" | "completed" | "error" | "loading"
  >("in_progress");
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const qa: InterviewQuestionAnswer[] = (session.responses || []).map(
          (r: {
            questionId: string;
            questionText: string;
    ...
    return (
      <div className="py-8">
        <Alert variant="destructive">
          ...
          <AlertDescription>
            Form template ID is required to start an interview.
          </AlertDescription>
        </Alert>
      </div>
    );
...
function AutosaveIndicator({
  ...
  if (status === "saved") {
    ...
    return (
      <p
        className="mt-2 inline-flex items-center gap-1.5 text-xs text-success"
        role="status"
        aria-live="polite"
      >
