...
import { ChevronLeft, ChevronRight, Search, FileText, Plus } from 'lucide-react'
...
type FormStatus = 'DRAFT' | 'IN_PROGRESS' | 'AWAITING_SIGNATURE' | 'COMPLETED' | 'CANCELLED'
type FormType = 'WORK_ORDER' | 'JSA' | 'AUTHORITY_TO_COMMENCE' | 'SITE_INDUCTION' | 'CUSTOM'
...
const ALL_STATUSES: FormStatus[] = ['DRAFT', 'IN_PROGRESS', 'AWAITING_SIGNATURE', 'COMPLETED', 'CANCELLED']
const ALL_FORM_TYPES: FormType[] = ['WORK_ORDER', 'JSA', 'AUTHORITY_TO_COMMENCE', 'SITE_INDUCTION', 'CUSTOM']
...
function StatusBadge({ status }: { status: FormStatus }) {
  ...
  return (
    <Badge className={colourMap[status]}>
...
function SignatureBadge({ required, completed, status }: { required: number; completed: number; status: FormStatus }) {
  if (status !== 'AWAITING_SIGNATURE') return null
  const allSigned = completed >= required
  return (
    <Badge className={allSigned ? 'border-transparent bg-green-100 text-green-700' : 'border-transparent bg-amber-100 text-amber-700'}>
    ...
    <tr>
      <td colSpan={7} className="py-16 text-center">
        <div className="flex flex-col items-center gap-3 text-muted-foreground">
          ...
          <p className="text-xs">Adjust your filters or create a new submission from a template.</p>
...
export default function FormSubmissionsPage() {
  ...
  // Filters
  const [activeStatusChip, setActiveStatusChip] = useState<FormStatus | 'ALL'>('ALL')
  ...
  const filtered = useMemo(() => {
    ...
    const effectiveStatus = activeStatusChip !== 'ALL' ? activeStatusChip : statusFilter !== 'ALL' ? statusFilter : null
    if (effectiveStatus) {
    ...
    return items
  }, [submissions, activeStatusChip, statusFilter, formTypeFilter, searchText, dateFrom, dateTo])
  ...
  useEffect(() => {
    setCurrentPage(1)
  }, [activeStatusChip, statusFilter, formTypeFilter, searchText, dateFrom, dateTo])
    ...
    <div className="flex flex-col gap-6 p-6 max-w-7xl mx-auto w-full">
      ...
      <div className="flex flex-col gap-1">
        <div className="flex items-center gap-1.5 text-sm text-muted-foreground">
          <Link href="/dashboard/forms/templates" className="hover:text-foreground transition-colors">
            Forms
          </Link>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {ALL_STATUSES.filter(s => s !== 'CANCELLED').map(status => {
          const chipColour: Record<FormStatus, { active: string; inactive: string }> = {
            DRAFT: {
              active: 'border-gray-600 bg-gray-600 text-white',
              inactive: 'border-gray-200 bg-gray-50 text-gray-700 hover:bg-gray-100',
            },
            IN_PROGRESS: {
          ...
          return (
            <button
              ...
              className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-sm font-medium transition-colors ${
                isActive ? chipColour[status].active : chipColour[status].inactive
              }`}
            >
              {STATUS_LABELS[status]}
              <span className={`rounded-full px-1.5 py-0.5 text-xs leading-none ${isActive ? 'bg-white/25' : 'bg-black/10'}`}>
                {statusCounts[status]}
              </span>
            </button>
          )
        ...
        <div className="flex flex-wrap gap-3 p-4 items-end">
          ...
          <div className="min-w-[180px]">
            <Select
              ...
              onValueChange={val => setFormTypeFilter(val as FormType | 'ALL')}
            >
              <SelectTrigger>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-muted/40">
                ...
                <th className="px-4 py-3 text-left font-medium text-muted-foreground">Form Type</th>
                <th className="px-4 py-3 text-left font-medium text-muted-foreground">Linked Report</th>
                <th className="px-4 py-3 text-left font-medium text-muted-foreground">Completeness</th>
                <th className="px-4 py-3 text-left font-medium text-muted-foreground">Signatures</th>
                ...
                paginated.map(sub => (
                  <tr key={sub.id} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className="text-xs text-muted-foreground">{FORM_TYPE_LABELS[sub.formType]}</span>
                      <br />
        ...
        {!loading && filtered.length > ITEMS_PER_PAGE && (
          <div className="flex items-center justify-between border-t px-4 py-3">
            <p className="text-xs text-muted-foreground">
              Showing {(currentPage - 1) * ITEMS_PER_PAGE + 1}–{Math.min(currentPage * ITEMS_PER_PAGE, filtered.length)} of {filtered.length} submissions
            </p>
