...
export default function SettingsPage() {
  ...
  const hasOrganization = !!(
    profile?.organizationId || session?.user?.organizationId
  );
  ...
  const handleUpdateName = async () => {
    ...
    if (profile?.subscriptionStatus === "TRIAL") {
      toast.error(
        "Profile updates are locked for free users. Upgrade to unlock this feature.",
      );
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      ...
      const updateData = canEditBusinessInfo
        ? formData
        : { name: formData.name };
        ...
        if (isOnboarding) {
          ...
          if (onboardingResponse.ok) {
            ...
            if (onboardingData.nextStep) {
              ...
              if (nextStepRoute) {
                toast.success("Step 1 complete! Redirecting to next step...", {
                  duration: 2000,
                });
                setTimeout(() => {
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.success(
          'Logo uploaded. Click "Save Changes" to save all information.',
        );
      } else {
        const error = await response.json();
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case "PAST_DUE":
        return "text-orange-600 dark:text-orange-400 bg-orange-500/20";
      default:
        return "text-neutral-600 dark:text-slate-400 bg-neutral-200 dark:bg-slate-500/20";
    }
  };
                ...
                <div className="flex items-center justify-between mb-6">
                  ...
                  {!editingName && (
                    <button
                      ...
                      title={
                        profile?.subscriptionStatus === "TRIAL"
                          ? "Upgrade to unlock profile updates"
                          : "Edit Name"
                      }
                    >
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    {editingName ? (
                      ...
                      <p className={cn("text-neutral-700 dark:text-slate-300")}>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    {editing && canEditBusinessInfo ? (
                      ...
                      <p className={cn("text-neutral-700 dark:text-slate-300")}>
                        {profile?.businessABN || "Not provided"}
                      </p>
                    )}
                  </div>
                    ...
                    <>
                      <div>
                        ...
                        >
                          ...
                          {profile.reportLimits.availableReports} /{" "}
                          {profile.reportLimits.baseLimit +
                            profile.reportLimits.addonReports}
                        </div>
                        <div
                          className={cn(
                            "text-sm mt-1",
                        ...
                        >
                          ...
                          {profile.reportLimits.addonReports > 0 &&
                            ` + Add-ons: ${profile.reportLimits.addonReports}`}
                        </div>
                      </div>
                        ...
                        <div
                          ...
                          style={{
                            width:
                              profile?.creditsRemaining == null
                                ? "0%"
                                : `${Math.min(100, ((profile?.totalCreditsUsed || 0) / ((profile?.totalCreditsUsed || 0) + (profile?.creditsRemaining ?? 0) || 1)) * 100)}%`,
                          }}
                        ></div>
                      </div>
