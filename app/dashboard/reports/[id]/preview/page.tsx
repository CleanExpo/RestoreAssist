...
interface Report {
  ...
  psychrometricReadings?: Array<{ temperature?: number; relativeHumidity?: number; dewPoint?: number; location?: string }>
...
export default function ReportPreviewPage() {
  ...
  const psychroReadings = report.psychrometricReadings ?? psychro?.readings ?? []

  const formatCurrency = (val?: number | null) =>
    val != null ? `$${val.toLocaleString("en-AU", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : "—"
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 print:px-0 print:py-4">
      ...
      <div className="hidden print:block mb-6">
        ...
        {report.reportNumber && (
          <p className="text-slate-500 text-sm">Report #{report.reportNumber}</p>
        )}
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-gradient-to-r from-cyan-600 to-cyan-500 rounded-xl p-6 text-white">
          <div className="flex items-start justify-between">
            <div>
              ...
              {report.reportNumber && (
                <p className="text-cyan-100 text-sm">Report #{report.reportNumber}</p>
              )}
            </div>
        ...
        <SectionCard title="Property Information" icon={<MapPin size={18} />}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Field label="Insurer / Insurance Type" value={report.insuranceType} />
            <Field
            ...
            {report.affectedArea != null && (
              <Field label="Affected Area" value={`${report.affectedArea} m²`} />
            )}
          </div>
        </SectionCard>
        ...
        {(report.waterCategory || report.waterClass) && (
          <SectionCard title="Damage Classification" icon={<Droplets size={18} />}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            {report.detailedReport && (
              <div className="mt-4 pt-4 border-t border-slate-100">
                <p className="text-xs font-medium text-slate-500 uppercase tracking-wide mb-2">IICRC Assessment Notes</p>
        ...
        {(psychro || psychroReadings.length > 0) && (
          <SectionCard title="Environmental Data" icon={<Thermometer size={18} />}>
            {psychro && (psychro.temperature != null || psychro.humidity != null) && (
              <div className="grid grid-cols-3 gap-4 mb-4">
                {psychro.temperature != null && (
                  <div className="bg-blue-50 rounded-lg p-3 text-center">
                    <p className="text-xs text-blue-600 font-medium">Temperature</p>
                    <p className="text-xl font-bold text-blue-800">{psychro.temperature}°C</p>
                  </div>
                )}
                {psychro.humidity != null && (
                  <div className="bg-green-50 rounded-lg p-3 text-center">
                    <p className="text-xs text-green-600 font-medium">Humidity</p>
                    <p className="text-xl font-bold text-green-800">{psychro.humidity}%</p>
                  </div>
                )}
                {psychro.dewPoint != null && (
                  <div className="bg-purple-50 rounded-lg p-3 text-center">
                    <p className="text-xs text-purple-600 font-medium">Dew Point</p>
                    <p className="text-xl font-bold text-purple-800">{psychro.dewPoint}°C</p>
                  </div>
                )}
              </div>
            )}
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-200">
                      ...
                      <th className="text-right py-2 px-2 text-slate-500 font-medium">Temp (°C)</th>
                      <th className="text-right py-2 px-2 text-slate-500 font-medium">RH (%)</th>
                      <th className="text-right py-2 px-2 text-slate-500 font-medium">Dew Pt (°C)</th>
                    </tr>
                  </thead>
                  <tbody>
                    {psychroReadings.map((r, i) => (
                      <tr key={i} className="border-b border-slate-100 last:border-0">
                        <td className="py-2 px-2 text-slate-700">{r.location ?? `Reading ${i + 1}`}</td>
                        <td className="py-2 px-2 text-right text-slate-700">{r.temperature ?? "—"}</td>
                        <td className="py-2 px-2 text-right text-slate-700">{r.relativeHumidity ?? "—"}</td>
                        <td className="py-2 px-2 text-right text-slate-700">{r.dewPoint ?? "—"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        {scopeItems.length > 0 && (
          <SectionCard title="Scope of Works" icon={<FileText size={18} />}>
            {report.scopeOfWorksData?.summary && (
              <p className="text-slate-600 text-sm mb-4">{report.scopeOfWorksData.summary}</p>
            )}
            <div className="space-y-3">
              {scopeItems.map((item, i) => (
                <div key={item.id ?? i} className="flex items-start gap-3 p-3 bg-slate-50 rounded-lg">
                  <ChevronRight size={16} className="text-cyan-500 mt-0.5 shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="text-slate-800 text-sm font-medium">{item.description}</p>
                    {item.justification && (
                  ...
                  {item.totalCost != null && (
                    <p className="text-slate-700 text-sm font-medium shrink-0">{formatCurrency(item.totalCost)}</p>
                  )}
                </div>
              ))}
            </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-200">
                      ...
                      <th className="text-right py-2 px-2 text-slate-500 font-medium">Unit Cost</th>
                  ...
                  <tbody>
                    {costData.lineItems.map((li, i) => (
                      <tr key={i} className="border-b border-slate-100 last:border-0">
                        <td className="py-2 px-2 text-slate-700">{li.description}</td>
                        <td className="py-2 px-2 text-right text-slate-600">{li.quantity ?? "—"}</td>
                        <td className="py-2 px-2 text-right text-slate-600">{formatCurrency(li.unitCost)}</td>
                        <td className="py-2 px-2 text-right text-slate-700 font-medium">{formatCurrency(li.totalCost)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="border-t border-slate-200 pt-4 space-y-2">
              ...
              {(costData.totalIncGST ?? report.totalCost) != null && (
                <div className="flex justify-between text-base font-bold text-slate-800 pt-1 border-t border-slate-200">
                  ...
                  <span className="text-cyan-600">{formatCurrency(costData.totalIncGST ?? report.totalCost)}</span>
                </div>
              )}
            </div>
          </SectionCard>
        ...
        <div className="text-center text-xs text-slate-400 py-4 print:block hidden">
          Generated by RestoreAssist &bull; {new Date(report.createdAt).toLocaleDateString("en-AU")}
        </div>
      </div>
    </div>
  )
