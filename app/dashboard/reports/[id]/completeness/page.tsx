...
export default function ReportCompletenessPage() {
  ...
  const completeSections =
    result?.sections.filter((s) => s.status === "complete").length ?? 0;
  const partialSections =
    result?.sections.filter((s) => s.status === "partial").length ?? 0;
  const missingSections =
    result?.sections.filter((s) => s.status === "missing").length ?? 0;
