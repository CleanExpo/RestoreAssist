...
export default function CreditsPage() {
  ...
  const isUnlimited =
    ...
    (data?.subscriptionStatus === "TRIAL" && data?.trialStatus?.hasUnlimitedTrial)

  const isTrial = data?.subscriptionStatus === "TRIAL"
  const isActive = data?.subscriptionStatus === "ACTIVE"
  const isExpiredOrCanceled =
    data?.subscriptionStatus === "EXPIRED" || data?.subscriptionStatus === "CANCELED"
  ...
  const addonReports = data?.reportLimits?.addonReports ?? data?.addonReports ?? 0
  const totalLimit = baseLimit + addonReports
  const usagePct = totalLimit > 0 ? Math.min(100, Math.round((monthlyUsed / totalLimit) * 100)) : 0
  const nearLimit = usagePct >= 70

  const showUpgradeCTA = isTrial || isExpiredOrCanceled || (isActive && nearLimit)
    ...
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-8">
      ...
      <div className={cn(
        ...
        <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-3">
          ...
          <div className="bg-white/10 rounded-xl p-3">
            ...
            <p className="text-white font-semibold text-lg">
              ...
              {!isUnlimited && totalLimit > 0 && (
                <span className="text-blue-200 text-sm font-normal"> / {totalLimit}</span>
              )}
            </p>
          ...
          <div className="bg-white/10 rounded-xl p-3">
            ...
            <p className="text-white font-semibold text-lg">{data?.totalCreditsUsed ?? 0}</p>
      ...
      <div className={cn(
        ...
        <div className="flex items-center gap-2">
          ...
          <h2 className={cn("font-semibold text-lg", "text-neutral-900 dark:text-slate-50")}>
            Usage Breakdown
          </h2>
        </div>
        ...
        {!isUnlimited && totalLimit > 0 && (
          <div className="space-y-2">
            <div className="flex justify-between items-center text-sm">
              <span className={cn("text-neutral-600 dark:text-slate-400")}>Monthly reports used</span>
              <span className={cn(
                "font-medium",
                usagePct >= 90
            ...
            <div className="h-3 rounded-full bg-neutral-100 dark:bg-slate-800 overflow-hidden">
              <div
                className={cn("h-full rounded-full transition-all duration-500", getUsageBarColor(usagePct))}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          {isActive && baseLimit > 0 && (
            <div className={cn(
              ...
              <p className={cn("text-xs font-medium mb-1", "text-neutral-500 dark:text-slate-400")}>
                Base plan limit
              </p>
              <p className={cn("text-2xl font-bold", "text-neutral-900 dark:text-slate-50")}>
                {baseLimit}
              </p>
              <p className={cn("text-xs mt-1", "text-neutral-500 dark:text-slate-400")}>
                reports / month
              </p>
            </div>
          )}
          ...
          {(addonReports > 0 || isActive) && (
            <div className={cn(
              ...
              <div className="flex items-center gap-1.5 mb-1">
                ...
                <p className={cn("text-xs font-medium", "text-neutral-500 dark:text-slate-400")}>
                  Addon reports
                </p>
              </div>
              <p className={cn("text-2xl font-bold", "text-neutral-900 dark:text-slate-50")}>
          ...
          <div className={cn(
            ...
            <p className={cn("text-base font-bold", "text-neutral-900 dark:text-slate-50")}>
              {isTrial
                ? formatTrialEnd(data?.trialEndsAt)
                : formatResetDate(data?.monthlyResetDate)}
            </p>
