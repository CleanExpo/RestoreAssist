          ...
          <div className="flex items-center gap-2.5">
            <div
              className={cn(
                ...
                ready
                  ? "bg-emerald-100 dark:bg-emerald-900/50 text-success dark:text-success"
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                {ready ? (
                  <Badge className="bg-emerald-100 text-success dark:bg-emerald-900/50 dark:text-success text-[10px] py-0 px-1.5 border-0">
                    Ready to promote
                  </Badge>
                ) : dataCollection ? (
                  <Badge
                  ...
                  <Badge
                    variant="outline"
                    className="text-[10px] py-0 px-1.5 text-warning dark:text-warning border-amber-200 dark:border-amber-800/60"
                  >
                    Threshold not met
                  </Badge>
                ) : null}
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-between text-xs">
              <span className="text-neutral-600 dark:text-slate-400">
                NIR{" "}
                {OBS_TYPE_LABELS[criteria.observationType] ??
                  criteria.observationType}
              </span>
              <span
                className={cn(
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="font-mono font-semibold text-neutral-800 dark:text-slate-200">
                  ...
                  <span className="text-neutral-400 font-normal">
                    vs threshold {evaluation.threshold}
                  </span>
                </span>
              </div>
            )}
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-neutral-500 dark:text-slate-400 mt-1">
            ...
            , update{" "}
            <code className="text-xs bg-neutral-100 dark:bg-slate-800 px-1 py-0.5 rounded font-mono">
        ...
        <div className="rounded-xl border border-emerald-200 dark:border-emerald-800/60 bg-emerald-50 dark:bg-emerald-950/20 px-5 py-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-success dark:text-success mt-0.5">
              ...
              to update each status to{" "}
              <code className="font-mono">VALIDATED</code>.
            </p>
          </div>
        </div>
      ) : (
        <div className="rounded-xl border border-neutral-200 dark:border-slate-800 bg-neutral-50 dark:bg-slate-900/50 px-5 py-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-neutral-500 dark:text-slate-400 mt-0.5">
              ...
              {meta.manualObservations} manual · {meta.derivedObservations}{" "}
              auto-derived
            </p>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="rounded-xl border border-neutral-200 dark:border-slate-800 divide-y divide-neutral-100 dark:divide-slate-800 overflow-hidden">
            {report.actionItems.map((item, i) => (
              <div
                key={i}
                className="flex items-start gap-3 px-4 py-3 bg-white dark:bg-slate-900"
              >
                <ChevronRight
                  size={14}
      ...
      {report.readyToPromote.length > 0 && (
        <div className="rounded-xl border border-cyan-200/60 dark:border-cyan-800/40 bg-cyan-50/40 dark:bg-cyan-950/20 px-5 py-4 space-y-3">
          ...
          <ol className="space-y-1.5 text-xs text-neutral-600 dark:text-slate-400 list-decimal list-inside">
            <li>
              Review the underlying observations (sanity check for outliers)
            </li>
            <li>
              Open{" "}
            ...
            <li>
              Fill in <code className="font-mono">validatedBy</code>,{" "}
              <code className="font-mono">validatedAt</code>,{" "}
              <code className="font-mono">validationNotes</code>
            </li>
            <li>Open a PR — the diff IS the audit trail</li>
            <li>
              Once merged: update the evidence register with the PR URL as the
              source
            </li>
          </ol>
          <div className="flex flex-wrap gap-2 pt-1">
            {report.readyToPromote.map((r) => (
              <Badge
                key={r.claim.id}
                className="bg-emerald-100 text-success dark:bg-emerald-900/50 dark:text-success border-0 gap-1"
              >
