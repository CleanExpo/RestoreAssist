...
export default function Claim002CostEntryPage() {
  ...
  const validateEntries = (toValidate: CostEntry[]): string[] => {
    ...
    toValidate.forEach((e, i) => {
      ...
      if (isNaN(cost) || cost <= 0) errs.push(`${row}: Total cost must be a positive number (AUD).`)
      if (cost > 1_000_000) errs.push(`${row}: Cost seems too high (>$1M). Please verify.`)
    })
    return errs
  }
  ...
  const handleBatchSubmit = async () => {
    ...
    try {
      ...
      if (!Array.isArray(raw)) {
        setBatchErrors(["JSON must be an array of objects."]); return
      }
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-neutral-500 dark:text-slate-400 mt-0.5">
              Record per-claim total costs (AUD) for NIR vs. existing-format claims.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-lg border border-cyan-200/60 dark:border-cyan-800/40 bg-cyan-50/40 dark:bg-cyan-950/20 px-4 py-3 space-y-1.5">
        ...
        <p className="text-xs text-neutral-600 dark:text-slate-400 leading-relaxed">
          Enter the <strong>total claim settlement cost (AUD)</strong> for each water damage claim reviewed.
          Source from Guidewire ClaimCenter settlement reports or insurer-provided CSV exports.
          Use <strong>group: NIR</strong> for claims processed using the standardised format,
          and <strong>group: Control</strong> for claims processed under the existing non-standardised format.
          Minimum <strong>50 per group</strong> required for the Welch t-test to be valid (p &lt; 0.05).
        </p>
      </div>
      ...
      {successCount !== null && (
        <div className="rounded-lg border border-emerald-200 dark:border-emerald-800/50 bg-emerald-50 dark:bg-emerald-950/30 px-4 py-3 flex items-center gap-3">
          ...
          <p className="text-sm text-emerald-700 dark:text-emerald-300">
            <strong>{successCount}</strong> cost observation{successCount !== 1 ? "s" : ""} recorded for CLAIM-002.
          </p>
        </div>
      )}
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <div className="grid grid-cols-[1fr_100px_1fr_28px] gap-2 px-1">
              {["Total cost (AUD) *", "Group *", "Claim ref / notes", ""].map((h, i) => (
            ...
            {entries.map((entry, idx) => (
              <div key={idx} className="grid grid-cols-[1fr_100px_1fr_28px] gap-2 items-center">
                {/* Cost */}
                <input
                  type="number"
                  min="0"
                  ...
                  onChange={e => updateEntry(idx, "totalCostAUD", e.target.value)}
      ...
      {batchMode && (
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardDescription className="text-xs">
              Paste an array of cost records. Each record needs <code className="font-mono">totalCostAUD</code> and <code className="font-mono">group</code>.
            </CardDescription>
          </CardHeader>
