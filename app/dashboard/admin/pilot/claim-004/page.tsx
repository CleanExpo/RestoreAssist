  ...
  const validateEntries = (toValidate: AdjusterEntry[]): string[] => {
    ...
    toValidate.forEach((e, i) => {
      ...
      if (isNaN(mins) || mins <= 0)
        errs.push(`Row ${i + 1}: Review minutes must be a positive number.`);
      if (mins > 480)
        errs.push(
          `Row ${i + 1}: Review time seems too high (>8 hours). Please verify.`,
        );
    });
    return errs;
  };
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-neutral-500 dark:text-slate-400 mt-0.5">
              Record adjuster report review times (minutes) for NIR vs. existing
              formats.
            </p>
          </div>
        </div>
      ...
      <div className="rounded-lg border border-cyan-200/60 dark:border-cyan-800/40 bg-cyan-50/40 dark:bg-cyan-950/20 px-4 py-3 space-y-2">
        ...
        <p className="text-xs text-neutral-600 dark:text-slate-400 leading-relaxed">
          Adjuster sessions can be recorded two ways: (1) adjusters self-report
          via the token-gated API endpoint using their team&apos;s pilot token,
          or (2) use this form for sessions timed during structured test rounds.
          Record review time in <strong>decimal minutes</strong> (e.g. 42.5 for
          42 min 30 sec). Use <strong>NIR group</strong> for
          RestoreAssist-generated reports, <strong>Control group</strong> for
          non-standardised formats. Minimum{" "}
        ...
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardDescription className="text-xs">
              One row per review session. Adjuster code and report ref are
              optional (anonymised).
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            ...
            {entries.map((entry, idx) => (
              <div
                key={idx}
                className="grid grid-cols-[90px_80px_110px_1fr_1fr_28px] gap-2 items-center"
              >
                {/* Adjuster code */}
                <input
                  type="text"
            ...
            <div className="flex items-center justify-between pt-1">
              <p className="text-xs text-neutral-400 dark:text-slate-500">
                {entries.length} session{entries.length !== 1 ? "s" : ""} to
                submit
              </p>
              <Button
      ...
      {batchMode && (
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardDescription className="text-xs">
              Paste an array of adjuster session records. Each needs{" "}
              <code className="font-mono">reviewMinutes</code> and{" "}
    ...
    "adjusterCode": "ADJ-03"
      ...
      <div className="rounded-lg border border-neutral-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 py-3 space-y-2">
        ...
        <p className="text-[10px] text-neutral-400 dark:text-slate-500 pt-1 border-t border-neutral-100 dark:border-slate-800">
          Structured test protocol: 6 adjusters × 10 NIR reports + 10 existing
          format reports each. Adjuster codes are anonymised — use ADJ-01
          through ADJ-06 or similar.
        </p>
      </div>
    </div>
  );
