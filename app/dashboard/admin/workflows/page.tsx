...
> = {
  ...
  RUNNING:          { label: 'Running',           className: 'bg-blue-500/10 text-blue-600 dark:text-blue-400',        icon: Loader2 },
  PAUSED:           { label: 'Paused',            className: 'bg-amber-500/10 text-amber-600 dark:text-amber-400',     icon: Clock },
  COMPLETED:        { label: 'Completed',         className: 'bg-green-500/10 text-green-600 dark:text-green-400',     icon: CheckCircle },
  FAILED:           { label: 'Failed',            className: 'bg-red-500/10 text-red-600 dark:text-red-400',           icon: XCircle },
  PARTIALLY_FAILED: { label: 'Partial Fail',      className: 'bg-orange-500/10 text-orange-600 dark:text-orange-400', icon: AlertCircle },
  CANCELLED:        { label: 'Cancelled',         className: 'bg-neutral-500/10 text-neutral-500 dark:text-neutral-400', icon: XCircle },
}
...
> = {
  ...
  SKIPPED:     { label: 'Skipped',     className: 'bg-neutral-500/10 text-neutral-500' },
  CANCELLED:   { label: 'Cancelled',   className: 'bg-neutral-500/10 text-neutral-500' },
  DEAD_LETTER: { label: 'Dead Letter', className: 'bg-red-900/20 text-red-700 dark:text-red-400' },
}
...
function TaskRow({ task }: { task: AgentTask }) {
  ...
  const cfg = TASK_STATUS_CONFIG[task.status] ?? { label: task.status, className: '' }
  ...
  return (
    <tr className="border-b border-neutral-100 dark:border-neutral-800 bg-neutral-50/50 dark:bg-neutral-800/30">
      ...
      <td className="px-4 py-2 text-xs text-neutral-500">{formatDate(task.startedAt)}</td>
      <td className="px-4 py-2 text-xs text-neutral-500">{formatDate(task.completedAt)}</td>
      <td className="px-4 py-2 text-xs text-neutral-500">
        {task.durationMs != null ? `${task.durationMs}ms` : formatDuration(task.startedAt, task.completedAt)}
      </td>
      <td className="px-4 py-2 text-xs" colSpan={2}>
        {task.errorMessage ? (
          <div>
            <span
              className={cn('text-red-500 dark:text-red-400 cursor-pointer', expandError ? '' : 'line-clamp-1')}
  ...
  return (
    <>
            ...
            <div className="flex-1 h-1.5 bg-neutral-200 dark:bg-neutral-700 rounded-full overflow-hidden">
              <div
                className={cn(
                  ...
                  workflow.status === 'FAILED' || workflow.status === 'PARTIALLY_FAILED'
                    ? 'bg-red-500'
      ...
      {expanded && tasks.length === 0 && !loadingTasks && (
        <tr className="bg-neutral-50/80 dark:bg-neutral-800/20">
          <td colSpan={9} className="px-4 py-3 text-xs text-neutral-400 text-center">
...
export default function WorkflowMonitorPage() {
  ...
  const handleResume = async (id: string) => {
    try {
      const res = await fetch(`/api/agents/workflows/${id}/resume`, { method: 'POST' })
      if (res.ok) {
        await fetchWorkflows()
      }
    } catch (err) {
      ...
      <Card className="bg-white dark:bg-neutral-900 border-neutral-200 dark:border-neutral-800">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-neutral-900 dark:text-white text-base">
            ...
            Workflows
            <span className="text-sm font-normal text-neutral-400">({workflows.length})</span>
          </CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          {workflows.length === 0 ? (
