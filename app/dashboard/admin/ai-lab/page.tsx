      ...
      {open && (
        <div className="px-3 pb-3 space-y-2 border-t border-neutral-100 dark:border-slate-700/50 pt-2">
          <ScoreBar
            ...
            score={c.subScores.structural}
            weight={30}
          />
          <ScoreBar
            label="IICRC citation density"
            score={c.subScores.citation}
            weight={25}
          />
          <ScoreBar
            label="Equipment ratio"
            score={c.subScores.equipment}
            weight={20}
          />
          <ScoreBar
            label="Specificity"
            score={c.subScores.specificity}
            weight={15}
          />
          <ScoreBar
            label="Category compliance"
            score={c.subScores.category}
            weight={10}
          />
        </div>
      )}
...
export default function AILabPage() {
  ...
  const [vectoriseResult, setVectoriseResult] = useState<{
    embedded: number;
    skipped: number;
    total: number;
    remaining: number;
  ...
  const runVectorise = async () => {
    ...
    try {
      const resp = await fetch("/api/admin/vectorise", {
        method: "POST",
    ...
    <div className="space-y-8 p-6 max-w-4xl mx-auto">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Embed un-processed HistoricalJob records using OpenAI
            text-embedding-3-small. Required for RAG scope generation and
            similar-job lookup.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex items-center gap-4">
            ...
            <span className="text-xs text-neutral-400">
              Processes 50 un-embedded jobs per run. Idempotent — safe to
              re-run.
            </span>
          </div>
          ...
          {vectoriseResult && (
            <div className="grid grid-cols-4 gap-3">
              {[
                ...
                {
                  label: "Duration",
                  value: `${vectoriseResult.durationMs}ms`,
                  color: "text-neutral-400",
                },
              ].map((s) => (
                <div
                  key={s.label}
                  className="text-center p-3 rounded-lg bg-neutral-50 dark:bg-slate-800/50 border border-neutral-100 dark:border-slate-700/50"
                >
          ...
          {vectoriseResult && (
            <div className="text-xs text-neutral-400">
              Provider: {vectoriseResult.provider} · Total jobs:{" "}
              {vectoriseResult.total}
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Scores generated scopes against 5 quality dimensions using
            deterministic analysis. No API cost.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex gap-4 flex-wrap">
            <div className="flex-1 min-w-[160px]">
              <label className="text-xs font-medium text-neutral-500 dark:text-slate-400 block mb-1.5">
                Claim Type
              </label>
              <select
                value={evalClaimType}
                onChange={(e) =>
                  setEvalClaimType(e.target.value as ClaimType | "all")
                }
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-4 gap-3">
                {[
                  {
                    label: "Mean Score",
                    value: Math.round(evalReport.meanComposite),
                    suffix: "/100",
                  },
                  {
                    label: "Min Score",
                    value: Math.round(evalReport.minComposite),
                    suffix: "",
                  },
                  {
                    label: "Max Score",
                    value: Math.round(evalReport.maxComposite),
                    suffix: "",
                  },
              ...
              {Object.keys(evalReport.meanByClaimType).length > 1 && (
                <div className="space-y-1.5">
                  ...
                  {Object.entries(evalReport.meanByClaimType).map(
                    ([ct, score]) => (
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Autonomous prompt improvement loop — Claude edits prompts, scores
            variants, promotes the best. ~$0.02 per API call.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex gap-4 flex-wrap">
          ...
          <div className="text-xs text-neutral-400 bg-amber-50 dark:bg-amber-900/10 border border-amber-200/50 dark:border-amber-800/30 rounded-lg p-2.5">
            Estimated cost: ~${(optBudget * 0.02).toFixed(2)} AUD · Budget{" "}
            {optBudget} calls · Threshold +{optThreshold} points required to
            promote
          </div>
            ...
            <div className="rounded-lg border border-neutral-200 dark:border-slate-700/50 bg-neutral-50 dark:bg-slate-800/50 p-4 space-y-3">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-sm text-neutral-900 dark:text-white">
                  {optResult.promoted
                    ? "New variant promoted to production"
              ...
              <div className="grid grid-cols-3 gap-3 text-center">
                <div>
                  <div className="text-lg font-bold">
                    {Math.round(optResult.baselineScore)}
                  </div>
                ...
                <div>
                  ...
                  >
                    {optResult.improvement > 0 ? "+" : ""}
                    {Math.round(optResult.improvement)}
                  </div>
                  <div className="text-xs text-neutral-400">Improvement</div>
                </div>
                <div>
                  <div className="text-lg font-bold">
                    ${optResult.estimatedCostAud.toFixed(2)}
                  </div>
                  <div className="text-xs text-neutral-400">Cost AUD</div>
                </div>
              </div>
              <div className="text-xs text-neutral-400">
                {optResult.iterationsRun} iterations ·{" "}
                {optResult.claimType.replace("_", " ")}
                {optResult.promptVariantId &&
                  ` · Variant ID: ${optResult.promptVariantId.slice(0, 8)}…`}
              </div>
            </div>
          )}
        </CardContent>
