...
function DailyCostChart({ dailyCosts }: { dailyCosts: DailyCost[] }) {
  const [tooltip, setTooltip] = useState<{ day: DailyCost; x: number } | null>(
    null,
  );
  ...
  const maxDayCost = useMemo(() => {
    return Math.max(
      ...dailyCosts.map((d) =>
        Object.values(d.costs).reduce((s, v) => s + v, 0),
      ),
      0.01,
    );
  }, [dailyCosts]);
        ...
        <div className="absolute top-0 left-1/2 -translate-x-1/2 z-10 pointer-events-none bg-neutral-900 dark:bg-neutral-800 text-white text-xs rounded-lg shadow-lg p-3 min-w-[180px] border border-neutral-700">
          ...
          {ALL_EVENT_TYPES.map((et) => {
            ...
            return (
              <div key={et} className="flex justify-between gap-4">
                <span className={EVENT_COLORS[et].text}>
                  {EVENT_LABELS[et]}
                </span>
          ...
          <div className="border-t border-neutral-600 mt-1 pt-1 flex justify-between">
            ...
            <span className="font-semibold">
              $
              {Object.values(tooltip.day.costs)
                .reduce((s, v) => s + v, 0)
                .toFixed(3)}{" "}
              AUD
            </span>
...
export default function AiUsageDashboardPage() {
  ...
  const [eventTypeFilter, setEventTypeFilter] = useState<Set<EventTypeName>>(
    new Set(),
  );
  ...
  const filteredEventTypes = useMemo(() => {
    if (!data) return [];
    return data.byEventType.filter(
      (row) => eventTypeFilter.size === 0 || eventTypeFilter.has(row.eventType),
    );
  ...
  const filteredUsers = useMemo(() => {
    ...
    rows.sort((a, b) =>
      sortDesc ? b.totalCost - a.totalCost : a.totalCost - b.totalCost,
    );
    return rows;
  }, [data, billingFilter, sortDesc]);

  // Admin guard — show lock card, don't redirect.
  // Conditional render MUST come after hooks (react-hooks/rules-of-hooks).
  ...
  const handleExportCsv = () => {
    ...
    const rows = [
      ...
      ...filteredEventTypes.map((row) => {
        const pct =
          data.totalCostMtd > 0
            ? ((row.totalCost / data.totalCostMtd) * 100).toFixed(1)
            : "0.0";
        return [
        ...
        <>
          ...
          <Card className="bg-white dark:bg-neutral-900 border-neutral-200 dark:border-neutral-800">
            <CardHeader>
              ...
              <CardDescription>
                Last 30 days — stacked by event type
              </CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card className="bg-white dark:bg-neutral-900 border-neutral-200 dark:border-neutral-800">
            <CardHeader>
              ...
              <CardDescription>
                Costs by AI feature for selected month
              </CardDescription>
            </CardHeader>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-neutral-100 dark:border-neutral-800">
                      ...
                      <th className="text-right py-3 px-4 text-neutral-500 font-medium">
                        Events
                      </th>
                      <th className="text-right py-3 px-4 text-neutral-500 font-medium">
                        Units
                      ...
                      <th className="py-3 px-4 text-neutral-500 font-medium w-40">
                        % of Spend
                      </th>
                    </tr>
                  </thead>
                  ...
                  {filteredEventTypes.length > 0 && (
                    <tfoot>
                      <tr className="border-t border-neutral-200 dark:border-neutral-700">
                        <td
                          className="py-3 px-4 font-semibold text-neutral-900 dark:text-white"
                          colSpan={4}
                        >
                          Total
                        </td>
          ...
          <Card className="bg-white dark:bg-neutral-900 border-neutral-200 dark:border-neutral-800">
            <CardHeader>
              ...
              <CardDescription>
                ...
                {filteredUsers.length !== 1 ? "s" : ""} — sorted by total cost
              </CardDescription>
            </CardHeader>
            <CardContent className="p-0">
              <div className="overflow-x-auto">
