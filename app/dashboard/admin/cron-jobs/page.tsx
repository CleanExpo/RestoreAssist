    ...
    try {
      ...
      if (data.success) {
        setJobStates((prev) => ({
          ...
          [jobId]: {
            status: "success",
            message: `Completed with HTTP ${data.status}`,
          },
        }));
      } else {
        setJobStates((prev) => ({
          ...
          [jobId]: {
            status: "error",
            message: data.error ?? `HTTP ${data.status}`,
          },
        }));
      }
    } catch (err) {
        ...
        <CardContent className="pt-4 pb-4">
          <div className="flex items-start gap-3">
            ...
            <p className="text-sm text-neutral-600 dark:text-neutral-400">
              These jobs run automatically on a schedule. Use "Run Now" to
              trigger them manually for testing or to catch up on missed runs.
              Each job executes with the configured{" "}
                  ...
                  <div className="flex items-center gap-3 flex-shrink-0">
                    ...
                    <Button
                      ...
                      className={cn(
                        "gap-2",
                        !isRunning &&
                          "border-cyan-200 dark:border-cyan-800/60 text-cyan-600 dark:text-brand-tan hover:bg-cyan-50 dark:hover:bg-cyan-950/30",
                      )}
                    >
                      {isRunning ? (
                        <>
