...
export default function WebhookLogsPage() {
  ...
  async function handleRetry(event: WebhookEvent) {
    ...
    try {
      const res = await fetch(`/api/webhooks/logs/${event.id}/retry`, {
        method: "POST",
      });
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-neutral-900 dark:text-slate-50">
              Webhook Event Log
            </h1>
        ...
        {[
          {
            label: "Total Events",
            value: total,
            color: "text-neutral-900 dark:text-slate-50",
          },
          {
            label: "Processed",
          ...
          <Card
            key={stat.label}
            className="border border-neutral-200 dark:border-slate-800"
          >
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center px-4">
              <Activity
                size={40}
                className="text-neutral-300 dark:text-slate-700 mb-4"
              />
              <p className="text-neutral-500 dark:text-slate-400 text-sm max-w-sm">
                No webhook events recorded yet. Events appear here when
              ...
              <TableBody>
                {events.map((event) => (
                  ...
                  >
                    ...
                    <TableCell>
                      <Badge
                        className={`text-xs font-medium ${providerColor(event.provider)}`}
                      >
                        {event.provider === "QUICKBOOKS"
                          ? "QuickBooks"
                          : event.provider.charAt(0) +
                            event.provider.slice(1).toLowerCase()}
                      </Badge>
                    </TableCell>
                    ...
                    <TableCell>
                      <Badge
                        className={`text-xs font-medium ${statusColor(event.status)}`}
                      >
                        {event.status}
                      </Badge>
                    </TableCell>
      ...
      <Dialog
        open={!!payloadEvent}
        onOpenChange={(open) => !open && setPayloadEvent(null)}
      >
        <DialogContent className="max-w-2xl max-h-[80vh] flex flex-col">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-base">
              ...
              {payloadEvent && (
                <Badge
                  className={`ml-2 text-xs ${providerColor(payloadEvent.provider)}`}
                >
