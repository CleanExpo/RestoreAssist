...
export default function FormTemplateDetailPage({ params }: { params: { id: string } }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setTemplate((prev) => prev ? { ...prev, name: updated.name, description: updated.description, formType: updated.formType } : prev)
      setIsDirty(false)
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <h1 className="text-xl font-semibold text-slate-800">Template not found</h1>
          <p className="text-slate-500">This form template does not exist or you do not have access to it.</p>
          <Link
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="flex items-start gap-3">
          <div className="flex-1 min-w-0">
            <h1 className="text-2xl font-bold text-slate-900 truncate">{template.name}</h1>
            <div className="mt-2 flex flex-wrap items-center gap-2">
              ...
              <span className="text-xs text-slate-400">
                Created {new Date(template.createdAt).toLocaleDateString("en-AU", { day: "numeric", month: "short", year: "numeric" })}
              </span>
            </div>
          </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-100 px-6 py-4">
            <h2 className="text-sm font-semibold text-slate-700 uppercase tracking-wider">Template Details</h2>
          </div>
          <div className="p-6 space-y-5">
            ...
            {/* Form type */}
            <div className="space-y-1.5">
              <label className="block text-sm font-medium text-slate-700" htmlFor="template-form-type">
                Form Type
              </label>
              <select
        ...
        <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
          ...
          {template.questions.length === 0 ? (
            <div className="p-10 text-center">
              ...
              <p className="text-sm text-slate-400">No questions defined for this template</p>
            </div>
          ) : (
            <div className="divide-y divide-slate-100">
              ...
              <div className="grid grid-cols-[3rem_1fr_8rem_5rem] gap-3 px-6 py-2.5 bg-slate-50">
                ...
                <span className="text-xs font-medium text-slate-500 text-center">Required</span>
              </div>
              {/* Rows */}
              {template.questions.map((q) => (
                ...
                >
                  <span className="text-xs font-mono text-slate-400">{q.order}</span>
                  <span className="text-sm text-slate-800 truncate" title={q.questionText}>
                    {q.questionText}
                  </span>
                  <span
                    className={cn(
