...
import { Plus, Edit, Trash2, Search, X, FileText, Calendar, Layers } from "lucide-react"
...
const CLAIM_COLORS: Record<string, string> = {
  water_damage: "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400",
  fire_damage: "bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400",
  mould_remediation: "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400",
  storm_damage: "bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400",
...
export default function ScopeTemplatesPage() {
  ...
  // Delete confirmation
  const [deletingTemplate, setDeletingTemplate] = useState<ScopeTemplate | null>(null)
  ...
  const filtered = useMemo(() => {
    return templates.filter((t) => {
      ...
      const matchesType = claimTypeFilter ? t.claimType === claimTypeFilter : true
  ...
  async function handleDelete() {
    ...
    setDeleting(true)
    try {
      const res = await fetch(`/api/scope-templates/${deletingTemplate.id}`, { method: "DELETE" })
      if (res.ok) {
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className={cn(
          ...
          <h3 className={cn("text-base font-semibold mb-1", "text-neutral-900 dark:text-slate-50")}>
            {searchTerm || claimTypeFilter ? "No templates match your filters" : "No scope templates yet"}
          </h3>
          <p className={cn("text-sm max-w-xs mx-auto", "text-neutral-500 dark:text-slate-400")}>
        ...
        <>
          ...
          <div className={cn(
            ...
            <div className={cn(
              ...
              <h2 className={cn("text-lg font-semibold", "text-neutral-900 dark:text-slate-50")}>
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-5">
              ...
              <div className="space-y-1">
                <label className={cn("text-sm font-medium", "text-neutral-700 dark:text-slate-300")}>
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  onChange={(e) => setFormData((f) => ({ ...f, claimType: e.target.value }))}
                  className={cn(
                    "w-full px-3 py-2 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-cyan-500/50",
                    "bg-neutral-50 dark:bg-slate-800",
                    "border border-neutral-300 dark:border-slate-700",
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className={cn(
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className={cn("text-base font-semibold", "text-neutral-900 dark:text-slate-50")}>
                    Delete Template
                  </h3>
  ...
  return (
    <div className={cn(
      ...
      <div className="flex items-center gap-4 text-xs">
        <span className={cn("flex items-center gap-1", "text-neutral-400 dark:text-slate-500")}>
          <Layers size={12} />
