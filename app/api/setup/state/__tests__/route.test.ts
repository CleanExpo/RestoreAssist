...
describe.skipIf(!process.env.DATABASE_URL)('PATCH /api/setup/state — tenant isolation', () => {
  let userAId = '';
  let orgAId = '';
  let userBId = '';
  let orgBId = '';
  ...
  beforeAll(async () => {
    const a = await prisma.user.create({ data: { email: `iso-a-${Date.now()}@test.com` } });
    userAId = a.id;
    const oa = await prisma.organization.create({
      data: { name: 'Iso A Co', ownerId: a.id, legalName: 'Iso A', state: 'NSW', primaryColor: '#aaaaaa' },
    });
    orgAId = oa.id;
    await prisma.user.update({ where: { id: a.id }, data: { organizationId: oa.id } });

    const b = await prisma.user.create({ data: { email: `iso-b-${Date.now()}@test.com` } });
    userBId = b.id;
    const ob = await prisma.organization.create({
      data: { name: 'Iso B Co', ownerId: b.id, legalName: 'Iso B', state: 'VIC', primaryColor: '#bbbbbb' },
    });
    orgBId = ob.id;
    await prisma.user.update({ where: { id: b.id }, data: { organizationId: ob.id } });

    const c = await prisma.user.create({ data: { email: `iso-c-${Date.now()}@test.com` } });
    userCId = c.id;
  });

  afterAll(async () => {
    await prisma.organization.deleteMany({ where: { id: { in: [orgAId, orgBId] } } }).catch(() => {});
    await prisma.user.deleteMany({ where: { id: { in: [userAId, userBId, userCId] } } }).catch(() => {});
    await prisma.$disconnect();
  });
  ...
  it("user B's PATCH mutates only B's org, never A's", async () => {
    (getServerSession as any).mockResolvedValue({ user: { id: userBId, email: 'b@t.com' } });
    const req = new Request('http://localhost/api/setup/state', {
      method: 'PATCH',
    ...
    const a = await prisma.organization.findUnique({ where: { id: orgAId }, select: { primaryColor: true } });
    const b = await prisma.organization.findUnique({ where: { id: orgBId }, select: { primaryColor: true } });
  ...
  it('an authenticated user who owns no org cannot patch anyone (404)', async () => {
    (getServerSession as any).mockResolvedValue({ user: { id: userCId, email: 'c@t.com' } });
    const req = new Request('http://localhost/api/setup/state', {
      method: 'PATCH',
