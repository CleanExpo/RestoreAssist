...
describe("POST /api/setup/hydrate", () => {
  ...
  beforeAll(async () => {
    const u = await prisma.user.create({
      data: { email: `hydrate-${Date.now()}@test.com` },
    });
    testUserId = u.id;
    const o = await prisma.organization.create({
      data: { name: "Hydrate Test Co", ownerId: u.id },
    });
    testOrgId = o.id;
  ...
  beforeEach(() => {
    ...
    mockGetServerSession.mockResolvedValue({
      user: { id: testUserId, email: "hydrate-test@test.com" },
    });
  });
  ...
  it("creates 3 HydrationJob rows + sets setupStartedAt on valid ABN", async () => {
    ...
    const res = await POST(
      makeReq({ abn: "53004085616", website: "https://example.com" }),
    );
