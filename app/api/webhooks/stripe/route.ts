...
async function hasWebhookEventModel(): Promise<boolean> {
  ...
  try {
    if (
      ...
      typeof prisma.stripeWebhookEvent.findFirst === "function"
    ) {
  ...
  if (modelAvailable) {
    try {
      ...
      if (err.code === "P2002") {
        ...
        console.log(
          `Webhook event ${event.id} already claimed (status=${existing?.status}), skipping`,
        );
...
async function processInvoicePayment(session: Stripe.Checkout.Session) {
  ...
  if (existingPayment) {
    console.log(
      "⚠️ INVOICE PAYMENT ALREADY PROCESSED (checkout session):",
      session.id,
    );
    return;
  }
  ...
  console.log(
    `✅ INVOICE PAYMENT PROCESSED: +$${(amountPaid / 100).toFixed(2)} for invoice ${invoice.invoiceNumber}`,
  );
}
...
async function processAddonPurchase(session: Stripe.Checkout.Session) {
  ...
  if (!addon) {
    console.error(
      "❌ ADD-ON ERROR: Addon not found in PRICING_CONFIG for key:",
      addonKey,
    );
    return;
  }
  ...
  try {
    if (
      prisma.addonPurchase &&
      typeof prisma.addonPurchase.findUnique === "function"
    ) {
  ...
  if (canUsePurchaseTable) {
    try {
      ...
      if (
        purchaseError.code === "P2002" ||
        purchaseError.message?.includes("Unique constraint")
      ) {
        console.log(
          "⚠️ ADD-ON ALREADY PROCESSED (unique constraint):",
          session.id,
        );
        return;
      }
      console.warn(
        "⚠️ Could not create AddonPurchase record:",
  ...
  console.log(
    `✅ ADD-ON PROCESSED: +${addonReports} reports for user ${userId}`,
  );
}
...
async function processSubscriptionCheckout(session: Stripe.Checkout.Session) {
  ...
  if (!userId && !session.customer_email) {
    console.error(
      "❌ SUBSCRIPTION ERROR: No userId or email in checkout session",
    );
    return;
  }
  ...
  if (userId) {
    ...
    console.log(
      `✅ SUBSCRIPTION ACTIVATED for user ${userId}${isFirstSubscription ? " (signup bonus applied)" : ""}`,
    );
    ...
    if (userDetails) {
      provisionWorkspace({
        ...
        console.error(
          `[Stripe webhook] provisionWorkspace failed for user ${userId}:`,
          err,
        ),
      );
    }
    ...
    if (userByEmail) {
      ...
      console.log(
        `✅ SUBSCRIPTION ACTIVATED for email ${session.customer_email}${isFirstSubscription ? " (signup bonus applied)" : ""}`,
      );
      ...
      provisionWorkspace({
        ...
        console.error(
          `[Stripe webhook] provisionWorkspace failed for email ${session.customer_email}:`,
          err,
        ),
      );
    }
...
async function handleSubscriptionCreated(event: Stripe.Event) {
  ...
  const isFirstSubscription =
    !signupBonusApplied &&
    (userByCustomer.subscriptionStatus !== "ACTIVE" ||
      !userByCustomer.lastBillingDate);
  ...
  console.log(
    `✅ SUBSCRIPTION CREATED for customer ${subscription.customer}${isFirstSubscription ? " (signup bonus applied)" : ""}`,
  );
}
...
async function handleSubscriptionUpdated(event: Stripe.Event) {
  ...
  console.log(
    `✅ SUBSCRIPTION UPDATED: ${subscription.id} -> ${subscription.status} (mapped to ${mappedStatus})`,
  );
}
...
async function handlePaymentIntentSucceeded(event: Stripe.Event) {
  ...
  if (paymentIntent.metadata?.type === "invoice") {
    ...
    if (existingPayment) {
      console.log(
        "⚠️ INVOICE PAYMENT ALREADY PROCESSED (payment intent):",
        paymentIntent.id,
      );
      return;
    }
    ...
    console.log(
      `✅ INVOICE PAYMENT PROCESSED (payment intent): +$${(amountPaid / 100).toFixed(2)} for invoice ${invoice.invoiceNumber}`,
    );
    return;
  }
  ...
  try {
    ...
    if (existingPurchase) {
      console.log(
        "⚠️ ADD-ON ALREADY PROCESSED (payment intent):",
        paymentIntent.id,
      );
      return;
    }
    ...
    console.log(
      `✅ ADD-ON PROCESSED (payment intent): +${addonReports} reports for user ${userId}`,
    );
  } catch (error: any) {
    if (error.code === "P2002") {
      console.log(
