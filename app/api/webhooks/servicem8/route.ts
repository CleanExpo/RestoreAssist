...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!webhookSecret) {
      console.error('[ServiceM8 Webhook] SERVICEM8_WEBHOOK_SECRET not configured')
    ...
    console.log(`[ServiceM8 Webhook] Received ${entries.length} entries for service: ${service}`)

    // Map ServiceM8 service to standard event type
    function mapEventType(svc: string, entry: Record<string, unknown>): string | null {
      const active = entry.active === 1 || entry.active === '1'
      switch (svc) {
        case 'GeoActivity':       return 'job.status_updated'
        case 'Job':               return active ? 'job.updated' : 'job.cancelled'
        case 'Client':            return 'customer.updated'
        case 'JobContact':        return 'contact.updated'
        default:                  return null
      }
    }
    ...
    for (const entry of entries as Record<string, unknown>[]) {
      try {
        ...
        if (existing) {
          console.log(`[ServiceM8 Webhook] Duplicate event for ${entryUuid} — skipping`)
          continue
        }
        ...
        console.log(`[ServiceM8 Webhook] Queued ${webhookEvent.id}: ${standardEventType} for ${entryUuid}`)
      } catch (err) {
        console.error('[ServiceM8 Webhook] Failed to queue entry:', err)
      }
    }

    return NextResponse.json({ success: true, processed: queuedEvents.length, eventIds: queuedEvents })
  } catch (error) {
