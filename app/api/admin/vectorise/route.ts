...
export async function POST(request: NextRequest) {
  ...
  const provider = (body.provider ?? (process.env.OPENAI_API_KEY ? "openai" : "hash-fallback")) as "openai" | "hash-fallback"
  ...
  const toEmbed = allJobs.filter((j) => j.embeddedAt === null).slice(0, batchSize)
