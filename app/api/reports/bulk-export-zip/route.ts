...
async function generateReportPDF(reportId: string): Promise<Buffer | null> {
  try {
    ...
    const response = await fetch(
      `${baseUrl}/api/reports/${reportId}/download?type=summary`,
      {
        method: "GET",
        headers: {
    ...
    if (!response.ok) {
      console.error(
        `Failed to generate PDF for report ${reportId}: ${response.status}`,
      );
      return null;
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      !pdfType ||
      !["basic", "enhanced", "forensic", "inspection"].includes(pdfType)
    ) {
      return NextResponse.json(
        {
          error: "Invalid PDF type",
          message:
            "pdfType must be one of: basic, enhanced, forensic, inspection",
        },
        { status: 400 },
      );
    }
      ...
      const batchResults = await Promise.allSettled(
        batch.map(async (report) => {
          try {
            ...
            if (!pdfBuffer) {
              errors.push(
                `Failed to generate PDF for report ${report.reportNumber}`,
              );
              return null;
            }
