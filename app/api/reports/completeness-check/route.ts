...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    sections.push({
      ...
      score: clientIssues.length === 0 ? 100 : clientIssues.length === 1 ? 66 : 33,
      status: clientIssues.length === 0 ? 'complete' : clientIssues.length <= 1 ? 'partial' : 'missing',
      issues: clientIssues,
    })
    ...
    if (!insp) {
      ...
      if (!insp.environmentalData) inspIssues.push('Environmental data not recorded')
      if (insp.moistureReadings.length === 0) inspIssues.push('No moisture readings recorded')
      if (insp.affectedAreas.length === 0) inspIssues.push('No affected areas defined')
    }
    sections.push({
      name: 'Inspection Data',
      score: inspIssues.length === 0 ? 100 : Math.max(0, 100 - inspIssues.length * 25),
      status: inspIssues.length === 0 ? 'complete' : inspIssues.length <= 1 ? 'partial' : 'missing',
      issues: inspIssues,
    })

    // --- IICRC Classification ---
    const classIssues: string[] = []
    if (!insp || insp.classifications.length === 0) classIssues.push('IICRC classification not run')
    sections.push({
    ...
    if (!insp || insp.scopeItems.length === 0) scopeIssues.push('No scope items generated')
    sections.push({
      name: 'Scope of Works',
    ...
    if (!insp || insp.costEstimates.length === 0) costIssues.push('No cost estimates calculated')
    sections.push({
      name: 'Cost Estimates',
