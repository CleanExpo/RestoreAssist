...
Analyze this document thoroughly and extract every piece of available information, regardless of the report format or system that generated it.`,
      ...
      try {
        ...
        if (jsonMatch) {
          ...
          throw new Error(
            "No JSON found in response. The AI may have returned an unexpected format.",
          );
        }
      } catch (parseError: any) {
        return NextResponse.json(
        ...
        return NextResponse.json(
          {
            ...
            details:
              "The API key is invalid or expired. Please check your integration settings.",
            suggestion:
              "Go to Settings > Integrations and verify your Anthropic API key is correct and active.",
          },
          { status: 401 },
        );
      }
        ...
        return NextResponse.json(
          {
            ...
            suggestion:
              "Please check your API key permissions or contact support.",
          },
          { status: 403 },
        );
      }
        ...
        return NextResponse.json(
          {
            ...
            details:
              "Failed to connect to the analysis service after multiple retry attempts.",
            suggestion:
        ...
        return NextResponse.json(
          {
            ...
            suggestion:
              "The PDF may be too large or complex. Try uploading a smaller file, or split the document into smaller sections. For very large reports, consider processing sections separately.",
            fileSize: fileSizeMB + "MB",
          },
          { status: 408 },
        );
        ...
        return NextResponse.json(
          {
            ...
            suggestion:
              "Please ensure the PDF is: (1) Not password-protected, (2) Not corrupted, (3) A valid PDF format. Try opening the PDF in a PDF viewer first to verify it's valid.",
            fileName: file.name,
          },
          { status: 400 },
        );
