...
export async function PATCH(request: NextRequest) {
  try {
    ...
    // 3. Parse request
    const { ids, status, reason }: BulkStatusUpdateRequest =
      await request.json();
      ...
      return NextResponse.json(
        {
          ...
          message:
            "status must be one of: DRAFT, PENDING, APPROVED, COMPLETED, ARCHIVED",
        },
        { status: 400 },
      );
    }
    ...
    try {
      ...
      if (reportsUpdated > 0) {
        ...
        if (status.toUpperCase() === "COMPLETED") {
          notifyAdminOfCompletedReports(session.user.id, ownedIds).catch(
            (error) => {
      ...
      return NextResponse.json(
        {
          ...
          failed:
            ...
            (ids.length - reportsUpdated - unauthorizedIds.length),
 ...
 */
async function notifyAdminOfCompletedReports(
  userId: string,
  reportIds: string[],
) {
  ...
  for (const report of reports) {
    ...
    sendReportCompletedEmail({
      ...
      console.error(
        `Failed to send report completion email for ${report.id}:`,
        err,
      );
    });
