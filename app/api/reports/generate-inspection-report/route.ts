...
## Client Information`;
  ...
  if (
    ...
    inspectionData.moistureReadings.forEach((reading: any) => {
      if (
        hasValue(reading.location) ||
        hasValue(reading.surfaceType) ||
        hasValue(reading.moistureLevel)
      ) {
  ...
  if (
    ...
    inspectionData.affectedAreas.length > 0
  ) {
    prompt += `\n\n## Affected Areas`;
  ...
  // Hazard Profile
  if (
    hasValue(report.methamphetamineScreen) ||
    hasValue(report.biologicalMouldDetected)
  ) {
    prompt += `\n\n## Hazard Profile`;
  ...
  // Timeline Estimation
  if (
    hasValue(report.phase1Start) ||
    hasValue(report.phase2Start) ||
    hasValue(report.phase3Start)
  ) {
    prompt += `\n\n## Timeline Estimation`;
  ...
  if (
    businessInfo &&
    (hasValue(businessInfo.businessName) ||
      hasValue(businessInfo.businessAddress))
  ) {
    prompt += `\n\n## Business Information`;
...
### Moisture Readings
${
  inspectionData.moistureReadings && inspectionData.moistureReadings.length > 0
    ? inspectionData.moistureReadings
        .map(
          (reading: any) => `
...
### Affected Areas
${
  inspectionData.affectedAreas && inspectionData.affectedAreas.length > 0
    ? inspectionData.affectedAreas
        .map(
          (area: any) => `
...
${
  inspectionData.classifications && inspectionData.classifications.length > 0
    ? inspectionData.classifications
        .map(
          (cls: any) => `
...
### Scope Items
${
  inspectionData.scopeItems && inspectionData.scopeItems.length > 0
...
### Cost Estimates
${
  inspectionData.costEstimates && inspectionData.costEstimates.length > 0
...
function buildVisualReportData(data: {
  ...
  const dryingIndex =
    psychrometricAssessment?.dryingPotential?.dryingIndex || 33.6;
  const dryingStatus =
    psychrometricAssessment?.dryingPotential?.status || "Fair";
  ...
  if (scopeAreas && scopeAreas.length > 0) {
    ...
    scopeAreas.forEach((area: any, idx: number) => {
      const roomMaterials =
        area.materials ||
        materials[idx] ||
        (materials.length > 0 ? materials[0] : "Various");
  ...
  return {
    header: {
      ...
      claimRef:
        report.claimReferenceNumber || report.reportNumber || "INS-2025-001234",
      location: report.propertyPostcode
        ? `${stateInfo?.name || "Brisbane"}, ${stateInfo?.code || "QLD"}`
        : "",
