...
export async function POST(
  ...
  // RA-1266: detailed report generation is one of the most expensive
  // endpoints (multi-thousand-token Claude call + pdf-lib CPU). Retry
  // without idempotency burns both.
  return withIdempotency(request, userId, async () => {
    try {
      ...
      const wrapText = (
        ...
        words.forEach((word) => {
          ...
          try {
            ...
            if (sanitizedWord) {
              currentLine =
                currentLine + (currentLine ? " " : "") + sanitizedWord;
            }
          }
        });
      ...
      const reportDetails = [
        ...
        {
          label: "Order Number:",
          value: report.reportNumber || report.id.slice(0, 8),
        },
        {
          label: "Date On-Site:",
          value: formatDate(report.inspectionDate || report.createdAt),
      ...
      reportDetails.forEach((detail, index) => {
        ...
        const valueWidth = isLeftCol
          ? colWidth - labelWidth - 20
          : width - valueX - 50;
      ...
      const contactDetails = [
        {
          label: "Phone Number (Client):",
          value: report.client?.phone || "N/A",
          checkbox: false,
        },
        {
        ...
        {
          label: "Present @ Inspection:",
          value: report.clientName || "N/A",
          checkbox: false,
        },
        {
          label: "Claim Covered:",
        ...
        {
          label: "Cause of Loss:",
          value: report.sourceOfWater || "N/A",
          checkbox: false,
        },
      ];
      ...
      contactDetails.forEach((detail, index) => {
        ...
        if (detail.checkbox) {
          const checkboxX = isLeftCol
            ? 50 + labelWidth
            : 50 + colWidth + labelWidth;
      ...
      if (
        parsedReport.moistureReadings &&
        Array.isArray(parsedReport.moistureReadings) &&
        parsedReport.moistureReadings.length > 0
      ) {
        ...
        moistureReadings = [
          {
            ...
            areaTested:
              ...
              "Gypsum board walls, carpet, hardwood flooring, concrete subfloor",
            benchmark: "N/A",
          },
          {
      ...
      const assessmentFields = [
        {
          label: "Main Roof Damaged:",
          value: report.structuralDamage ? "Yes" : "No",
        },
      ...
      assessmentFields.forEach((field) => {
        ...
        if (
          field.label.includes("Main Roof") ||
          field.label.includes("Asbestos")
        ) {
          const checkboxX = 180;
      ...
      const damageQuestions = [
        report.contentsDamage
          ? `Has there been any content damaged? If so what? ${report.contentsDamage}`
          : "Has there been any content damaged? If so what? Make sure to take photos and provide detailed list including make,model and serial number where possible",
        report.structuralDamage
          ? `Is there structural damage to premises? If so, what? ${report.structuralDamage}`
          : "Is there structural damage to premises? If so, what? Take photos",
        `Where has the water travelled? ${scope?.siteVariables?.floors || report.affectedArea ? `${report.affectedArea} sqm affected` : ""}`,
        report.microbialGrowth
          ? `Has water wicked up walls etc? ${report.microbialGrowth}`
          : "Has water wicked up walls etc?",
        ...
        const areas = [
          {
            ...
            size:
              ...
              (report.affectedArea ? `${report.affectedArea} sqm` : "N/A"),
            flooringType:
              scope.siteVariables?.materials ||
              report.structuralDamage ||
              "N/A",
            subFloor:
              scope.siteVariables?.subFloor || report.structuralDamage || "N/A",
            age:
              scope.siteVariables?.age ||
              (report.completionDate
                ? `Built ${new Date(report.completionDate).getFullYear()}`
                : "N/A"),
            insulation:
              scope.siteVariables?.insulation ||
              report.structuralDamage ||
              "N/A",
            underlay: scope.siteVariables?.underlay || "N/A",
            resNr:
              scope.siteVariables?.resNr ||
              (report.hazardType === "Water" ? "Res" : "NR"),
          },
        ];
      ...
      if (report.affectedArea && report.affectedArea > 50) {
        recommendations.push(
          "Implement comprehensive drying strategy with multiple dehumidifiers and air movers to address the extensive affected area.",
        );
      }

      if (report.microbialGrowth) {
        recommendations.push(
          "Immediate microbial remediation required to prevent further contamination and ensure occupant safety.",
        );
      }

      if (report.hvacAffected) {
        recommendations.push(
          "HVAC system requires inspection and cleaning to prevent cross-contamination.",
        );
      }

      if (estimate && estimate.totalIncGST) {
        recommendations.push(
          `Estimated remediation cost: $${estimate.totalIncGST.toFixed(2)} including GST. This estimate is based on current market rates.`,
        );
      }

      recommendations.push(
        "Continue monitoring until dry standards per IICRC S500 guidelines are met.",
      );
      ...
      // Date
      const dateText = report.inspectionDate
...
function generateComprehensiveAssessment(
  ...
  if (
    ...
    Array.isArray(report.moistureReadings) &&
    report.moistureReadings.length > 0
  ) {
    ...
    report.moistureReadings.forEach((reading: any, index: number) => {
      ...
      const parts: string[] = [];
      if (reading.moistureContent || reading.wme)
        parts.push(`WME: ${reading.moistureContent || reading.wme}`);
      if (reading.relativeHumidity || reading.rh)
        parts.push(`RH: ${reading.relativeHumidity || reading.rh}%`);
      if (reading.temperature || reading.airTemp)
        parts.push(`Temp: ${reading.temperature || reading.airTemp}C`);
  ...
  if (
    ...
    Array.isArray(report.psychrometricReadings) &&
    report.psychrometricReadings.length > 0
  ) {
  ...
  if (
    ...
    report.dryingPlan ||
    report.dehumidificationCapacity ||
    report.airmoversCount
  ) {
  ...
  if (scope) {
    ...
    if (scope.complianceNotes) {
      ...
      const notes = scope.complianceNotes
        .split(/[-•]/)
  ...
  if (estimate) {
    ...
    if (
      ...
      estimate.lineItems.forEach((item: any, index: number) => {
        ...
        assessment +=
          `${index + 1}. ${desc} ${qty} ${rate} ${subtotal}\n`
            .replace(/\s+/g, " ")
            .trim() + "\n";
      });
      assessment += "\n";
    ...
    // Summary totals
    const totals: string[] = [];
    if (estimate.labourSubtotal)
      totals.push(`Labour: $${estimate.labourSubtotal.toFixed(2)}`);
    if (estimate.equipmentSubtotal)
      totals.push(`Equipment: $${estimate.equipmentSubtotal.toFixed(2)}`);
    if (estimate.chemicalsSubtotal)
      totals.push(`Chemicals: $${estimate.chemicalsSubtotal.toFixed(2)}`);
    if (estimate.subcontractorSubtotal)
      totals.push(
        `Subcontractors: $${estimate.subcontractorSubtotal.toFixed(2)}`,
      );
    if (totals.length > 0) {
    ...
    if (estimate.overheads)
      assessment += `• Overheads: $${estimate.overheads.toFixed(2)}\n`;
    if (estimate.profit)
      assessment += `• Profit: $${estimate.profit.toFixed(2)}\n`;
    if (estimate.contingency)
      assessment += `• Contingency: $${estimate.contingency.toFixed(2)}\n`;
    if (estimate.subtotalExGST)
      assessment += `• Subtotal Ex GST: $${estimate.subtotalExGST.toFixed(2)}\n`;
  ...
  if (
    ...
    if (report.safetyPlan)
      assessment += `• Safety Plan: ${report.safetyPlan}\n`;
    if (report.containmentSetup)
      assessment += `• Containment: ${report.containmentSetup}\n`;
    if (report.decontaminationProcedures)
      assessment += `• Decontamination: ${report.decontaminationProcedures}\n`;
    if (report.postRemediationVerification)
      assessment += `• Verification: ${report.postRemediationVerification}\n`;
    assessment += "\n";
  }
