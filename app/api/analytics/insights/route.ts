...
export async function GET(request: NextRequest) {
  try {
    ...
    const currentWithCost = currentReports.map((r) => ({
      ...r,
      revenue: cost(r),
    }));
    const previousWithCost = previousReports.map((r) => ({
      ...r,
      revenue: cost(r),
    }));

    const totalCurrentRevenue = currentWithCost.reduce(
      (s, r) => s + r.revenue,
      0,
    );
    const totalPrevRevenue = previousWithCost.reduce(
      (s, r) => s + r.revenue,
      0,
    );
    ...
    const topHazardEntry = Array.from(hazardCounts.entries()).sort(
      (a, b) => b[1] - a[1],
    )[0];
    ...
    completedCurrent.forEach((r) => {
      ...
      const hours =
        (completionDate.getTime() - r.createdAt.getTime()) / (1000 * 60 * 60);
    ...
    const hazardAvgHours = Array.from(completionHoursByHazard.entries()).map(
      ([hazard, hours]) => ({
        hazard,
    ...
    const slowestHazards = [...hazardAvgHours]
      .sort((a, b) => b.avgHours - a.avgHours)
      .slice(0, 5);
    const fastestHazards = [...hazardAvgHours]
      .filter((h) => h.count >= 2)
        ...
        const reportChangePct =
          prev.reports === 0
            ? cur.reports > 0
              ? 100
              : 0
            : ((cur.reports - prev.reports) / prev.reports) * 100;
        return {
          name,
          currentRevenue: cur.revenue,
          currentReports: cur.reports,
          revenueChangePct: Math.round(revenueChange),
          reportChangePct: Math.round(reportChangePct),
        };
      })
    ...
    const revenueTrend =
      revenueChangePct > 5 ? "up" : revenueChangePct < -5 ? "down" : "stable";
    ...
    const hazardSentence =
      topHazard !== "N/A"
        ? `${topHazard} leads with ${topHazardCount} report${topHazardCount !== 1 ? "s" : ""}.`
        : "";
    ...
    const summary = [revenueSentence, hazardSentence, turnaroundSentence]
      .filter(Boolean)
      .join(" ");
    ...
    return NextResponse.json({
      ...
      topGrowingClients,
      slowestHazards: slowestHazards.map((h) => ({
        hazard: h.hazard,
        avgHours: Math.round(h.avgHours * 10) / 10,
        count: h.count,
      })),
      fastestHazards: fastestHazards.map((h) => ({
        hazard: h.hazard,
        avgHours: Math.round(h.avgHours * 10) / 10,
        count: h.count,
      })),
