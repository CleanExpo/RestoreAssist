...
async function handleExcelExport(
  ...
  summarySheet.addRows([
    {
      ...
      value: `${formatDate(fromDate)} to ${formatDate(toDate)}`,
    },
    ...
    { metric: "Total Revenue", value: formatCurrency(totalRevenue) },
    {
      ...
      value: formatCurrency(
        reports.length > 0 ? totalRevenue / reports.length : 0,
      ),
    },
  ]);
  ...
  const hazardRows = Array.from(hazardCounts.entries()).map(
    ([type, count]) => ({
      type,
      count,
...
function handlePDFExport(
  ...
  if (data.length > 0) {
    ...
    if (data.length > 20) {
      yPosition += 5;
      doc.text(`... and ${data.length - 20} more reports`, 20, yPosition);
    }
  }
