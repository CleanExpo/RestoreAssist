  ...
  try {
    ...
    const avgCompletionTime =
      turnaroundTimes.length > 0
        ? turnaroundTimes.reduce((sum, h) => sum + h, 0) /
    ...
    const trendMap = new Map<
      string,
      { reports: number; revenue: number; date: Date }
    >();
    ...
    reportsWithCost.forEach((report) => {
      if (report.state) {
        stateRevenue.set(
          report.state,
          (stateRevenue.get(report.state) || 0) + (report.cost || 0),
        );
      }
    });
    ...
    const previousStartDate = new Date(
      currentStartDate.getTime() - periodDuration,
    );
    ...
    const prevAvgReportValue =
      prevTotalReports > 0 ? prevTotalRevenue / prevTotalReports : 0;
    ...
    const prevAvgCompletion =
      prevTurnaroundTimes.length > 0
        ? prevTurnaroundTimes.reduce((sum, h) => sum + h, 0) /
          prevTurnaroundTimes.length
        : 0;
    ...
    const periodComparison = {
      ...
      previous: {
        reports: prevTotalReports,
        revenue: Math.round(prevTotalRevenue),
      },
      changes: {
