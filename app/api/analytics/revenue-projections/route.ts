...
export async function GET(request: NextRequest) {
  try {
    ...
    if (userIdParam && userIdParam !== session.user.id) {
      // JWT role claim is stale (CLAUDE.md rule 1) — gate on the DB-fresh role
      // so a demoted admin/manager loses cross-user revenue visibility at once.
    ...
    if (sortedDates.length === 0 || reports.length === 0) {
      ...
      const historicalForChart: Array<{
        date: string;
        revenue: number;
        isProjected: boolean;
      }> = [];
    ...
    const projectedTotals = {
      days30: projectedData.slice(0, 30).reduce((sum, p) => sum + p.revenue, 0),
      days60: projectedData.slice(0, 60).reduce((sum, p) => sum + p.revenue, 0),
      days90: projectedData.slice(0, 90).reduce((sum, p) => sum + p.revenue, 0),
    };
