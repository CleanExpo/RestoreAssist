...
function buildSummaryFromResults(
  ...
  for (const r of results) {
    ...
    totalMissingElements.australianStandards +=
      r.missingElements?.australianStandards ?? 0;
  ...
  return {
    ...
    averageScores: {
      completeness: n
        ? results.reduce((s, r) => s + (r.scores?.completeness ?? 0), 0) / n
        : 0,
      compliance: n
        ? results.reduce((s, r) => s + (r.scores?.compliance ?? 0), 0) / n
        : 0,
      standardization: n
        ? results.reduce((s, r) => s + (r.scores?.standardization ?? 0), 0) / n
        : 0,
      scopeAccuracy: n
        ? results.reduce((s, r) => s + (r.scores?.scopeAccuracy ?? 0), 0) / n
        : 0,
      billingAccuracy: n
        ? results.reduce((s, r) => s + (r.scores?.billingAccuracy ?? 0), 0) / n
        : 0,
    },
    totalEstimatedMissingRevenue: results.reduce(
      (s, r) => s + (r.estimatedMissingRevenue ?? 0),
      0,
    ),
    topIssues: results
      .flatMap((r) => r.issues ?? [])
      .reduce((acc: any[], issue: any) => {
        const existing = acc.find(
          (i: any) => i.elementName === issue.elementName,
        );
        if (existing) {
          existing.count++;
