/**
 * RA-6920 B1 — SERVICE_CRM add-on gate on the DR-NRPG connect surface.
 *
 * POST /api/dr-nrpg/connect is gated by requireAddon(SERVICE_CRM): a
...
vi.mock("@/lib/prisma", () => ({
  prisma: {
    drNrpgIntegration: { upsert: vi.fn(), deleteMany: vi.fn(), findUnique: vi.fn() },
  },
}));
...
const mockUpsert = (
  prisma as unknown as { drNrpgIntegration: { upsert: ReturnType<typeof vi.fn> } }
).drNrpgIntegration.upsert;

function makePost(body: object): NextRequest {
  return new NextRequest("http://localhost/api/dr-nrpg/connect", {
    method: "POST",
...
describe("RA-6920 B1 — SERVICE_CRM add-on gate on /api/dr-nrpg/connect", () => {
  it("returns 402 when the workspace is not entitled to SERVICE_CRM", async () => {
    ...
    const res = await POST(makePost({ drNrpgApiKey: "k" }));
  ...
  it("passes the gate for an entitled (or grandfathered) workspace and proceeds", async () => {
    ...
    const res = await POST(
      makePost({ drNrpgApiKey: "real-key", drNrpgBaseUrl: "https://api.dr-nrpg.com.au" }),
    );
