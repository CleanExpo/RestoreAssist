...
export async function GET(
  ...
  try {
    ...
    if (report.status === 'DRAFT') {
      return NextResponse.json({ error: 'Report is not available for download' }, { status: 403 })
    }

    if (!report.detailedReport) {
      return NextResponse.json({ error: 'Report PDF content not available' }, { status: 404 })
    }
