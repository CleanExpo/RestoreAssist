...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: "Failed to fetch scope templates", details: process.env.NODE_ENV === "development" ? error?.message : undefined },
      { status: 500 }
    )
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      _count: { items: template.items ? JSON.parse(template.items).length : 0 },
    }, { status: 201 })
  } catch (error: any) {
    console.error("[scope-templates] POST error:", error)
    return NextResponse.json(
      { error: "Failed to create scope template", details: process.env.NODE_ENV === "development" ? error?.message : undefined },
      { status: 500 }
    )
  }
}
