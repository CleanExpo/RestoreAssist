...
export async function POST(
  ...
  try {
    ...
    // ── Sprint G: Evidence-based submission gate (RA-401) ─────────────────────
    // Validates that all mandatory evidence items have been captured per the
    // claim-type workflow definition. This is the enforcement mechanism that
    // ensures "a technician cannot submit when core evidence is missing."
    const body = await request
      .clone()
      .json()
      .catch(() => ({}));
    const claimType = body.claimType || "water_damage";
    const evidenceValidation = await validateSubmission(id, claimType);

    if (!evidenceValidation.passed) {
      return NextResponse.json(
        {
          error: "Inspection cannot be submitted — required evidence missing",
          completionPercentage: evidenceValidation.completionPercentage,
          totalRequired: evidenceValidation.totalRequired,
          totalCaptured: evidenceValidation.totalCaptured,
          gaps: evidenceValidation.gaps,
