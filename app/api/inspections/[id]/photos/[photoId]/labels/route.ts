...
const VALID_DAMAGE_CLASSES: DamageClass[] = ['CLASS_1', 'CLASS_2', 'CLASS_3', 'CLASS_4'];
const VALID_ROOM_TYPES: RoomType[] = [
  'KITCHEN','BATHROOM','LAUNDRY','TOILET','BEDROOM','LIVING','DINING',
  'HALLWAY','GARAGE','ROOF_SPACE','SUBFLOOR','BASEMENT','COMMERCIAL_OFFICE',
  'COMMERCIAL_WAREHOUSE','COMMON_AREA','EXTERNAL','OTHER',
];
...
export async function PATCH(
  ...
  try {
    ...
    if (body.damageCategory !== undefined &&
        !VALID_DAMAGE_CATEGORIES.includes(body.damageCategory)) {
      errors.push(`damageCategory must be one of: ${VALID_DAMAGE_CATEGORIES.join(', ')}`);
    }
    if (body.damageClass !== undefined &&
        !VALID_DAMAGE_CLASSES.includes(body.damageClass)) {
      errors.push(`damageClass must be one of: ${VALID_DAMAGE_CLASSES.join(', ')}`);
    }
    ...
    if (body.moistureSource !== undefined &&
        !VALID_MOISTURE_SOURCES.includes(body.moistureSource)) {
      errors.push(`moistureSource must be one of: ${VALID_MOISTURE_SOURCES.join(', ')}`);
    }
    if (body.affectedMaterial !== undefined) {
      if (!Array.isArray(body.affectedMaterial)) {
        ...
        const invalid = body.affectedMaterial.filter(
          (m: string) => !VALID_AFFECTED_MATERIALS.includes(m as AffectedMaterial)
        );
        if (invalid.length > 0) errors.push(`Invalid affectedMaterial values: ${invalid.join(', ')}`);
      }
    }
    if (body.surfaceOrientation !== undefined &&
        !VALID_SURFACE_ORIENTATIONS.includes(body.surfaceOrientation)) {
      errors.push(`surfaceOrientation must be one of: ${VALID_SURFACE_ORIENTATIONS.join(', ')}`);
    }
    if (body.damageExtentEstimate !== undefined &&
        !VALID_DAMAGE_EXTENTS.includes(body.damageExtentEstimate)) {
      errors.push(`damageExtentEstimate must be one of: ${VALID_DAMAGE_EXTENTS.join(', ')}`);
    }
    if (body.secondaryDamageIndicators !== undefined) {
      if (!Array.isArray(body.secondaryDamageIndicators)) {
        ...
        const invalid = body.secondaryDamageIndicators.filter(
          (s: string) => !VALID_SECONDARY_INDICATORS.includes(s as SecondaryDamageIndicator)
        );
        if (invalid.length > 0) errors.push(`Invalid secondaryDamageIndicators values: ${invalid.join(', ')}`);
      }
    }
    if (body.photoStage !== undefined && !VALID_PHOTO_STAGES.includes(body.photoStage)) {
      errors.push(`photoStage must be one of: ${VALID_PHOTO_STAGES.join(', ')}`);
    }
    if (body.captureAngle !== undefined && !VALID_CAPTURE_ANGLES.includes(body.captureAngle)) {
      errors.push(`captureAngle must be one of: ${VALID_CAPTURE_ANGLES.join(', ')}`);
    }
    if (body.labelledBy !== undefined && !VALID_LABELLED_BY.includes(body.labelledBy)) {
    ...
    const labelFields = [
      'damageCategory','damageClass','s500SectionRef','roomType','moistureSource',
      'affectedMaterial','surfaceOrientation','damageExtentEstimate','equipmentVisible',
      'secondaryDamageIndicators','photoStage','captureAngle','labelledBy',
      'technicianNotes','moistureReadingLink',
    ] as const;
    ...
    if (Object.keys(updateData).length === 0) {
      return NextResponse.json({ error: "No label fields provided" }, { status: 400 });
    }
    ...
    const asbestosStopWork = hasStopWorkIndicator({
      secondaryDamageIndicators: (labels.secondaryDamageIndicators ?? []) as SecondaryDamageIndicator[],
    });
