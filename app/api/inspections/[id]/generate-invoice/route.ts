  ...
  try {
    ...
    if (inspection.scopeItems.length === 0) {
      return NextResponse.json(
        { error: "No selected scope items found. Select at least one scope item before generating an invoice." },
        { status: 400 }
      )
    }
    ...
    const runCreate = async () =>
      prisma.$transaction(async (tx) => {
        const sequence = await tx.invoiceSequence.upsert({
          ...
          create: { userId: session.user.id, year, prefix: "RA", lastNumber: 1 }
        })
        ...
        const invoice = await tx.invoice.create({
          data: {
            ...
            customerName,
            customerEmail,
            customerAddress: client?.address ?? inspection.propertyAddress ?? null,
    ...
    try {
      ...
      if (firstError?.code === "P2002") {
        ...
        await prisma.invoiceSequence.upsert({
          ...
          create: { userId: session.user.id, year, prefix: "RA", lastNumber: maxNum }
        })
        invoice = await runCreate()
      } else {
        throw firstError
