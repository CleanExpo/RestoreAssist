...
// Emits RestoreAssist's versioned structured scope contract (the machine-readable
// twin of the PDF scope), built off the same source so the two cannot drift.
  ...
  try {
    ...
    return new NextResponse(JSON.stringify(scope, null, 2), {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="scope-${id.slice(-8)}.json"`,
        "Cache-Control": "no-store",
      },
    });
  } catch (error) {
    return fromException(request, error, { stage: "scope-export" });
  }
}
