...
// Renders the plain-English S500 scope-of-works document (Markdown) from the same
// versioned contract the PDF and structured export use — one source, no drift.
export async function POST(
  ...
  try {
    ...
    const markdown = buildScopeNarrative(scope);

    return new NextResponse(markdown, {
      headers: {
        "Content-Type": "text/markdown; charset=utf-8",
        "Content-Disposition": `attachment; filename="scope-of-works-${id.slice(-8)}.md"`,
        "Cache-Control": "no-store",
      },
    });
  } catch (error) {
    return fromException(request, error, { stage: "scope-narrative" });
  }
}
