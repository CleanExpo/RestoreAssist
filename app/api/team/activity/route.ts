...
export async function GET(request: NextRequest) {
  try {
    ...
    if (roleFilter === "MANAGER")
      ...
      memberIds = teamMembers.filter((m) => m.role === "USER").map((m) => m.id);
    if (userIdParam && memberIds.includes(userIdParam))
      memberIds = [userIdParam];
    ...
    for (const report of recentReports) {
      ...
      const updatedLater =
        report.updatedAt.getTime() - report.createdAt.getTime() > 60_000;
      if (
        report.status === "COMPLETED" &&
        (report.completionDate || report.updatedAt > report.createdAt)
      ) {
        addActivity({
    ...
    for (const interview of recentInterviews) {
      ...
      addActivity({
        ...
        metadata: {
          ...
          formTemplateId: interview.formTemplateId,
        },
      });
    }
    ...
    return fromException(request, error, { stage: "team-activity" });
  }
}
