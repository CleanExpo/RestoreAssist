...
export async function GET(
  ...
  try {
    ...
    if (isIntegrationDevMode() && code === "mock-auth-code") {
      ...
      await prisma.integration.update({
        ...
        data: {
          ...
          companyId:
            provider === "SERVICEM8" || provider === "ASCORA"
              ? "mock-company-id"
              : null,
          config: null,
          lastSyncAt: new Date(),
      ...
      return NextResponse.redirect(
        new URL(
          `/dashboard/integrations?success=${encodeURIComponent(`${PROVIDER_CONFIG[provider].name} connected (dev mode)`)}`,
          baseUrl,
        ),
      );
    }
    ...
    return NextResponse.redirect(
      new URL(
        `/dashboard/integrations?success=${encodeURIComponent(`${PROVIDER_CONFIG[provider].name} connected successfully`)}`,
...
function redirectWithError(error: string, provider: string): NextResponse {
  ...
  return NextResponse.redirect(
    new URL(
      `/dashboard/integrations?error=${encodeURIComponent(error)}&provider=${provider}`,
      baseUrl,
    ),
  );
}
