...
export async function POST(request: NextRequest) {
  try {
      ...
      include: {
        ...
        inspection: {
          include: {
            scopeItems: true,
            costEstimates: true,
            classifications: { where: { isFinal: true }, take: 1, orderBy: { createdAt: 'desc' } },
          },
        },
      },
    })
    ...
    const totalExGSTDollars  = costEstimates.reduce((s, ce) => s + (ce.subtotal ?? 0), 0)
    const totalIncGSTDollars = costEstimates.reduce((s, ce) => s + (ce.total    ?? 0), 0)
    const fallback           = report.totalCost ?? 0

    const totalExGST  = Math.round((totalExGSTDollars  > 0 ? totalExGSTDollars  : fallback)       * 100)
    const totalIncGST = Math.round((totalIncGSTDollars > 0 ? totalIncGSTDollars : fallback * 1.1) * 100)
    ...
    const payload: NIRJobPayload = {
      ...
      scopeItems: (inspection?.scopeItems ?? []).map((item: any) => ({
        ...
        unit:           item.unit ?? 'each',
        unitPriceExGST: Math.round((costEstimates.find(ce => ce.scopeItemId === item.id)?.rate     ?? 0) * 100),
        gstRate:        10,
        subtotalExGST:  Math.round((costEstimates.find(ce => ce.scopeItemId === item.id)?.subtotal ?? 0) * 100),
...
function mapDamageType(raw?: string | null): 'WATER' | 'FIRE' | 'MOULD' | 'GENERAL' {
  if (!raw) return 'GENERAL'
  const u = raw.toUpperCase()
  if (u.includes('WATER') || u.includes('FLOOD') || u.includes('LEAK'))   return 'WATER'
  if (u.includes('FIRE')  || u.includes('SMOKE') || u.includes('SOOT'))   return 'FIRE'
  if (u.includes('MOULD') || u.includes('MOLD')  || u.includes('FUNGAL')) return 'MOULD'
  return 'GENERAL'
}
