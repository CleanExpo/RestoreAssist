...
describe("evaluateScopeQuality()", () => {
  ...
  describe("result shape", () => {
    it("returns all required score fields", () => {
      const result = evaluateScopeQuality(FULL_SCOPE, {
    ...
    it("returns details object with all required fields", () => {
      const { details } = evaluateScopeQuality(FULL_SCOPE, {
        claimType: "water_damage",
      });
  ...
  describe("structural score (## N. headings)", () => {
    it("full 7-section scope → structural = 100", () => {
      const { structural, details } = evaluateScopeQuality(FULL_SCOPE, {
        claimType: "water_damage",
      });
    ...
    it("empty scope → structural = 0, all sections missing", () => {
      const { structural, details } = evaluateScopeQuality(EMPTY_SCOPE, {
        claimType: "water_damage",
      });
...
## 4. Psychrometric Conditions
`;
      const { structural, details } = evaluateScopeQuality(partialScope, {
        claimType: "water_damage",
      });
...
`;
      const { structural } = evaluateScopeQuality(keywordScope, {
        claimType: "water_damage",
      });
    ...
    it("sectionsFound and sectionsMissing are disjoint and cover 1-7", () => {
      ...
      const all = new Set([
        ...details.sectionsFound,
        ...details.sectionsMissing,
      ]);
      for (let s = 1; s <= 7; s++) {
        expect(all.has(s)).toBe(true);
      }
      // No overlap
      const overlap = details.sectionsFound.filter((s) =>
        details.sectionsMissing.includes(s),
      );
  ...
  describe("citationDensity score", () => {
    it("7+ IICRC refs → citationDensity = 100", () => {
      ...
      const { citationDensity, details } = evaluateScopeQuality(richScope, {
        claimType: "water_damage",
      });
...
## 7. Sign-off
`;
      const { citationDensity, details } = evaluateScopeQuality(noRefScope, {
        claimType: "water_damage",
      });
...
## 7. Sign-off
`;
      const { citationDensity, details } = evaluateScopeQuality(threeRefScope, {
        claimType: "water_damage",
      });
    ...
    it("recognises §section references as IICRC refs", () => {
      ...
      const { details } = evaluateScopeQuality(sectionRefScope, {
        claimType: "water_damage",
      });
  ...
  describe("specificity score (hedging penalty)", () => {
    it("no hedging words → specificity = 100", () => {
      const { specificity, details } = evaluateScopeQuality(FULL_SCOPE, {
        claimType: "water_damage",
      });
    ...
    it("each hedging word reduces specificity by 15", () => {
      ...
      const { specificity, details } = evaluateScopeQuality(HEDGING_SCOPE, {
        claimType: "water_damage",
      });
...
## 7. Sign-off
`;
      const { specificity } = evaluateScopeQuality(maxHedgeScope, {
        claimType: "water_damage",
      });
    ...
    it('detects "as needed" as a hedging phrase (multi-word)', () => {
      ...
      const { details } = evaluateScopeQuality(scopeWithAsNeeded, {
        claimType: "water_damage",
      });
    ...
    it('does not penalise "4 adequate" — hedging followed by digit is exempt', () => {
      ...
      const scopeWithJustAdequate = FULL_SCOPE.replace(
        "Visual assessment",
        "Adequate visual assessment",
      );
      const { details } = evaluateScopeQuality(scopeWithJustAdequate, {
        claimType: "water_damage",
      });
  ...
  describe("categoryCompliance score", () => {
    it("no damage category → compliance = 70 (neutral — requirements unknown)", () => {
      const { categoryCompliance } = evaluateScopeQuality(FULL_SCOPE, {
        claimType: "water_damage",
      });
    ...
    it("Cat 2 scope with antimicrobial → compliance = 100", () => {
      const { categoryCompliance } = evaluateScopeQuality(
        CAT2_SCOPE_COMPLIANT,
        {
          claimType: "water_damage",
          damageCategory: 2,
        },
    ...
    it("Cat 2 scope without antimicrobial → compliance < 100", () => {
      const noAntimicrobialScope = CAT2_SCOPE_COMPLIANT.replace(
        /antimicrobial/gi,
        "treatment",
      );
      const { categoryCompliance } = evaluateScopeQuality(
        noAntimicrobialScope,
        {
          claimType: "water_damage",
          damageCategory: 2,
        },
    ...
    it("Cat 3 scope without PPE or porous removal → compliance < 100", () => {
      const noPPEScope = CAT3_SCOPE_COMPLIANT.replace(
        /PPE|remove porous/gi,
        "safety measures",
      );
  ...
  describe("composite score (weighted average)", () => {
    it("composite is correctly weighted: 0.30×structural + 0.25×citation + 0.20×equip + 0.15×specificity + 0.10×category", () => {
      ...
      const expected = Math.round(
        0.3 * r.structural +
          ...
          0.2 * r.equipmentAccuracy +
          0.15 * r.specificity +
          0.1 * r.categoryCompliance,
      );
    ...
    it("empty scope scores low (neutral equipment/specificity/category still apply)", () => {
      ...
      const r = evaluateScopeQuality(EMPTY_SCOPE, {
        claimType: "water_damage",
      });
    ...
    it("scope with sections and citations but heavy hedging scores lower than clean scope", () => {
      const clean = evaluateScopeQuality(FULL_SCOPE, {
        claimType: "water_damage",
      });
      const hedgy = evaluateScopeQuality(HEDGING_SCOPE, {
        claimType: "water_damage",
      });
    ...
    it("more IICRC references → higher composite (all else equal)", () => {
      ...
      const many = evaluateScopeQuality(FULL_SCOPE, {
        claimType: "water_damage",
      });
