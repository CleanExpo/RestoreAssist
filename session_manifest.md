...
- **AI Services Extraction Wave-1 SHIPPED.** Plan at `docs/superpowers/plans/2026-05-18-ai-services-extraction-wave.md`. All wave-1 tasks complete; 19/19 tests green across `lib/services/ai/`.
  ...
  - **Wave-2 plan landed at `15955267`** (`docs/superpowers/plans/2026-05-18-ai-services-extraction-wave-2.md`) — 7 tasks: streaming gateway foundation + 1 streaming migration + 4 batch migrations + docs.
  - **Wave-2 Task 1 SHIPPED at `84bd5180 → 394f6c43`.** `callAnthropicStream` added to `lib/services/ai/anthropic-gateway.ts` returning `ServiceResult<Anthropic.MessageStream, AnthropicReason>`. 7/7 stream tests + 7/7 existing batch tests = 14/14 across both suites. SDK ESM type paths required `MessageStreamParams` from `@anthropic-ai/sdk/resources/messages/messages` and `MessageStream` from `@anthropic-ai/sdk/lib/MessageStream` (deeper paths than wave-1's batch import — bundler-resolution requirement). APPROVED by spec + quality review.
  - **Wave-2 remaining (Tasks 2-7):** generate-scope streaming migration (the only streaming consumer), then 4 batch route migrations (`vision/extract-reading`, `sketches/import-from-image`, `ai/auto-classify-photo/[photoId]`, `support/tickets/route.ts`), then docs/manifest close-out. (Webhooks stay direct-SDK — legitimate boundary.)
