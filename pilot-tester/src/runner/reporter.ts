...
export async function writeReport(report: RunReport): Promise<WrittenReport> {
...
function renderMarkdown(report: RunReport): string {
  ...
  lines.push(`## Per-job`);
  lines.push("");
  lines.push("| Company | Job | Domain | Status | Det | Adj | Latency |");
  lines.push("|---|---|---|---|---|---|---|");
  for (const r of report.results) {
    ...
    lines.push(
      `| ${r.company.name} | ${r.job.key} | ${r.job.domain} | ${status} | ${det !== undefined ? det.toFixed(0) : "—"} | ${adj} | ${(r.durationMs / 1000).toFixed(1)}s |`,
    );
  }
  lines.push("");
