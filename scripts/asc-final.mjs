...
const REVIEW_URL =
  "https://appstoreconnect.apple.com/apps/6761808113/distribution/reviewsubmissions";
const VERSION_URL =
...
await page
  .goto(REVIEW_URL, { timeout: 30000, waitUntil: "domcontentloaded" })
  .catch(() => {});
...
const removeBtn = page
  .getByRole("button", { name: /Remove from Review/i })
  .first();
let removed = false;
...
for (let scroll of [500, 1000, 1500, 2000]) {
  ...
  if (await addBuild.isVisible({ timeout: 1000 })) {
    ...
    const allItems = await page
      .locator("td, li, tr, [role='option'], [role='row']")
      .all();
...
try {
  ...
  console.log("[asc] save skipped (disabled or not found)");
}
...
if (await submitBtn.isVisible({ timeout: 10000 })) {
  ...
  if (await conf.isVisible({ timeout: 3000 })) {
    await conf.click();
    await page.waitForTimeout(3000);
  }
