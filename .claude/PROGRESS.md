...
**Phase:** App Store Submission — Blocked on D-U-N-S propagation (~April 15)
**Last updated:** 2026-04-13

## App Store Status (2026-04-13)

| Task                                   | Status    | Notes                                                                                   |
| -------------------------------------- | --------- | --------------------------------------------------------------------------------------- |
| Android CI/CD (`android-release.yml`)  | Fixed     | Commit `7e966de1` — Java 21, pnpm setup, no Next.js build, correct signing flags        |
| Android AAB build                      | Verified  | Run `24318785151` — AAB built (4.78 MB), artifact saved until 2026-05-12                |
| Google Play org account (phill.mcgurk) | Started   | Org type + developer name "Restore Assist" entered — blocked at D-U-N-S step            |
| D-U-N-S 775125643 address propagation  | Pending   | Submitted April 8 via illion → propagates ~April 15 → retry Google Play + Apple then    |
| Google Play service account            | Needs fix | After new org account created: enable Android Publisher API + new service account JSON  |
| Apple Developer enrollment             | Pending   | Needs D-U-N-S propagation → go to developer.apple.com/programs/enroll/ as Organization  |
| Store listings content                 | Done      | `distribution/store-listings.md` created — full copy for both Google Play + App Store   |
| iOS GitHub secrets                     | Pending   | `IOS_CERTIFICATE_BASE64`, `IOS_CERTIFICATE_PASSWORD`, `IOS_PROVISIONING_PROFILE_BASE64` |
| Google Play store listing upload       | Pending   | After org account active + service account fixed                                        |
| App Store Connect listing + TestFlight | Pending   | After Apple enrollment + iOS cert/profile secrets added                                 |

### D-U-N-S Retry Instructions (April 15+)

1. Go to `play.google.com/console/signup` (signed in as `phill.mcgurk@gmail.com`)
2. Resume: Organization → Company → developer name "Restore Assist" → Payments profile
3. Create new payments profile → D-U-N-S `775125643`
4. After Play Console active: create service account → enable `androidpublisher.googleapis.com` → update `GOOGLE_PLAY_SERVICE_ACCOUNT_JSON` secret
5. Go to `developer.apple.com/programs/enroll/` → Organization → D-U-N-S `775125643`
...
## 2026-04-08 20:12 — Session End

## 2026-04-13 08:06 — Session End

## 2026-04-13 08:07 — Session End
...
## 2026-04-13 08:38 — Session End

## 2026-04-13 08:42 — Session End

## 2026-04-13 08:45 — Session End

## 2026-04-13 08:48 — Session End

## 2026-04-13 08:51 — Session End

## 2026-04-13 09:11 — Session End

## 2026-04-13 09:15 — Session End
